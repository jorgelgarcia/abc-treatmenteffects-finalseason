...
\singlespacing
\begin{abstract}
\noindent This paper estimates long-term benefits from an influential early childhood program targeting disadvantaged families. The program was evaluated by random assignment and followed participants through their mid-30s. It has significant positive impacts on health, children's future labor incomes, crime, education, and mothers' labor incomes, with greater monetized benefits for males. Returns are estimated using a systematic approach that pools multiple datasets based on economic principles. The overall rate of return is 13.7\% per annum, and the benefit/cost ratio is 7.3. These estimates are robust to numerous sensitivity analyses and remain significant when evaluated against treatment substitutes used by control-group families.
\end{abstract}
...
Since we are only interested in means for cost-benefit analysis, we can get by with the following statement about conditional means, which has testable implications, as we show below:
...
%\includepdf[pages=-]{abc_comprehensivecba_appendix-pub-aer.pdf}
