...
\noindent We construct the IPW using the same algorithm, independently of the age within $a \leq a^*$ in which an outcome is partially complete. For notational simplicity, we derive the IPW scheme without indexing the outcomes by age. We restore to the notation used throughout the text in the next appendix.\\
...
\noindent Using these synthetic control and treatment groups, we construct the dynamic relationships between outcomes and predictors in the auxiliary samples and use them to form predictions in the non-experimental sample. Then, we construct treatment effects for each outcome at each age. In our main estimation, we match both on $\bm{B}_k$ and $\bm{X}^d_{k}$. We next examine the sensitivity to the use of different matching variables next. We find little sensitivity.\\

\noindent \textbf{[JJH: We surely match on $\bm{B}$. (Be clear here).] [JLG: Yes. On $\bm{B}$ and $\bm{X}$ although there is little sensitivity. See next paragraph.] [JLG: The coefficients characterizing the dynamic relationship remain roughly the same when estimated across the different versions of synthetic treatment and control groups we use. That does not mean that we do not use the predictors. We use them, as stated and summarized next. It is the case, however, that the version of the auxiliary samples that we use to fit the dynamic relationship gives us very similar parameterizations. That is, there is little sensitivity. This is a version of invariance and it's something we were hoping to be true. Otherwise different samples would give different parameterizations. This is a statement about the production functions, not about the levels of the predictors in the synthetic treatment/control groups.]}\\

\noindent The lack of sensitivity implies that the dynamic relationship that we fit in the auxiliary sample is invariant to the sample that we use to estimate it, which favors Assumption~\ref{ass:summary}. This does not mean that the prediction is the same for the synthetic and control groups. It means that the parameterization is in agreement, but not the level of the predictors.\\

\noindent A non-parametric alternative to this procedure is to find a match (or a set of matches) $i''$ in the auxiliary sample for each individual $i$ in the experimental sample and impute the  dynamic trajectories of individual(s) $i''$ to individual $i$, using Algorithm~\ref{alg:match}. We present this alternative in Appendix~\ref{appendix:nonpar}.\\

\noindent \textbf{[JJH: Logically, we should find which X have have treatment effects.] [JLG: Yes. But when studying the variables available across datasets, we are basically down to PIAT, Education, and lagged income. This have treatment effects and we have no other option than relying on them.] [JLG: Yes. I agree. I have added this when describing the predictor variables.]}\\
...
\begin{subfigure}[h]{0.25\textwidth}
		...
		\includegraphics[width=\textwidth]{output/labor_20-65_pset8_mset4_female_sensitivity.eps}
\end{subfigure}%
\begin{subfigure}[h]{0.25\textwidth}
		\centering
...
\begin{subfigure}[h]{0.25\textwidth}
		...
		\includegraphics[width=\textwidth]{output/labor_20-65_pset8_mset4_male_sensitivity.eps}
\end{subfigure}%
\begin{subfigure}[h]{0.25\textwidth}
		\centering
...
\multicolumn{3}{c}{\underline{Labor Income}} & \multicolumn{3}{c}{\underline{Transfer Income}} \\ 
71,345 & 300,896 & 50,390 & -3,047.05 & -6,462.79 & 2,355.674  \\
(86,343) & (241,588) & (63,060) & (7,480.68) & (6,306.029) & (11,918.17) \\
\multicolumn{6}{l}{\textbf{b. Non-Parametric (This Section)}} \\
\multicolumn{3}{c}{\underline{Labor Income}}  &\multicolumn{3}{c}{\underline{Transfer Income}} \\ 
62,080 & 289,471 & 59,163 & -3,344.40 & 414.44 & -5,586.00 \\
(75,030) & (232,414) & (74,039) & (8,210.69) & (403.96) & (28,261.51) \\ \bottomrule
\end{tabular}
\begin{tablenotes}
\footnotesize
...
\noindent \textbf{[JJH: How do we pool?] [JLG: This should be clear in this current version from what we say above.] [JJH: It's not to me. We have three samples ---how are the observations for each used.] [This is stated in ``\textbf{Summary: Auxiliary Samples Used to Predict by Age}''. I wrote a bullet for each sample, together with the ages in which each of the samples are used.]}\\
...
\pagebreak
\noindent \textbf{[JJH: What if we do by gender?][JLG: By gender we loose all predicting power and the $R^2$'s drop. We can add them, but it doesn't look good at all.]}\\

\noindent \textbf{[JLG: Education (30) and Labor Income (21) predict labor income. We have trouble with transfer income; but it's what we have to predict. Treatment effects on transfer income are very minor.]} 
...
\begin{sidewaystable}[H]
\begin{threeparttable}
\caption{Prediction of Transfer Income at Age 30 Accounting for $R, \bm{B}_k, \bm{\theta},$ and $\bm{X}_{k,a}$ Female Sample, ABC/CARE}
\label{table:end3}
\centering
\scriptsize
...
\begin{sidewaystable}[H]
\begin{threeparttable}
\caption{Prediction of Transfer Income at Age 30 Accounting for $R, \bm{B}_k, \bm{\theta},$ and $\bm{X}_{k,a}$ Male Sample, ABC/CARE}
\centering
\scriptsize
...
\paragraph{Using Mincer Functions to Forecast Parental Income} \label{appendix:mincerpar}
...
\subsection{Inference Procedures} \label{appendix:bootstrap}
...
\item Randomly pair individual $i'$ in $s'$ with individual $i$ in $s$. The prediction accounting for forecasting error is $\bm{Y}_{i,e,s}^d + \bm{\mathcal{E}}_{i',n,s'}^d$. 
...
\item For an incomplete outcome (an outcome after age $a^*$), we construct a prediction for treatment- and control-group individuals following the steps in Appendix~\ref{appendix:bootstrapspreds}. \textbf{[JJH: The weighting part of Algorithm~\ref{alg:match} has been dropped. Restore!] [JLG: Yes, I omitted this part, which is part of our construction of the empirical bootstrap distribution.]}
...
\noindent \textbf{[JJH: This is good -But how do we pool the different samples? Just add them back in?] [JLG: Yes, I'm clarifying this in an additional summary at the end of Appendix C.3.1.]}