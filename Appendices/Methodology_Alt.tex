...
\mathbb{E}[Y^r] &= \int \int y f_{Y|\bm{B} = b,R=r, A=1} (y | \bm{B} = b,R=r, A=1) f_{\bm{B}} (b|R=r) dydb
\end{align}
...
\noindent where the variables are defined analogously to Equation~\eqref{eq:outcome2}. Note that the arguments of $\phi^d_{k,a}$ are still vectors in this case, and we abuse notation to make unbolded variables generic scalars. Define an associated measurement system for the error term $\varepsilon^d_{k,a}$:
...
\item \textbf{[JJH: We say estimates are ``close'' - but NPV estimates are wildly diferent - how to explain this? Close in what metric?] [JLG: The metric is that, in any of these specifications, the program is still socially efficient. The NPVs are different because these are very much different processes with large standard errors in our samples. It's unrealistic to expect perfect alignment. If you look at the IRRs and BC ratios, we would hardly ever reject equality. Look at the standard errors.]}
\end{tablenotes}
\end{threeparttable}
}
\end{sidewaystable}
...
\noindent \textbf{[JJH: Jorge - which steps - if any - are actually done. How much we drop?]}\\

\noindent \textbf{[JLG: The GMM section was dropped. It served its purpose in case a question were to come out in a seminar but it didn't and it certainly makes it look like we are doing what we do not do.]}\\
...
\item Resample the non-experimental sample with replacement at the individual level. When an individual is resampled, his/her whole life-cycle history is considered. Denote this sample by $(n,s')$. \textbf{[JLG: You were right that steps 2 and 3 were flipped]}
...
\item Use the fitted dynamic relationship to fit out of sample in $(e,s)$. This gives us an age-by-age forecast \textit{without forecasting error} for each individual in the experimental sample. Store the predictions at all ages for individual $i$ in this sample in a vector $\mathcal{Y}_{i,e,s}$, where $\mathcal{Y}_{i,e,s}$ is the vector of forecasts for individual $i$ in the experimental bootstrap sample $s$. \textbf{[JJH: Each draw is a life cycle path?] [JLG: Correct. I have clarified this in items 1 and 3 of this list.]}
...
\item Randomly pair individual $i$ in sample $(e,s)$ with individual $j$ in sample $(n,s')$. This random pairing is stratified by the variables $\bm{B}_k$ and $\bm{X}_{k,a}^d$ defined in the paper. The forecast accounting for forecasting error is $\mathcal{Y}_{i,e,s}+ \bm{\mathcal{E}}_{j,e,s'}$. \textbf{[JJH: Shouldn't this be done conditional on regressors? We are certainly conditioning on X or do we invoke independence of errors here?] [JLG: Yes. I am sorry this wasn't here. It's a stratified pairing and our support conditions allow us to do this without empty cells issues.]}
...
\noindent \textbf{[JJH: What about standard for the second -nonparametric- matching procedure?] [JLG: It comes next. We add back the the parametric residuals. I think that we could do better; but also believe that this is valid. We are estimating the conditional mean function with non-parametrically and the residual parametrically. This makes the computation really simple.]}
...
\item Perform the procedure to construct the synthetic cohort, estimate the dynamic relationship and impute the profiles by matching on baseline variables and on variables affected by treatment as explained in Appendix~\ref{appendix:nonpar}. Store the predictions at all ages for individual $i$ in this sample in a vector $\mathcal{\tilde{Y}}_{i,e,s}$, where $\mathcal{\tilde{Y}}_{i,e,s}$ is the vector of forecasts for individual $i$ in the experimental bootstrap sample $s$.
