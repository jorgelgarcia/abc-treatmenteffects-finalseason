...
\noindent \textbf{[JLG: Note that in (1) the conditioning actually says $A = 1$. The printing faints and it looks a $\Lambda = 1$. It is not.]}
...
\noindent Table~\ref{table:predsens} compares the results from the approach we use throughout the main text and the non-parametric approach that we introduce in this section. We present results for labor income. We use pre- and post-treatment variables to match (using the same variables as the main approach of Appendix~\ref{appendix:match}). We present the discounted net present value (treatment - control) for labor income in 2014 dollars, together with the internal rates of return and benefit-cost ratios associated to these net present values. Table~\ref{table:predsens} also presents results from other specifications that we discuss below in Appendix~\ref{appendix:predsensitivity}. The approaches are in close agreement.
...
\begin{threeparttable}
\caption{Net Present Value of Labor Income and Benefit/Cost Analysis Under Different Specifications for Labor Income Forecasts}
...
\item \\
\item \textbf{[JLG: Please revisit this table. Specification 1 was obviously outdated and that's why it jumped out to you as very different from the rest of the specifications. We seem to have some decent consistency across the board but, more importantly, between Specification 1 and 5 (nonparametric).]}
\end{tablenotes}
\end{threeparttable}
}
\end{sidewaystable}
...
\noindent \textbf{[JJH: What about standard errors for the second -nonparametric- matching procedure?] [JLG: It comes next. We add back the the parametric residuals. I think that we could do better; but also believe that this is valid. We are estimating the conditional mean function non-parametrically and the residual parametrically. This makes the computation really simple.] [JJH: But can we check the other way? How much difference does it make?] [JLG: Yes. Let us clear G and this won't take more than a morning.] [JJH: OK - Let's move on!] [JLG: Here are the net present values (gain in terms of labor income). Parametric strategy:  pooled 133,032 (s.e.\ 76,634); males 238,256 (s.e.\ 185,375); females 41,908 (s.e.\ 24,606). Non-parametric forecast with parametric adjustment for forecasting error:  pooled 132,924 (s.e.\ 11,253); males 195,530 (s.e.\ 20,210); females 69,317 (s.e.\ 4,350). Non-parametric forecast with non-parametric adjustment for forecasting error: pooled 137,133 (s.e.\ 19,989); males 204,256 (s.e.\ 32,212); females 64,522 (s.e.\ 9,987). The difference between the last two is really trivial, and given that that is what's in right now I would advocate for this to be good for our mental health but not to include it. It's really your call, though. It would take me a bit to run the computations to report the IRRs and B/Cs for completing Table C.12 because then I need to re-run the whole thing instead of only producing the NPVs associated to labor income and at the end the different in IRRs and B/Cs with the decimals that we report would probably be practically zero.]}
...
\item Compute the residuals in step 7 of Appendix~\ref{appendix:bootstrapspreds}.
