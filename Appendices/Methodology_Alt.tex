...
\item \textbf{NLSY79:} male and birth year. 
\item \textbf{PSID:} male and birth year.
\item \textbf{CNLSY:} male and birth year.

\noindent In addition, recall that we place initial restrictions on the auxiliary sample. See Appendix~\ref{app:datasets}.\footnote{We experimented with other variables to construct the synthetic treatment and control groups and found little sensitivity in our estimates. Results using various other sets of variables to construct the synthetic groups are available from the authors upon request.}
\end{enumerate}
