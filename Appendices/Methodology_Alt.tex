...
This appendix presents our approach for identifying and estimating life-cycle treatment effects. Differences in the approach used for each outcome are based on different scenarios of data availability. We proceed as follows. Appendix~\ref{app:method_fullobs} focuses on outcomes that are fully observed over the course of the experiment with little attrition. Appendix~\ref{app:method_partialobs} focuses on outcomes that are partially observed over the course of the experiment with substantial rates of attrition. Appendix~\ref{app:method_noobs}  focuses on outcomes that we do not observe, and thus need to forecast out of sample. Appendices~\ref{app:method_irr} and ~\ref{app:method_cbratio} provide technical details on the computations of the internal rate of return and the benefit/cost ratios, respectively. Appendix~\ref{appendix:bootstrap} provides the steps for constructing our statistical inference. Finally, Appendix~\ref{appendix:results} describes our procedure for selecting the background variables that we condition on.
...
\noindent This subsection provides a framework for testing Assumption~\ref{ass:exog}. Recall from the main paper that our forecasts are based on the vector of production functions for output $\bm{Y}^d_{k,a}$:
\begin{equation}
\bm{Y}^d_{k,a} = \bm{\phi}^d_{k,a} (\bm{X}^d_{k,a}, \bm{B}_k) \ + \ \bm{\varepsilon}^d_{k,a},  \\
\end{equation}
...
\begin{equation}
Y^d_{k,a} =\phi^d_{k,a} (\bm{X}^d_{k,a}, \bm{B}_k) \ + \ \varepsilon^d_{k,a},  \\
\end{equation}
...
\item Compute the residuals in step 7 of Appendix~\ref{appendix:bootstrapspreds}.\footnote{When we compute the residuals using the fully nonparametric model we get the following net present value for labor income: pooled (males and females) 137,133 (s.e.\ 19,989); males 204,256 (s.e.\ 32,212); females 64,522 (s.e.\ 9,987). Table~\ref{table:predsens} shows that these results are closely aligned to the two approaches reported in the paper.}
