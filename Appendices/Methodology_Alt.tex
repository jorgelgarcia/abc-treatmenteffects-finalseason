...
\noindent As noted in the text, our strategy is developed and implemented in two stages. In Stage I, we find comparison groups that are ``comparable'' using different criteria documented below. This can be thought of as a type of matching. The matching could be coarse or fine. We use a variety of matching criteria: (a) constructing coarse matches using a subset of variables in $\mathcal{B}$; (b) using the full set of eligibility criteria embodied in $B \in \mathcal{B}$; (c) matching on variables $X^d_{k,a}, k\in \{e,n\}, a\in \{1, \dots, A\}$ that are (i) affected by treatment, (ii) are exogenous (in the sense of being unrelated to model error terms (e.g., as in Equation~\ref{eq:outcome}),\footnote{As noted in the text, for mean treatment effect forecasts we only need exogeneity of the predictors with regard to the \emph{differences} in the errors across treatment and control, not the levels.} and (iii) are available in both the experimental and nonexperimental samples. Stage II uses the constructed samples to build models to make out of (experimental) sample forecasts, assuming structural invariance (Assumption~\ref{ass:summary}). Stage II  might be executed in several ways.\\

\noindent The most direct way is to use matching on common variables in both samples and to use post-$a^*$ realizations in the comparison samples to construct forecasts. This approach is most effective when the $\bm{X}^d_{k,a}$ are age invariant, $k \in \{e,n\}, a \in\{1,\dots, A\}$. We implement this approach in this appendix. We also consider a parametric model-based approach that estimates Equation~\ref{eq:outcome} in the main text in the experimental and comparison samples and extrapolates using the comparison samples. We adopt both approaches for comparison samples constructed in different ways. We find a surprising robustness across approaches in our estimates that inspires confidence in the benefit/cost estimates reported in the text.\\
...
\noindent Matched samples can be constructed in many ways using the various criteria listed at the end of Section~\ref{app:datasets}. One method that combines Stage I (sample construction) and Stage II (estimation) is the following. This provides a nonparametric approach to forecasting. It is conditional on a common value of $\bm{B}$ in both samples.\\

\noindent Match an individual $l(i)$ in the auxiliary sample to person $i$ in the treatment samples to find synthetic treatment and control groups by following Algorithm~\ref{alg:match}.\\
...
\noindent We can also use synthetic control and treatment groups constructed in various ways to estimate dynamic relationships between outcomes and predictors in the auxiliary samples and use them to form predictions in the non-experimental sample. In this model-based approach, we construct treatment effects for each outcome at each age. In our main approach to estimation, we match both on $\bm{B}_k$ and $\bm{X}^d_{k}$. We next examine the sensitivity to the use of different matching variables next. We find little sensitivity to choices of matching variables and to using parametric or non-parametric approaches. The lack of sensitivity suggests that the dynamic relationship that we fit in the auxiliary sample is invariant to the sample used to estimate it, which supports exogeneity Assumption~\ref{ass:summary}.\\
