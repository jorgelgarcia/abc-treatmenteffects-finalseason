...
\noindent \textbf{[JLG: This is the corresponding adaptation of Appendix C, accounting for the few changes in the labels of the assumptions and the new test of invariance across the experimental and non-experimental samples that I am proposing. The rest remains unchaged.]}
...
\noindent When producing our predictions, we have two alternatives: (i) directly use the auxiliary samples; or (ii) use a weighted version of the auxiliary samples so that they more closely align with respect to observed characteristics. We follow the second alternative and match the experimental and auxiliary or non-experimental samples following the procedure in Section~\ref{sec:usingaux}. We construct the weights using the Mahalanobis metric of equation \eqref{eq:ingrownnail} of Section~\ref{sec:usingaux}. This allows us to weigh the individuals in the non-experimental samples to match them on observed characteristics to the individuals using the Mahalanobis metric based on male and black indicators, year of birth, years of education at age 30, and labor income at age 30. We choose these variables because they are available in all three non-experimental samples and we intend to locate common comparison samples.
...
\subsection{Testing  Assumptions~\ref{ass:summary} and~\ref{ass:invariance}} \label{app:invariance}

\noindent In this section we provide tests for Assumptions~\ref{ass:summary} and ~\ref{ass:invariance}. These assumptions, together with Assumptions~\ref{ass:exog} and ~\ref{ass:exogne}, imply the following null hypotheses. We test Assumptions~\ref{ass:summary} and ~\ref{ass:invariance} in this subsection and Assumptions~\ref{ass:exog} and ~\ref{ass:exogne} in the next.

\begin{eqnarray}
\phi_{j,t}^0 \left( \bm{X}_{t}^d, \bm{B} \right) &=& \phi_{j,t}^1 \left( \bm{X}_{t}^1 \right) \\ \nonumber 
&:= & \phi_{j,t} \left( \bm{X}_{t}^d, \bm{B} \right) \label{eq:firstimply}
\end{eqnarray}

\noindent and \\
 
\begin{eqnarray}
\phi_{j,t} \left( \bm{X}_{t}^d, \bm{B} \right) = \widetilde{\phi}_{j,t} \left( \bm{X}_{t}^d, \bm{B} \right). \label{eq:secondimply}
\end{eqnarray}

\noindent First, we test Assumption~\ref{ass:summary}. In the main text, we show that this implies: 
...
&=&\mathbb{E} \left[ Y_{j,t} | \bm{X}_{t}  = \bm{x}_{t}, \bm{B} =  \bm{b} \right]. \label{eq:invariancetestapp}
\end{eqnarray}

\noindent That is, a \textbf{direct} test is to use the experimental sample and ask if once we account for a set of the variables in $\bm{X}_{t}$, $R$ (randomization to treatment assignment in ABC/CARE) predicts the outcome of interest, conditional on $\bm{B}$. This tests if this specific set of $\bm{X}_{t}$ suffices to summarize the treatment described by $R$. The set of tests for this includes: PIAT scores at ages 5 to 7, education at age 30, and labor income at age 21. The reason why we do not use the the full set if variables in  $\bm{X}_{t}$ is that  $\bm{X}_{t}$ includes variables that are outcomes themselves, such as body-mass index and labor income at age 30. Note that $R = D$ in our main methodology. That is, children who are offered treatment attend it. For the three main outcomes we predict, once we condition on $\bm{B}, \bm{X}_{t}, \bm{\theta}$, the coefficient associated to $R$ is not significant. We test this with and without accounting for endogeneity, as explained in the main text (see Section~\ref{section:cbamethodology}).
...
\begin{sidewaystable}[H]
\begin{threeparttable}
\caption{Prediction of Body-Mass Index at Age 34 Accounting for $R, \bm{B}, \bm{\theta},$ and $\bm{X}_{t}$ Pooled Sample, ABC/CARE}
\centering
\scriptsize
...
\begin{sidewaystable}[H]
\begin{threeparttable}
\caption{Prediction of Body-Mass Index at Age 34 Accounting for $R, \bm{B}, \bm{\theta},$ and $\bm{X}_{t}$ Females Sample, ABC/CARE}
\centering
\scriptsize
...
\begin{sidewaystable}[H]
\begin{threeparttable}
\caption{Prediction of Body-Mass Index at Age 34 Accounting for $R, \bm{B}, \bm{\theta},$ and $\bm{X}_{t}$ Males Sample, ABC/CARE}
\label{table:end3}
\centering
\scriptsize
...
\begin{sidewaystable}[H]
\begin{threeparttable}
\caption{Prediction of Reports Very Good Health at Age 40 Accounting for $R, \bm{B}, \bm{\theta},$ and $\bm{X}_{t}$ Pooled Sample, Perry Preschool Project}
\label{table:perry3}
\centering
\scriptsize
...
\begin{sidewaystable}[H]
\begin{threeparttable}
\caption{Prediction of Reports Very Good Health at Age 40 Accounting for $R, \bm{B}, \bm{\theta},$ and $\bm{X}_{t}$ Females Sample, Perry Preschool Project}
\label{table:perry3}
\centering
\scriptsize
...
\begin{sidewaystable}[H]
\begin{threeparttable}
\caption{Prediction of Reports Very Good Health at Age 40 Accounting for $R, \bm{B}, \bm{\theta},$ and $\bm{X}_{t}$ Males Sample, Perry Preschool Project}
\label{table:perry3}
\centering
\scriptsize
...
\noindent Second, we test Assumption~\ref{ass:invariance}. In the main text, we show that this implies: 

\begin{equation}
\mathbb{E} \left[ Y_{j,t} | \bm{X}_{t} = \bm{x}_{t}, \bm{B}, S = s \right] = \mathbb{E} \left[ Y_{j,t} | \bm{X}_{t} = \bm{x}_{t}, \bm{B} \right]. \label{eq:withbetimplicationapp}
\end{equation}

\noindent The test is analogous to that of Assumption~\ref{ass:summary}. We ask if, when pooling the experimental and the non-experimental samples, the indicator associated with being in the experimental ($S = 1$) or non-experimental ($S = 0$) sample is significantly different from zero. We are able to test this at $t = t^*$, where we observe the predicted outcomes at ages 30. As we explain before, our non-experimental source at $t = t^*$ is the CNLSY. Tables~\ref{table:inv0} to \ref{table:invfin}. Once we account for our complete set of prediction variables in $\bm{B}, \bm{X}_{t}$ we do not find strong evidence against Assumption~\ref{ass:invariance}. Body-mass index at age 34 is an expected exception given the noticeable outliers in the ABC/CARE sample. 

\begin{sidewaystable}[H]
\begin{threeparttable}
\caption{Prediction of Labor Income at Age 30 Accounting for $R, \bm{B}, \bm{\theta},$ and $\bm{X}_{t}$ Pooled Sample, ABC/CARE and CNLSY}
\label{table:inv0}
\centering
\scriptsize
...
\begin{sidewaystable}[H]
\begin{threeparttable}
\caption{Prediction of Labor Income at Age 30 Accounting for $R, \bm{B}, \bm{\theta},$ and $\bm{X}_{t}$ Females Sample, ABC/CARE and CNLSY}
\label{table:end2}
\centering
\scriptsize
...
\begin{sidewaystable}[H]
\begin{threeparttable}
\caption{Prediction of Labor Income at Age 30 Accounting for $R, \bm{B}, \bm{\theta},$ and $\bm{X}_{t}$ Males Sample, ABC/CARE and CNLSY}
\label{table:end2}
\centering
\scriptsize
...
\begin{sidewaystable}[H]
\begin{threeparttable}
\caption{Prediction of Transfer Income at Age 30 Accounting for $R, \bm{B}, \bm{\theta},$ and $\bm{X}_{t}$ Pooled Sample, ABC/CARE and CNLSY}
\label{table:end3}
\centering
\scriptsize
...
\begin{tablenotes}
\footnotesize
\item Note: Prediction of transfer income at age 30 based on the variables listed in the row using the ABC/CARE and the CNLSY sample constructed according to the procedure in Section~\ref{section:cbamethodology}. Empty cells indicate that the variable was not used in the prediction. For each coefficient we provide point estimate and $p$-value. $\hat{\bm{\theta}}_{c}$: factor score estimated based on the measurement system in \eqref{eq:sa-msystemmain} and PIAT sections that we do not use to predict (reading and comprehension) as well as by the Peabody Picture Vocabulary Test (PPVT) in CNLSY and IQ at ages 2, 3, 4, 5, 7, and 8 in ABC/CARE (cognitive skill). $\hat{\bm{\theta}}_{n}$: factor score estimated based on the measurement system in \eqref{eq:sa-msystemmain} and on six scales of the Behavior Problems Index (e.g., anxiety, dependency, social behavior) in CNLSY and measures of somatization, hostility, depression, and a global mental health index at age 21 in ABC/CARE (non-cognitive skill). Inference is based on the empirical bootstrap distribution. If the estimates for the constant terms are in the ten or hundred thousands, we report a figure that has been rounded to the thousands.
\end{tablenotes}
\end{threeparttable}
\end{sidewaystable}
...
\begin{sidewaystable}[H]
\begin{threeparttable}
\caption{Prediction of Transfer Income at Age 30 Accounting for $R, \bm{B}, \bm{\theta},$ and $\bm{X}_{t}$ Females Sample, ABC/CARE and CNLSY}
\label{table:end3}
\centering
\scriptsize
...
\begin{sidewaystable}[H]
\begin{threeparttable}
\caption{Prediction of Transfer Income at Age 30 Accounting for $R, \bm{B}, \bm{\theta},$ and $\bm{X}_{t}$ Males Sample, ABC/CARE and CNLSY}
\centering
\scriptsize
...
\begin{sidewaystable}[H]
\begin{threeparttable}
\caption{Prediction of Body-Mass Index at Age 34 Accounting for $R, \bm{B}, \bm{\theta},$ and $\bm{X}_{t}$ Pooled Sample, ABC/CARE and CNLSY}
\centering
\scriptsize
\input{output/abccarecnlsy_invariance_si34y_bmi_spooled.tex}
\begin{tablenotes}
\footnotesize
\item Note: Prediction of body-mass index at age 34 based on the variables listed in the row using the ABC/CARE and the CNLSY sample constructed according to the procedure in Section~\ref{section:cbamethodology}. Empty cells indicate that the variable was not used in the prediction. For each coefficient we provide point estimate and $p$-value. $\hat{\bm{\theta}}_{c}$: factor score estimated based on the measurement system in \eqref{eq:sa-msystemmain} and PIAT sections that we do not use to predict (reading and comprehension) as well as by the Peabody Picture Vocabulary Test (PPVT) in CNLSY and IQ at ages 2, 3, 4, 5, 7, and 8 in ABC/CARE (cognitive skill). $\hat{\bm{\theta}}_{n}$: factor score estimated based on the measurement system in \eqref{eq:sa-msystemmain} and on six scales of the Behavior Problems Index (e.g., anxiety, dependency, social behavior) in CNLSY and measures of somatization, hostility, depression, and a global mental health index at age 21 in ABC/CARE (non-cognitive skill). Inference is based on the empirical bootstrap distribution. If the estimates for the constant terms are in the ten or hundred thousands, we report a figure that has been rounded to the thousands.
\end{tablenotes}
\end{threeparttable}
\end{sidewaystable}
...
\begin{sidewaystable}[H]
\begin{threeparttable}
\caption{Prediction of Body-Mass Index at Age 34 Accounting for $R, \bm{B}, \bm{\theta},$ and $\bm{X}_{t}$ Females Sample, ABC/CARE and CNLSY}
\centering
\scriptsize
...
\begin{sidewaystable}[H]
\begin{threeparttable}
\caption{Prediction of Body-Mass Index at Age 34 Accounting for $R, \bm{B}, \bm{\theta},$ and $\bm{X}_{t}$ Males Sample, ABC/CARE and CNLSY}
\label{table:invfin}
\centering
\scriptsize
...
\begin{sidewaystable}[H]
\begin{threeparttable}
\caption{Prediction of Transfer Income at Age 30 Accounting for $\bm{B}$ and $\bm{\theta}, \bm{X}_{t}$, CNLSY} \label{table:endogti}
\centering
\footnotesize
