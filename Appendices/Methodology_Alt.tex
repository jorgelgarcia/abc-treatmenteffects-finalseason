...
\textbf{[TR: In the table below, do you mean to say ``Total Years of Education" instead of ``Total Number of special education"? What are the ``Ever Retained'' and ``Total Number Retained'' lines referring to?]}
\begin{table}[H]
...
When constructing the samples used to forecast transfer and labor income \textbf{[TR: Quick note: earlier in the section you talk about ``interpolating'' the subject's labor and transfer income. But for consistency's sake and to avoid confusion with the interpolation of PSID/NLSY data, it might be good to change those previous instances to say ``forecasting the subject's labor income'']}, we place the following restrictions on the admissible samples \textit{for all ages}. Additional restrictions are placed when forecasting the other outcomes, as we explain below.
\begin{enumerate}
...
This appendix uses the framework presented in Appendix~\ref{app:endogeneity} to test Assumption~\ref{ass:summary}. The tests are made under the null hypothesis that Assumption~\ref{ass:exog} holds. In the main text \textbf{[TR: specifically, in section 3.3.5 (or Equation 10) of the main text]} we show that Assumption~\ref{ass:summary}, together with Assumption~\ref{ass:exog}, implies:
...
\noindent In the main text, \textbf{[TR: Specifically, in Section 3.3.5]} we show that Assumption~\ref{ass:summary}, together with Assumption~\ref{ass:exog}, implies equality of conditional expectations in the experimental and auxiliary samples.
...
\item Resample the experimental sample with replacement at the individual level. This gives us a new (re-sampled) panel dataset. Information on the whole story about each individual is obtained in each re-sample.
...
\item Choose the control set that best predicts $Y_{j}$, as we describe in Appendix~\ref{appendix:bvariables}. Call this control set $k^*_{j}$. There is one control set for each of the partially complete outcomes $Y_{j}$.
