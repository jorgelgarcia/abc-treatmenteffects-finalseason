...
\noindent As noted in the text, our strategy is developed and implemented in two stages. In Stage I, we find comparison groups that are ``comparable'' using different criteria documented below. This can be thought of as a type of matching. We explore a variety of criteria: (a) A very crude match using a subset of variables in $\mathcal{B}$; (b) The full set of eligibility criteria embodied in $B \in \mathcal{B}$; (c) Variables $X^d_{k,a}, k\in \{e,n\}, a\in \{1, \dots, A\}$ that are (i) affected by treatment, (ii) are exogenous (in the sense of being unrelated to model error terms (e.g., as in Equation~\ref{eq:outcome}), and (iii) are available in both the experimental and nonexperimental samples. Stage II uses the constructed samples to make out of (experimental) sample forecasts, assuming structural invariance (Assumption~\ref{ass:summary}). Stage II  might be executed in several ways.\\

\noindent The most direct way is to use matching on common variables in both samples and to use post-$a^*$ realizations in the comparison samples to construct forecasts. This approach is most effective when the $X^d_{k,a}$ are age invariant, $k \in \{e,n\}, a \in\{1,\dots, A\}$. We implement this approach in this appendix. We also consider a parametric model-based approach that estimates Equation~\ref{eq:outcome} in the main text in the experimental and comparison samples and extrapolates using the comparison samples. We adopt both approaches for comparison samples constructed in different ways. We find a surprising robustness across approaches in our estimates that inspires confidence in the benefit/cost estimates reported in the text.\\
...
\noindent The first principle for finding non-experimental synthetic treatment or controls group members is that they be eligible for the program, i.e., $\bm{B}_n \in \mathcal{B}_{0}$. One requirement is that the variables be present in both experimental and control samples. The harder challenge is using these variables to find counterparts to the treated and the controlled in the non-experimental samples where no one directly receives treatment.\\

\noindent Consider outcome $Y^d_{k,j,a}$ generated by $\bm{B}_k, \bm{X}^d_{k,j,a}$ (defined as the relevant predictor variables for $Y_{k,j,a}$) and also generated by $\varepsilon^d_{k,j,a}$. Under exogeneity (Assumption~\ref{ass:exog}), we can match on both $\bm{B}_k$ and $\bm{X}^d_{k,j,a}$. This assumption allows us to  use variables $\bm{X}^d_{k,j,a}$ that are caused by treatment and that predict outcome $j$ and are invariant after $a^\ast$ (e.g., ability). If exogeneity is not satisfied, matching on $\bm{X}^d_{k,j,a}$ becomes problematic (see \citealp{Heckman_Navarro_2004_REStat}). Note that we can match on $\bm{B}_k$ within the the set of individuals in the auxiliary samples such that $\bm{B}_k \in \mathcal{B}_{0}$. This is preferable in scenarios in which Assumption~\ref{ass:exog} is violated because the entire analysis is conditional on $\bm{B} \in \mathcal{B}$. Below, we show that our estimates are not sensitive to whether we match on $\bm{B}_k$, match on $\bm{X}^d_{k,j,a}$ alone, or match on both $\bm{B}_k$ and $\bm{X}^d_{k,j,a}$.\\

\noindent For the moment, simplify notation and assume that $\bm{B}_k$ is absorbed into $\bm{X}^d_{k,j,a}$. We relax this assumption below. For each treatment group member $i$ in treatment status $d$, we find counterparts following the analysis of \citet{Heckman_Ichimura_etal_1998_REStud}. We do not construct different synthetic treatment and control groups for each age and for each outcome that we predict. We find one synthetic treatment and one synthetic control group in each auxiliary sample and use these samples to predict each outcomes at each age. We explain in Appendix~\ref{app:datasets} what auxiliary samples are used to predict outcomes at each age. That is, we use the same synthetic treatment and control groups to predict all of the outcomes. For this reason, we drop the age and outcome subindices for the rest of this section.\\

\noindent Matched samples can be constructed in many ways using the various criteria listed at the end of Section~\ref{app:datasets}. One method that combines Stage I (sample construction) and Stage II (estimation) is the following. This provides a nonparametric approach to forecasting.\\

\noindent Match an individual $l(i)$ in the auxiliary sample for whom $\bm{B}_{l(i),n} \in \mathcal{B}_{0}$ to find synthetic treatment and control groups by following Algorithm~\ref{alg:match}.\\

\onehalfspacing
\begin{algorithm} \label{alg:match}
For individual $i$ in experimental sample ($k=e$), an individual $l(i)$ in the auxiliary sample ($k=n$) for whom $\bm{B}_{l(i),n} \in \mathcal{B}_{0}$ is a potential counterpart if
...
\noindent We can also use synthetic control and treatment groups constructed in various ways to estimate the dynamic relationships between outcomes and predictors in the auxiliary samples and use them to form predictions in the non-experimental sample. In this model-based approach, we construct treatment effects for each outcome at each age. In our main estimation, we match both on $\bm{B}_k$ and $\bm{X}^d_{k}$. We next examine the sensitivity to the use of different matching variables next. We find little sensitivity to choices of matching variables and to using parametric or non-parametric approaches.\\

\noindent The lack of sensitivity suggests that the dynamic relationship that we fit in the auxiliary sample is invariant to the sample that we use to estimate it, which supports exogeneity Assumption~\ref{ass:summary}.\\

\noindent A non-parametric alternative to the model-based procedure is to find a match (or a set of matches) $l(i)$ in the auxiliary sample for each individual $i$ in the experimental sample and impute the dynamic trajectories of individual(s) $l(i)$ to individual $i$, using Algorithm~\ref{alg:match}. We develop this alternative in Appendix~\ref{appendix:nonpar}.\\
...
\begin{tablenotes}
\footnotesize
\item Note: All columns display regressions of labor income at age 30 on the different variables listed in the rows. If the space for the coefficient appears empty, it was not included in the regression. All money figures are in 2014 USD. The parentheses next to the variable indicates the age of measurement. Education is measured as years of education. Robust standard errors are in parentheses. We weigh the individuals in the auxiliary samples to match them on observed characteristics to the individuals using the procedure in Appendix~\ref{appendix:match}. $^{***}$: $p$-value $< .01$. $^{**}$: $p$-value $< .05$. $^{*}$: $p$-value $< .10$. \textbf{[JJH: These are model based?] [JLG: correct.]}
\end{tablenotes}
\end{threeparttable}
\end{sidewaystable}
...
\multicolumn{3}{c}{\underline{Labor Income}} \\
...
\noindent \textbf{[JJH: What if we do by gender?] [JLG: By gender we lose all predicting power and the $R^2$'s drop. We can add them, but it doesn't look good at all.] [JJH: Place in auxiliary appendix.]}
...
\paragraph{Using Mincer Equations to Forecast Parental Income} \label{appendix:mincerpar}
...
\noindent This section provides the precise steps for constructing the bootstrap distribution and for computing the standard errors for three of the main estimates in our paper. \textbf{[JJH: What estimates?] [JLG: the only estimates missing to describe how do we do the inference are the combining functions, and I will add the steps. I think it's important.]}
