...
\noindent In the main paper, we develop and document the results of one methodology to account for control substitution in the first phase of ABC/CARE. The objective of each method is to estimate the average treatment effect of ABC/CARE, holding fixed take-up of alternative preschool by the control groups. We want to construct a scenario in which the subjects in the control group do not attend alternative preschool. This allows us to evaluate center-based childcare relative to a counterfactual scenario in which subjects stay at home.\\
...
\floatfoot{
\footnotesize
\noindent Note: This plot presents the parameter associated with $D$ from a regression of $Y$ on $D$, $V$, and $\mathbf{X}$, using $R$, $\mathbf{Z}(1 - R)$ as instruments. The outcomes ($Y$) are IQ scores at different ages, with a national standard deviation of 15 and a mean of 100. $\mathbf{X}$ includes a set of controls selected from all available baseline controls to maximize explanatory power across all outcomes tested in the paper: gender of the subject, mother's IQ score, High-risk Index, and Apgar Score at 1 minute. The confidence intervals are calculated at the 10\% significance level.}
\end{figure}
...
\floatfoot{
\footnotesize
\noindent Note: This plot presents the parameter associated to $D$ from a regression of $Y$ on $D$, $V$, and $\mathbf{X}$, using $R$, $\mathbf{Z}(1 - R$) as instruments. The outcomes ($Y$) are different relevant adult outcomes labeled in the horizontal axis. $\mathbf{X}$ includes a set of controls selected from all available baseline controls to maximize explanatory power across all outcomes tested in the paper: gender of the subject, mother's IQ score, High-risk Index, and Apgar Score at 1 minute. The confidence intervals are calculated at the 10\% significance level.}
\end{figure}
