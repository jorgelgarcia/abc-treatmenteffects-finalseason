...
\subsubsection{Gender Differences in Treatment Effects}\label{little-gdiff-tes}

\begin{enumerate}
\item Resample the experimental sample with replacement at the individual level. This gives us a new (re-sampled) panel dataset. Full information about each individual is obtained in each re-sample.

\item Calculate the treatment effect of each $Y_{j}$ separately for males and females, as described in Appendix~\ref{little-TE}. 

\item Repeat this procedure 1,000 times to obtain the empirical bootstrap distribution.

\item Calculate a Wilcoxon signed-rank test for each variable $Y_{j}$ comparing (1) the empirical bootstrap distribution of the male treatment effects and (2) the empirical bootstrap distribution of the female treatment effects. The null hypothesis of this test is that the difference of the two distributions is a symmetric distribution centered at 0. A $p$-value less than the chosen significance level indicates that the distributions of male and female treatment effects are significantly different.
\end{enumerate}

\subsubsection{Proportion of Outcomes for which Males Outperform Females}\label{little-gdiff-proportions}

\begin{enumerate}

\item Resample the experimental sample with replacement at the individual level. This gives us a new (re-sampled) panel dataset. Full information about each individual is obtained in each re-sample.

\item Calculate the mean of each variable $Y_{j}$ within a category separately for males and females.

\item Compare the means across genders and calculate the number of variables within an outcome for which the mean of the males is larger than the mean of the females.

\item Calculate the proportion by dividing by the number of outcomes for that category.

\item Repeat this procedure 1,000 times to obtain the empirical bootstrap distribution. Follow the above steps for the treatment and control groups separately. Generate the $p$-value testing if the proportions are equal to 50\%. 

\item We also compare (1) the empirical bootstrap distribution of the proportions in the treatment group and (2) the empirical bootstrap distribution of the proportions in the control group using the Wilcoxon signed-rank test. This tests if the proportions in the two groups follow the same distribution, as described in Appendix~\ref{little-gdiff-tes}.
