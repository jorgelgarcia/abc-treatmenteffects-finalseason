...
\noindent Sf to the NLSY, the CNLSY collected data on cognitive ability, motor and social development, home environment, health information, education, attitudes, employment, income, family decisions, and more. We use these data to estimate a prediction model for subject income for ages 22 through 29. \\
...
\noindent We base our predictions of a generic outcome, $Y_{t}$ on background (pre-treatment) variables, $\mathcal{B}$, and variables that could have been affected by treatment $X_{t}$. $X_{t}$ could contain lagged values (or predictions) of $Y_{t}$.\\

\noindent At each age, our prediction is a function of the prediction variables. Our prediction is based on identifying and estimating a relationship between the outcome we want to predict at time $t$, $Y_{t}$, in the non-experimental samples and then using this relationship in the experimental sample.\\

\noindent To be considered a prediction variable, the variable must satisfy three restrictions: (i) it has to be available in the experimental and non-experimental samples; (ii) it has to have predictive power on the predicted outcome; (iii) a containing support assumption between the experimental and non-experimental datasets, whereby, the support of the non-experimental samples contain the support of the experimental sample, as we formalize in  Section~\ref{section:methodology},  Assumption~\ref{ass:contain}.\\
...
\noindent The variables we use are: male and a black indicator ($\mathcal{B}$) in NLSY79, CNLSY, and PSID; mother's education in CNLSY ($\mathcal{B}$); PIAT scores at ages 5-7;\footnote{We take the average of the PIAT across ages 5 to 7. For ABC, we observe this variable. For CARE, we use the mathematics section of the Woodcock Johnson at these same ages because we do not observe PIAT.} in CNLSY ($X_{t}$); years of education at age 30 and labor income at ages 21 and 30 in in NLSY79, CNLSY, and PSID ($X_{t}$). All these variables are available in the experimental sample.\\

\noindent We use CNLSY to predict between ages 21 and 30 and pool CNLSY and PSID to predict from age 30 to retirement (age 65, by assumption). We provide tests for restrictions (i) and (ii) next and then discuss potential sensitivity to the choice of these variables.

\noindent When using the non-experimental samples, and with the objective of increasing the similarity on observed variables of the individuals in the experimental and non-experimental samples, weigh the individuals in the non-experimental samples to match them on observed characteristics to the individuals using a Mahalanobis distance based on male and black indicators, year of birth, years of education at age 30, and labor income at ages 21 and 30, and marital status at age 30. We choose these variables because they are available in all three non-experimental samples and we intend to locate common comparison samples.
...
\noindent We present evidence on the second restriction next. Tables~\ref{table:predpsid} to \ref{table:predcnlsy} show that, in each of the auxiliary samples that we use, the prediction variables $X, \mathcal{B}$ are strong predictors of labor income at age 30. We present this evidence at age 30 both for brevity and to compare the predictive power of $X,\mathcal{B}$ on the outcomes we consider in the ABC/CARE sample.
...
\noindent An immediate route of inquiry is the sensitivity of our predictions to the use of different prediction variables. We reproduce three exercises to analyze this for labor income: (i) our baseline prediction, which is based on $X, \mathcal{B}$ and the lagged value of the predicted labor income, i.e. at time $t$ we base the prediction on $X, \mathcal{B}$ and (predicted) labor income at $t-1$; (ii) the same as (i) but without using lagged labor income; (iii) the same as (i) but without using $X$; (iv) a prediction based exclusively on $\mathcal{B}$; (v) a prediction based exclusively on $X$.\\

\noindent Any prediction based on $X$ produces labor income profiles with significant differences between the treatment and control groups, especially for males. Adding lagged labor income increases the precision of the prediction, while background variables $\mathcal{B}$ do not help distinguish treatment and control group (as expected given balance at baseline). Lagged labor income by itself is not sufficient to predict a difference between treatment and control. $X$ by itself is sufficient to match the life-cycle profile of the control group to the life-cycle profile of the disadvantaged individuals in the PSID.\\

Although the prediction in (i) is the most accurate if compared to low-skilled individuals in the PSID, they are robust to sensitivity analysis.\footnote{We obtain similar results exercises for transfer income and parental labor income and they are available under request.}\\

\begin{sidewaysfigure}[!htbp]
\centering
\caption{Labor Income Profiles, Prediction Based on Lagged Labor Income and $X,\mathcal{B}$}\label{fig:labor-income-profiles}
...
\begin{sidewaysfigure}[!htbp]
\centering
\caption{Labor Income Profiles, Prediction Based on $X,\mathcal{B}$}\label{fig:labor-income-profiles}
\begin{subfigure}[h]{0.4\textwidth}
		\centering
		\caption{Predicted for ABC/CARE Males} \label{fig:abcare1}
...
\begin{sidewaysfigure}[!htbp]
\centering
\caption{Labor Income Profiles, Prediction Based on Lagged Labor Income and $\mathcal{B}$}\label{fig:labor-income-profiles}
\begin{subfigure}[h]{0.4\textwidth}
		\centering
		\caption{Predicted for ABC/CARE Males} \label{fig:abcare1}
...
\begin{sidewaysfigure}[!htbp]
\centering
\caption{Labor Income Profiles, Prediction Based on $\mathcal{B}$}\label{fig:labor-income-profiles}
\begin{subfigure}[h]{0.4\textwidth}
		\centering
		\caption{Predicted for ABC/CARE Males} \label{fig:abcare1}
