...
\subsubsection{Predictive Power and Testing Assumption~\ref{ass:eczema}} \label{appendix:pred}
...
\noindent This exercise also allows us to provide evidence in favor of Assumption~\ref{ass:eczema}. If $\bm{X}_{t}, \bm{B}$ are the inputs in the production of labor income at age 30, their marginal effects on this variable should be the same across the synthetic control and treatment groups, under Assumption~\ref{ass:eczema}. In each of the non-experimental samples we consider, the point estimates of the marginal effects of $\bm{X}_{t},\bm{B}$ on labor income at age 30 are very close. We would fail to reject any difference across these coefficients. We do not these tests both for brevity and because the point estimates across synthetic cohorts is evidently close.\\

\noindent Being able to perform a test of Assumption~\ref{ass:eczema} using larger samples is appealing because recent tests of these assumptions have been based on small samples \citep{Heckman_Pinto_etal_2013_PerryFactor,Attanasio-etal_2015_NBER_Estimating-Production}. As Table~\ref{table:predabc} evidences, testing differences between the treatment and control groups becomes challenging because even when differences in coefficients could be stark, standard errors are sizable.
