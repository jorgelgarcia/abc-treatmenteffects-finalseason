...
\noindent Marital status transitions and childbearing in FAM are affected by the subject's mother's education level.  The ABC and CARE age-30 subject interview did not ask about mother's education, but the age-21 parent interview did.
...
One male in the ABC control group was missing smoking status and, although known to be not working, was also missing specific employment status (unemployed or out of the labor force).
 We use a multinomial logit model to jointly estimate the probability of each combined smoking and employment category among 25- to 35-year-olds in the PSID who were not working.  At the beginning of each simulation repetition, we use a Monte Carlo random draw generated from this distribution to assign the ABC and CARE subjects' smoking and employment statuses. \\
...
The only BMI data in ABC and CARE come from height and weight measured during the age-34 health interview.
...
\noindent Having decided to impute self-reported BMI from measured BMI using a linear model at the age-34 interview, the next step is to estimate a model for self-reported BMI at the age-30 interview.  We estimate this model using a sample of PSID respondent observations at ages 33--40 (the ages observed in the ABC and CARE age-34 interview).  The model covariates included BMI and age at the time of observation along with demographic, health, and relationship status covariates observed at the respondent's first interview between ages 30 and 32 (the ages observed in the ABC and CARE  age-30 interview).  In order to be included in the estimation sample, a PSID respondent must be observed at least once in the 30-32 age range and at least once in 33--34 age range.  We estimate this model using OLS regression ($R^2 = 0.757$).
At the beginning of the simulation, each ABC and CARE subject's expected BMI was estimated from this model using covariates from the age-30 subject interview and their BMI and age in the age-34 health interview. A Monte Carlo Normal random draw was generated using the subject's expected BMI and the estimated variance from the model.  This Monte Carlo draw was then assigned to be the subject's initial BMI in the simulation.  Using BMI from the age-34 health interview limited the ABC and CARE subjects simulated in FAM to only those who had height and weight measured in the health interview. \\
...
In order to impute the insurance category for the remaining two cases, we use ABC and CARE data to estimate a probit model for whether or not a subject had insurance.
