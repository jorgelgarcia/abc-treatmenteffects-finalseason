\def\arraystretch{0.6}

\setlength\tabcolsep{0.3em}

\subsubsection{{Combining Functions, Aggregated}}


	\begin{table}[H]
	\begin{threeparttable}
     \caption{Combining Functions, Pooled Sample} 
     \label{table:abccare_rslt_pooled_counts}
	\input{AppResOutput/abccare_jul7/rslt_pooled_counts}
	\begin{tablenotes}
	\footnotesize
	 Note: The statistics presented in the first row of this table count the number of socially positive 
	treatment effects as a percentage of all treatment effects. The statistics are bolded if they are 
	significantly different from 50\% at the 10\% level. The statistics in the second row count the
	number of socially positive treatment effects that are significant at the 10\% level as a percentage
	of all treatment effects. Each statistic is bolded if it is significantly different from 10\% at the 10\% 
	level.
	\end{tablenotes}
	\end{threeparttable}
	\end{table}  

	\begin{table}[H]
	\begin{threeparttable}
     \caption{Combining Functions, Male Sample} 
     \label{table:abccare_rslt_male_counts}
	\input{AppResOutput/abccare_jul7/rslt_male_counts}
	\begin{tablenotes}
	\footnotesize
	\item Note: The statistics presented in the first row of this table count the number of socially positive
	treatment effects as a percentage of all treatment effects. The statistics are bolded if they are
	significantly different from 50\% at the 10\% level. The statistics in the second row count the
	number of socially positive treatment effects that are significant at the 10\% level as a percentage
	of all treatment effects. Each statistic is bolded if it is significantly different from 10\% at the 10\% 
	level.
	\end{tablenotes}
	\end{threeparttable}
	\end{table}  

	\begin{table}[H]
	\begin{threeparttable}
     \caption{Combining Functions, Female Sample} 
     \label{table:abccare_rslt_female_counts}
	\input{AppResOutput/abccare_jul7/rslt_female_counts}
	\begin{tablenotes}
	\footnotesize
	\item Note: The statistics presented in the first row of this table count the number of socially positive 
	treatment effects as a percentage of all treatment effects. The statistics are bolded if they are 
	significantly different from 50\% at the 10\% level. The statistics in the second row count th
	number of socially positive treatment effects that are significant at the 10\% level as a percentage
	of all treatment effects. Each statistic is bolded if it is significantly different from 10\% at the 10\% 
	level.
	\end{tablenotes}
	\end{threeparttable}
	\end{table}  
\clearpage

\subsubsection{{Combining Functions, by Category}}

\noindent The tables in this section contain an extra column. This column contains the number of variables for the outcome category used to construct the combining function statistic. \\

	\begin{table}[H]
	\begin{threeparttable}
     \caption{Combining Functions by Category, Pooled Sample} 
     \label{table:abccare_rslt_pooled_counts_n50a100}
	\input{AppResOutput/abccare_jul7/rslt_pooled_counts_n50a100}
	\begin{tablenotes}
	\footnotesize
	\item Note: The statistics presented in this table count the number of socially positive treatment 
	 effects as a percentage of all treatment effects. Each statistic is bolded if it is significantly
	 different from 50\% at the 10\% level.
	\end{tablenotes}
	\end{threeparttable}
	\end{table}   

	\begin{table}[H]
	\begin{threeparttable}
     \caption{Combining Functions by Category $|$ 10\% Significance, Pooled Sample} 
     \label{table:abccare_rslt_pooled_counts_n10a10}
	\input{AppResOutput/abccare_jul7/rslt_pooled_counts_n10a10}
	\begin{tablenotes}
	\footnotesize
	\item Note: The statistics presented in this table count the number of socially positive treatment 
	effects that are significant at the 10\% level as a percentage of all treatment effects. Each statistic
	is bolded if it is significantly different from 10\% at the 10\% level.
	\end{tablenotes}
	\end{threeparttable}
	\end{table}   

	\begin{table}[H]
	\begin{threeparttable}
     \caption{Combining Functions by Category, Male Sample} 
     \label{table:abccare_rslt_male_counts_n50a100}
	\input{AppResOutput/abccare_jul7/rslt_male_counts_n50a100}
	\begin{tablenotes}
	\footnotesize
	\item Note: The statistics presented in this table count the number of socially positive treatment 
	 effects as a percentage of all treatment effects. Each statistic is bolded if it is significantly
	 different from 50\% at the 10\% level.
	\end{tablenotes}	
	\end{threeparttable}
	\end{table}   

	\begin{table}[H]
	\begin{threeparttable}
     \caption{Combining Functions by Category $|$ 10\% Significance, Male Sample} 
     \label{table:abccare_rslt_male_counts_n10a10}
	\input{AppResOutput/abccare_jul7/rslt_male_counts_n10a10}
	\begin{tablenotes}
	\footnotesize
	\item Note: The statistics presented in this table count the number of socially positive treatment 
	effects that are significant at the 10\% level as a percentage of all treatment effects. Each statistic
	is bolded if it is significantly different from 10\% at the 10\% level.
	\end{tablenotes}
	\end{threeparttable}
	\end{table}   

	\begin{table}[H]
	\begin{threeparttable}
     \caption{Combining Functions by Category, Female Sample} 
     \label{table:abccare_rslt_female_counts_n50a100}
	\input{AppResOutput/abccare_jul7/rslt_female_counts_n50a100}
	\begin{tablenotes}
	\footnotesize
	\item Note: The statistics presented in this table count the number of socially positive treatment 
	 effects as a percentage of all treatment effects. Each statistic is bolded if it is significantly
	 different from 50\% at the 10\% level.
	\end{tablenotes}	
	\end{threeparttable}
	\end{table}   

	\begin{table}[H]
	\begin{threeparttable}
     \caption{Combining Functions by Category $|$ 10\% Significance, Female Sample} 
     \label{table:abccare_rslt_female_counts_n10a10}
	\input{AppResOutput/abccare_jul7/rslt_female_counts_n10a10}
	\begin{tablenotes}
	\footnotesize
	\item Note: The statistics presented in this table count the number of socially positive treatment 
	effects that are significant at the 10\% level as a percentage of all treatment effects. Each statistic
	is bolded if it is significantly different from 10\% at the 10\% level.
	\end{tablenotes}
	\end{threeparttable}
	\end{table}   
\clearpage

\subsubsection{Treatment Effects for Pooled Sample}


	\begin{table}[H]
     \caption{Treatment Effects on IQ Scores, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat0}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat0}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Achievement Scores, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat1}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat1}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on HOME Scores, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat2}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat2}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Parent Income, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat3}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat3}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Mother's Employment, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat4}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat4}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Father at Home, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat5}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat5}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Education, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat6}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat6}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Employment and Income, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat7}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat7}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Crime, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat8}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat8}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Tobacco, Drugs, Alcohol, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat9}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat9}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Self-Reported Health, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat10}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat10}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Hypertension, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat11}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat11}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Cholesterol, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat12}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat12}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Diabetes, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat13}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat13}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Vitamin D Deficiency, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat14}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat14}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Obesity, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat15}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat15}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Mental Health, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat16}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat16}
	\end{table} 

\subsubsection{Treatment Effects for Male Sample}


	\begin{table}[H]
     \caption{Treatment Effects on IQ Scores, Male Sample}
     \label{table:abccare_rslt_male_cat0}
	\input{AppResOutput/abccare_jul7/rslt_male_cat0}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Achievement Scores, Male Sample}
     \label{table:abccare_rslt_male_cat1}
	\input{AppResOutput/abccare_jul7/rslt_male_cat1}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on HOME Scores, Male Sample}
     \label{table:abccare_rslt_male_cat2}
	\input{AppResOutput/abccare_jul7/rslt_male_cat2}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Parent Income, Male Sample}
     \label{table:abccare_rslt_male_cat3}
	\input{AppResOutput/abccare_jul7/rslt_male_cat3}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Mother's Employment, Male Sample}
     \label{table:abccare_rslt_male_cat4}
	\input{AppResOutput/abccare_jul7/rslt_male_cat4}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Father at Home, Male Sample}
     \label{table:abccare_rslt_male_cat5}
	\input{AppResOutput/abccare_jul7/rslt_male_cat5}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Education, Male Sample}
     \label{table:abccare_rslt_male_cat6}
	\input{AppResOutput/abccare_jul7/rslt_male_cat6}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Employment and Income, Male Sample}
     \label{table:abccare_rslt_male_cat7}
	\input{AppResOutput/abccare_jul7/rslt_male_cat7}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Crime, Male Sample}
     \label{table:abccare_rslt_male_cat8}
	\input{AppResOutput/abccare_jul7/rslt_male_cat8}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Tobacco, Drugs, Alcohol, Male Sample}
     \label{table:abccare_rslt_male_cat9}
	\input{AppResOutput/abccare_jul7/rslt_male_cat9}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Self-Reported Health, Male Sample}
     \label{table:abccare_rslt_male_cat10}
	\input{AppResOutput/abccare_jul7/rslt_male_cat10}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Hypertension, Male Sample}
     \label{table:abccare_rslt_male_cat11}
	\input{AppResOutput/abccare_jul7/rslt_male_cat11}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Cholesterol, Male Sample}
     \label{table:abccare_rslt_male_cat12}
	\input{AppResOutput/abccare_jul7/rslt_male_cat12}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Diabetes, Male Sample}
     \label{table:abccare_rslt_male_cat13}
	\input{AppResOutput/abccare_jul7/rslt_male_cat13}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Vitamin D Deficiency, Male Sample}
     \label{table:abccare_rslt_male_cat14}
	\input{AppResOutput/abccare_jul7/rslt_male_cat14}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Obesity, Male Sample}
     \label{table:abccare_rslt_male_cat15}
	\input{AppResOutput/abccare_jul7/rslt_male_cat15}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Mental Health, Male Sample}
     \label{table:abccare_rslt_male_cat16}
	\input{AppResOutput/abccare_jul7/rslt_male_cat16}
	\end{table} 
\subsubsection{Treatment Effects for Female Sample}


	\begin{table}[H]
     \caption{Treatment Effects on IQ Scores, Female Sample}
     \label{table:abccare_rslt_female_cat0}
	\input{AppResOutput/abccare_jul7/rslt_female_cat0}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Achievement Scores, Female Sample}
     \label{table:abccare_rslt_female_cat1}
	\input{AppResOutput/abccare_jul7/rslt_female_cat1}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on HOME Scores, Female Sample}
     \label{table:abccare_rslt_female_cat2}
	\input{AppResOutput/abccare_jul7/rslt_female_cat2}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Parent Income, Female Sample}
     \label{table:abccare_rslt_female_cat3}
	\input{AppResOutput/abccare_jul7/rslt_female_cat3}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Mother's Employment, Female Sample}
     \label{table:abccare_rslt_female_cat4}
	\input{AppResOutput/abccare_jul7/rslt_female_cat4}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Father at Home, Female Sample}
     \label{table:abccare_rslt_female_cat5}
	\input{AppResOutput/abccare_jul7/rslt_female_cat5}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Education, Female Sample}
     \label{table:abccare_rslt_female_cat6}
	\input{AppResOutput/abccare_jul7/rslt_female_cat6}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Employment and Income, Female Sample}
     \label{table:abccare_rslt_female_cat7}
	\input{AppResOutput/abccare_jul7/rslt_female_cat7}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Crime, Female Sample}
     \label{table:abccare_rslt_female_cat8}
	\input{AppResOutput/abccare_jul7/rslt_female_cat8}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Tobacco, Drugs, Alcohol, Female Sample}
     \label{table:abccare_rslt_female_cat9}
	\input{AppResOutput/abccare_jul7/rslt_female_cat9}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Self-Reported Health, Female Sample}
     \label{table:abccare_rslt_female_cat10}
	\input{AppResOutput/abccare_jul7/rslt_female_cat10}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Hypertension, Female Sample}
     \label{table:abccare_rslt_female_cat11}
	\input{AppResOutput/abccare_jul7/rslt_female_cat11}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Cholesterol, Female Sample}
     \label{table:abccare_rslt_female_cat12}
	\input{AppResOutput/abccare_jul7/rslt_female_cat12}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Diabetes, Female Sample}
     \label{table:abccare_rslt_female_cat13}
	\input{AppResOutput/abccare_jul7/rslt_female_cat13}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Vitamin D Deficiency, Female Sample}
     \label{table:abccare_rslt_female_cat14}
	\input{AppResOutput/abccare_jul7/rslt_female_cat14}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Obesity, Female Sample}
     \label{table:abccare_rslt_female_cat15}
	\input{AppResOutput/abccare_jul7/rslt_female_cat15}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Mental Health, Female Sample}
     \label{table:abccare_rslt_female_cat16}
	\input{AppResOutput/abccare_jul7/rslt_female_cat16}
	\end{table} 
\clearpage

\subsubsection{Treatment Effects for Pooled Sample, Step Down}

\noindent The tables in this section present $p$-values calculated using the step-down method\footnote{\citet{Lehmann_Romano_2005_testing,Romano_Shaikh_2006_AnnStat}. See \citet{Heckman_Moon_etal_2010_QE} for an example of an application.}

	\begin{table}[H]
     \caption{Treatment Effects on IQ Scores, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat0_sd}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat0_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Achievement Scores, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat1_sd}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat1_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on HOME Scores, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat2_sd}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat2_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Parent Income, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat3_sd}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat3_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Mother's Employment, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat4_sd}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat4_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Father at Home, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat5_sd}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat5_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Education, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat6_sd}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat6_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Employment and Income, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat7_sd}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat7_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Crime, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat8_sd}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat8_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Tobacco, Drugs, Alcohol, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat9_sd}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat9_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Self-Reported Health, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat10_sd}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat10_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Hypertension, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat11_sd}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat11_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Cholesterol, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat12_sd}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat12_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Diabetes, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat13_sd}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat13_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Vitamin D Deficiency, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat14_sd}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat14_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Obesity, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat15_sd}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat15_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Mental Health, Pooled Sample}
     \label{table:abccare_rslt_pooled_cat16_sd}
	\input{AppResOutput/abccare_jul7/rslt_pooled_cat16_sd}
	\end{table} 

\subsubsection{Treatment Effects for Male Sample, Step Down}


	\begin{table}[H]
     \caption{Treatment Effects on IQ Scores, Male Sample}
     \label{table:abccare_rslt_male_cat0_sd}
	\input{AppResOutput/abccare_jul7/rslt_male_cat0_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Achievement Scores, Male Sample}
     \label{table:abccare_rslt_male_cat1_sd}
	\input{AppResOutput/abccare_jul7/rslt_male_cat1_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on HOME Scores, Male Sample}
     \label{table:abccare_rslt_male_cat2_sd}
	\input{AppResOutput/abccare_jul7/rslt_male_cat2_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Parent Income, Male Sample}
     \label{table:abccare_rslt_male_cat3_sd}
	\input{AppResOutput/abccare_jul7/rslt_male_cat3_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Mother's Employment, Male Sample}
     \label{table:abccare_rslt_male_cat4_sd}
	\input{AppResOutput/abccare_jul7/rslt_male_cat4_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Father at Home, Male Sample}
     \label{table:abccare_rslt_male_cat5_sd}
	\input{AppResOutput/abccare_jul7/rslt_male_cat5_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Education, Male Sample}
     \label{table:abccare_rslt_male_cat6_sd}
	\input{AppResOutput/abccare_jul7/rslt_male_cat6_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Employment and Income, Male Sample}
     \label{table:abccare_rslt_male_cat7_sd}
	\input{AppResOutput/abccare_jul7/rslt_male_cat7_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Crime, Male Sample}
     \label{table:abccare_rslt_male_cat8_sd}
	\input{AppResOutput/abccare_jul7/rslt_male_cat8_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Tobacco, Drugs, Alcohol, Male Sample}
     \label{table:abccare_rslt_male_cat9_sd}
	\input{AppResOutput/abccare_jul7/rslt_male_cat9_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Self-Reported Health, Male Sample}
     \label{table:abccare_rslt_male_cat10_sd}
	\input{AppResOutput/abccare_jul7/rslt_male_cat10_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Hypertension, Male Sample}
     \label{table:abccare_rslt_male_cat11_sd}
	\input{AppResOutput/abccare_jul7/rslt_male_cat11_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Cholesterol, Male Sample}
     \label{table:abccare_rslt_male_cat12_sd}
	\input{AppResOutput/abccare_jul7/rslt_male_cat12_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Diabetes, Male Sample}
     \label{table:abccare_rslt_male_cat13_sd}
	\input{AppResOutput/abccare_jul7/rslt_male_cat13_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Vitamin D Deficiency, Male Sample}
     \label{table:abccare_rslt_male_cat14_sd}
	\input{AppResOutput/abccare_jul7/rslt_male_cat14_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Obesity, Male Sample}
     \label{table:abccare_rslt_male_cat15_sd}
	\input{AppResOutput/abccare_jul7/rslt_male_cat15_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Mental Health, Male Sample}
     \label{table:abccare_rslt_male_cat16_sd}
	\input{AppResOutput/abccare_jul7/rslt_male_cat16_sd}
	\end{table} 

\subsubsection{Treatment Effects for Female Sample, Step Down}


	\begin{table}[H]
     \caption{Treatment Effects on IQ Scores, Female Sample}
     \label{table:abccare_rslt_female_cat0_sd}
	\input{AppResOutput/abccare_jul7/rslt_female_cat0_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Achievement Scores, Female Sample}
     \label{table:abccare_rslt_female_cat1_sd}
	\input{AppResOutput/abccare_jul7/rslt_female_cat1_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on HOME Scores, Female Sample}
     \label{table:abccare_rslt_female_cat2_sd}
	\input{AppResOutput/abccare_jul7/rslt_female_cat2_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Parent Income, Female Sample}
     \label{table:abccare_rslt_female_cat3_sd}
	\input{AppResOutput/abccare_jul7/rslt_female_cat3_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Mother's Employment, Female Sample}
     \label{table:abccare_rslt_female_cat4_sd}
	\input{AppResOutput/abccare_jul7/rslt_female_cat4_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Father at Home, Female Sample}
     \label{table:abccare_rslt_female_cat5_sd}
	\input{AppResOutput/abccare_jul7/rslt_female_cat5_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Education, Female Sample}
     \label{table:abccare_rslt_female_cat6_sd}
	\input{AppResOutput/abccare_jul7/rslt_female_cat6_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Employment and Income, Female Sample}
     \label{table:abccare_rslt_female_cat7_sd}
	\input{AppResOutput/abccare_jul7/rslt_female_cat7_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Crime, Female Sample}
     \label{table:abccare_rslt_female_cat8_sd}
	\input{AppResOutput/abccare_jul7/rslt_female_cat8_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Tobacco, Drugs, Alcohol, Female Sample}
     \label{table:abccare_rslt_female_cat9_sd}
	\input{AppResOutput/abccare_jul7/rslt_female_cat9_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Self-Reported Health, Female Sample}
     \label{table:abccare_rslt_female_cat10_sd}
	\input{AppResOutput/abccare_jul7/rslt_female_cat10_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Hypertension, Female Sample}
     \label{table:abccare_rslt_female_cat11_sd}
	\input{AppResOutput/abccare_jul7/rslt_female_cat11_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Cholesterol, Female Sample}
     \label{table:abccare_rslt_female_cat12_sd}
	\input{AppResOutput/abccare_jul7/rslt_female_cat12_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Diabetes, Female Sample}
     \label{table:abccare_rslt_female_cat13_sd}
	\input{AppResOutput/abccare_jul7/rslt_female_cat13_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Vitamin D Deficiency, Female Sample}
     \label{table:abccare_rslt_female_cat14_sd}
	\input{AppResOutput/abccare_jul7/rslt_female_cat14_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Obesity, Female Sample}
     \label{table:abccare_rslt_female_cat15_sd}
	\input{AppResOutput/abccare_jul7/rslt_female_cat15_sd}
	\end{table} 

	\begin{table}[H]
     \caption{Treatment Effects on Mental Health, Female Sample}
     \label{table:abccare_rslt_female_cat16_sd}
	\input{AppResOutput/abccare_jul7/rslt_female_cat16_sd}
	\end{table} 
\clearpage