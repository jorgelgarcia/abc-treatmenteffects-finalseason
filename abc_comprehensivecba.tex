...
\floatfoot{
\footnotesize
\textbf{[JJH: Significance at what percentage? Get from Jorge] [JLG: Professor, let me know if this plot works. Otherwise we can simply state what is the $p$-value of pooled labor income and signal it with another symbol. The $p$-value is .21.]}\\
...
Let $\bm{Y}^1_t$ be the outcome vector at age $t$ for the treated. $\bm{Y}^0_t$ is the outcome vector for the controls. In principle, life-cycle treatment effects for the controls can depend on the exposures to treatment or to the various alternatives at each age. It would be desirable to estimate treatment effects for each possible exposure but our samples are too small to make credible estimates for very detailed exposures.
...
This subsection gives an informal summary of our approach. The rest of Section~\ref{section:cbamethodology} presents a formal justification as well as tests of key assumptions. Readers not interested in technical details may wish to skip to Section~\ref{section:cbaresults}, where we discuss our empirical results.

We have data on control- and treatment-group individuals through age $t^{\ast}$. We lack information on participant outcomes afterward. Yet, post-$t^{\ast}$ outcomes are required to construct counterfactual life-cycle profiles. We predict these outcomes using a three-step procedure. In step 1, we construct counterparts to treatment- and control-group members in the non-experimental samples. In step 2, we fit the dynamic relationships between the vector of outcomes at time $t$ ($\bm{Y}^{d}_{i,t}$) and predictor variables (and interactions) that are affected by treatment ($\bm{X}^{d}_{i,t}$), as well as background variables ($\bm{B}$). In step 3, we use the dynamic relationships so estimated to predict the post-$t^{\ast}$ outcomes.

Figure~\ref{fig:labor-income-profiles} displays the life-cycle labor income profiles \textit{predicted} for the treatment and control groups. It also compares the \textit{realized} labor income to the \textit{model-predicted} labor income at $t^*$. This pattern of life-cycle labor income is typical for low-skilled workers \citep{Blundell-etal_2015_J-Pub-E,Gladden_Taber_2000_WageProgression,Sanders-Taber_2012_AR,Lagakos_Moll_etal_2016_LifeCycle_NBER}.\footnote{The prediction of labor (transfer) income is based on indicator variables of being male and being black, mother's education, average PIAT scores from ages 5 to 7, years of education at age 30, body-mass index at 34, labor (transfer) income at ages 21 and 30, and one-period lagged labor (transfer) income. As we explain in the next section, our prediction is based on a first-order Markov process. We use labor (transfer) income at age 21 to initialize this process. The age-30 observation helps us validate our procedure. A justification for the use of these variables, evidence on their predictive power, and a sensitivity analysis to different prediction variables are in Appendix~\ref{appendix:methodology}. In that appendix and in Section 6.3, we examine our predictions for parental income.}
...
\begin{assumption} \label{ass:summary} \textbf{$\bm{X}_{t}$ Generates Treatment Effects}\\
...
\end{align}
\noindent for $t \in \{ 1, \ldots, T \}$ and for any outcome $j \in \mathcal{J}_{t}$.
\end{assumption}
...
For $t \in \{ 1, \ldots, T \}$, the support of $\left( \bm{Y}_{t}, \bm{X}_{t}, \bm{B} \right)$ in the experimental sample is contained in the support of $\left( \bm{Y}_{t}, \bm{X}_{t}, \bm{B} \right)$ in the non-experimental sample:
\begin{equation}
sup( \bm{Y}_{t}, \bm{X}_{t}, \bm{B} )^{s=1} \subseteq sup( \bm{Y}_{t}, \bm{X}_{t}, \bm{B} )^{s=0}
\end{equation}
where superscripts denote membership in the experimental ($s=1$) and control ($s=0$) samples.
\end{assumption}
\doublespacing
...
We provide evidence favoring Assumptions~\ref{ass:summary}, \ref{ass:invariance}, and \ref{ass:contain} in Appendix~\ref{appendix:methodology}. Exogeneity Assumptions~\ref{ass:exog} and \ref{ass:exogne} could easily be violated, especially when we include lagged dependent variables among the predictors. We motivate this concern and provide tests for these two assumptions.
...
To investigate the endogeneity issue in the experimental and comparison samples in Appendix~\ref{app:endogeneity}, we follow \citet{Heckman_Pinto_etal_2013_PerryFactor} and assume that $\varepsilon_{j,t}^d$ obeys a factor structure.
