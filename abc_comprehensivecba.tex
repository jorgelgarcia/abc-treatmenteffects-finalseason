...
Figure~\ref{figure:main} summarizes the main findings of this paper. It displays the discounted (by a 3\% discount rate) life-cycle program (net) benefits and costs (in 2014 USD) for a pooled sample of male and female participants along with the monetized benefits overall and disaggregated by category. Costs are substantial, as has frequently been noted by critics.\footnote{See, e.g., \citet{Whitehurst_2014_Senate_Testimony} and \citet{Fox_News_2014_Head_Start_Effects}.}$^,$\footnote{These results indicate that the programs we evaluate are an example of a developmental approach to improve health conditions \citet{Conti-Heckman_2010_Socio-HO}.} But so are the benefits, which far outweigh the costs. The benefits accrue accross multiple life domains.

\noindent \textbf{[JLG: All CBA results are up to date. Note that labor income became not significant when pooling samples, but significant when considering females and males separately. The non-significance is marginal. All ratios for females are now significant.] [JJH: Okay, typists--update the Sage HO too.] [Typist: This has been done.]}
...
The goals of these programs were to enhance the early-life skills of a disadvantaged population. The program (i) supported language, motor, and cognitive development and (ii) developed socio-emotional competencies considered crucial for school success including task-orientation, competence in communications, independence, and pro-social behavior.\footnote{\citet{Ramey_Collier_etal_1976_CarolinaAbecedarianProject, Ramey_etal_1985_Project-CARE_TiECSE, Sparling_1974_Synth_Edu_Infant_SPEECH, Wasik_Ramey_etal_1990_CD, Ramey-etal_2012-ABC}.}
...
\end{equation}
where $\eta^d_{k,t}$ is an unobserved determinant of $X^d_{k,t}$. We specify the dependence among the $\varepsilon^d_{j,t}$ and the $\eta^d_{k,t}$ below. The $X^d_{k,t}$ can be caused by treatment, as can $\bm{X}^d_{k,t-1}$. Examples of $\bm{X}^d_t$ in our data are education, IQ, health, work experience, and participation in crime. All are potential determinants of earnings.
...
\end{equation*}
\end{assumption}
i.e., treatment effects operate through inputs and not shifts in the input-output functions. This allows us to use post-$t^*$ relationships fit on the auxiliary samples to forecast post-$t^*$ outcomes of treatment group members. Evidence for the validity of this assumption in the context of early childhood programs is presented in \citet{Heckman_Pinto_etal_2013_PerryFactor} and \citet{Attanasio-etal_2015_NBER_Estimating-Production} \textbf{[JJH: 2016 NBER.] [Typist: Professor, this Attanasio cite is NBER 2015. We have checked with Indra and Jorge. If there is a different cite you are thinking about, please clarify.] [JLG: Professor, I think the authors haven't submitted the most up to date version of the paper to NBER.]}. In Appendix~\ref{app:invariance}, we test and do not reject this assumption using the factor structure control function approach developed in \citet{Heckman_Pinto_etal_2013_PerryFactor}.
...
\begin{theorem}
Putting Assumptions~\ref{ass:butts}--\ref{ass:corns} together, under full observability of $\bm{X}^d_t$, estimation on the matched samples based on \eqref{eq:ingrownnail} generates unbiased out of sample treatment effects. $\Box$
\end{theorem}

Our prediction is based on matching individuals in the non-experimental sample to individuals experimental sample \textit{on observed characteristics}. In general, the dependence between $\bm{X}_{t}$ and $\bm{\varepsilon}_{t}$ is not the same across these two types of samples, and could generate a violation of exogeneity (Assumption~\ref{ass:corns}). This violation can arise in two ways: (i) the experiment does not necessarily identify the production function in \eqref{eq:trenchfoot} because there could be unmeasured inputs (even generates by the experiment); (ii) even if we could measure all inputs, the inputs are not necessarily exogenous in the non-experimental sample. A simple example is the following: in the experimental sample, the program shifts of education. We match based on education to create a treatment synthetic cohort. But we could be doing so by matching to individuals in the non-experimental sample who are highly educates because they come from a better backgrounds, be more motivated, etc. This would bias our prediction from the matched samples.

We tackle this issue in Appendix~\ref{app:endogeneity} using a factor-structure-based control function approach developed and applied in \citet{Heckman_Pinto_etal_2013_PerryFactor}. Using measures of latent skills, we adopt a factor structure for $\bm{\varepsilon}^d_t, d \in \left\{0,1\right\}, t \in \left\{0,...,T\right\}$. We assume that the extracted factors are the source of violations of Assumption~\eqref{ass:corns}. We test if our predictions differ when we account for the factors, aside from the rest of the variable we use to predict. Our predictions do not display major changes and we cannot reject the null hypotheses that these factors do not have a significant effect on our prediction. We use this same structure to test Assumption~\ref{ass:eczema} in Appendix~\ref{app:invariance}. We fail to reject the null hypotheses implied by this assumption.

\noindent \textbf{[JLG: These two appendices are now in separate sections in Appendix C. I will work with the typists so we can integrate it to the HO.]}
\noindent \textbf{[JLG: I have sent the Mincer-type predictions to Appendix C. But note that the rates making the pepetuity = NPV is very close to 2\% for both experimental groups and both gender. This is in the HO.]}

\subsection{Health} \label{section:health}

 We predict and monetize health outcomes based on a version of Equation~\eqref{eq:trenchfoot} with a full vector of lagged dependent variables. This requires an adaptation of the method just explained. We consider three additional issues: (i) health outcomes such as diabetes or heart disease are absorbing states; (ii) health outcomes are highly interdependent within and across time periods; and (iii) there is no obvious terminal time period for benefits and costs except death, which is endogenous.\footnote{For example, for income we extrapolate up to the retirement age of 67. However, for health, we need to predict an age of death for each individual.} Our auxiliary model for health is an adaptation of the Future America Model (FAM). This model predicts health outcomes from the subjects' mid-30s up to their projected death \citep{Goldman_etal_2015_Future-Elderly-Model-Report}.\footnote{The simulation starts at the age in which we observe the subject's age-34 health follow-up. On average this happened at age 34 for both males and females, but there is variation ranging from age 30 to age 37.} Appendix~\ref{appendix:health} discusses the FAM methodology in detail. We initialize the health prediction using the same variables that we use when predicting labor and transfer income, along with the initial health conditions as listed in Table~\ref{table:transition}.
...
This section reports cost/benefit and rate of return analyses underlying Figure~\ref{figure:main}. The results we discuss are the central focus of this paper.
...
To analyze how our estimates compare to those based on the method in \citet{Kline_Walters_2016_QJE}, we display a series of exercises in the third column of Table~\ref{table:comparing}. For purposes of comparison, the fourth column of Table~\ref{table:comparing} displays the analogous exercise using our own method.
...
Our methodology: provides a more precise estimate of the net-present value (and the return to IQ) of the components, as our interpolation and extrapolation are based on matching our experimental sample to various non-experimental samples. We better quantify the effects of the analyzed experiment by considering the whole life-cycle; we also better approximate the statistical uncertainty of our estimates by considering both the sampling error in the experimental and non-experimental samples and the prediction error due to the interpolation and extrapolation. Proceeding in this fashion enables us to an extensive sensitivity analysis of each of the components we monetize. \textbf{[JJH: Where is the sensitivity analysis for (a) cohort effects in earnings and health and (b) crime judgements.] [JLG: The sensitivity to cohort effects in earnings and health, I'm working on it. For the sensitivity analysis of different costs assumption for crime, see Appendices F5, F6. See also footnote 69].}
