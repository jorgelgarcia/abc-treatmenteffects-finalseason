...
\item Matching
    ...
    \item Do we need structural invariance? (Not necessarily.) We can assume that there is a break in the \textbf{[Typist: Sentence ends]}. But need invariance as to levels
...
We use two approaches to construct synthetic treatment and control cohorts.\footnote{See, e.g., \citet{Mincer_1974_schooling} and the survey in \citet{Heckman_Lochner_ea_2006_HEE}.} One approach uses matching methods to create counterparts for treatment and control group members in the auxiliary samples. The validity of this approach rests critically on the assumption of exogeneity of the matching variables. A second approach relies on structural invariance of the underlying economic model. It can be applied even if exogeneity fails, provided that there is access to instruments. Outcomes of ``comparable'' older cohorts in auxiliary samples who are not exposed to the program are used to construct virtual treatment and control groups effectively in both approaches. We thus use two different methods to address the problem of finding who among the auxiliary samples would have received treatment if the program had been available to them. Under exogeneity, the two approaches agree.

In addition, older cohorts may have received different endowments and experienced different environments. An assumption of stationary life-cycle profiles across cohorts may be grossly at odds with the data.
...
We now present our analytical framework and its underlying assumptions for the structural Stage II forecasting analysis. Our analysis is based on a causal model for treatment ($d=1$) and control ($d=0$) outcomes for measure $j$ at age $a$ in sample $k \in \{e,n\}$ where $e$ denotes membership in the experimental sample and $n$ denotes membership in the auxiliary sample:
...
Under exogeneity assumption~\ref{ass:exog} and invariance condition~\ref{ass:summary} we can use matching to construct counterparts to treatment and control groups in the auxiliary sample.\footnote{\citet{Heckman_Ichimura_etal_1998_Econometrica} use this procedure.} Doing so compresses the two stages of constructing a comparison group and creating forecasts into one stage. We discuss this approach in Appendix~\ref{appendix:match}. Matching is one way to non-parametrically estimate the conditional mean functions. There is close agreement between non-parametric estimates based on matching and more parametric model-based approaches (see Appendix~\ref{appendix:nonpar}).

\subsubsection{Summarizing Our Approach and What We Do in This Paper}

Using the data sources listed in Table~\ref{table:sources}, we execute our analysis in two stages. In Stage I, we construct comparison samples of group members using different criteria: (a) barebones eligibility criteria ($\bm{B}_n \in \mathcal{B}$); (b) using $\bm{X}^d_k$ (variables affected by treatment); (c) using both (a) and (b). In constructing comparison groups using $\bm{X}^d_k$, we are limited to variables common to both experimental and comparison group samples. In Stage II, we use the samples so constructed to create post-$a^\ast$ forecasting models. The non-parametric approach described in Section~\ref{usingmatching} combines the two stages. The estimates reported in the text of this paper are based on the structural approach outlined here. We report estimates from the non-parametric approach for selected outcomes in Appendix~\ref{appendix:nonpar}. In Appendix~\ref{appendix:match}, we report sensitivity analyses for our structural estimates based on different choices of $\bm{B}$ and $\bm{X}^d_k$. We find that, in our samples, our estimates are robust to alternative choices of matching criteria in constructing comparison samples in Stage I.
