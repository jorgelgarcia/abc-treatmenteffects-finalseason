...
A central goal of this paper is to summarize the multiple benefits of ABC/CARE using benefit/cost and rate of return analyses. Doing so required us to use data outside the ABC/CARE experiment to predict the costs and benefits of the program over the life cycle after the measurement phase of the study ends.

Approaches to this problem in previous literature use a synthetic cohort approach.\footnote{See, e.g., \citet{Mincer_1974_schooling} and the summary in \citet{Heckman_Lochner_ea_2006_HEE}.} Outcomes of ``comparable'' older cohorts in auxiliary samples who are not exposed to the program are used to find treatment and control group members.\footnote{See, e.g., \cite{Heckman_Lochner_ea_2006_HEE}. \citet{Heckman_Moon_etal_2010_RateofReturn} use a variety of methods to predict future labor income in the Perry Preschool Project. They fit a variety of panel data labor income dynamics models and project future labor income with them. The various methods are in general agreement.} A major problem is finding counterparts to the treatment group in the auxiliary samples. Who among those samples would have received treatment? In addition, older cohorts may have received different endowments and experienced different environments. An assumption of stationarity in the profiles we predict may be grossly at odds with the data.
...
Comparisons between the experimental control group and the synthetic control group are particularly compelling. By design, neither group receives treatment. All persons offered treatment accept it, so it is straightforward to construct synthetic control groups in auxiliary samples using only eligibility criteria.
...
Y_{k,j,a}^d = \phi_{k,j,a}^d \left( \bm{X}_{k,a}^d, \bm{B}_k \right) + \varepsilon_{k,j,a}^d, \quad j \in \mathcal{J}_a, \quad d \in \{0,1\}, \: \text{ and } \: k \in \{e,n\},
\end{equation}
where $d$ indicates that a person is fixed to control ($d = 0$) or treatment ($d = 1$) status and $k$ indicates whether the outcome is observed in the experimental sample ($k = e$) or in the auxiliary sample ($k = n$).\footnote{Fixing and conditioning are fundamentally different concepts. See \citet{Heckman_Pinto_2015_EconometTheory} for a discussion. Our analysis applies to the methodology in that paper.} The relationships between the dependent and independent variables in \eqref{eq:outcome} do not necessarily coincide across the samples. We interpret  $\phi_{k,j,a}^d \left( \cdot \right)$ as a production function mapping the inputs $\bm{X}_{k,a}^d$ together with background variables $\bm{B}_k$ to the output $Y_{k,j,a}^d$. Among the $\bm{X}^d_{k,a}$ are variables caused by treatment, including lagged dependent variables.

Let $\bm{Y}_k^d$ denote the vector of all outcomes at all ages for $k \in \{e, n \}$, when treatment status is fixed to $d$. Similarly, $\bm{X}_k^d$ is the vector of all causal predictors of $\bm{Y}_k^d$ at all ages. The background variables may have different distributions in the two samples. Both $\bm{Y}_k^d$ and $\bm{X}_k^d$ include the full set of outcomes over the life cycle, even though they are not sampled after age $a^*$. We denote the joint distribution of these vectors conditional on $\bm{B}_k = \bm{b}$ by $F_{\bm{Y}_k^d, \bm{X}_k^d} | \bm{B}_k = \bm{b}$.

The auxiliary data come from older cohorts for which we observe fuller segments of their life cycles, i.e., we observe $a \in \{1, \ldots, A\}$. In the auxiliary data, however, we do not observe what treatment status $d$ would have been. Even if we did, we do not know if cohort ($c$) or time ($t$) effects make the experiences of the auxiliary-sample individuals different from the experiences of the individuals in the experimental sample.

To formalize this problem, let $Y_{j,k,a,c,t}^d$ be outcome $j$ for sample $k$ at age $a$ for cohort $c$ at time $t$ when treatment is fixed to $d$. We make the following assumption, and henceforth drop the ``$c$'' and ``$t$'' sub-indices.
...
\begin{equation}
Y_{e,a,c_{e},t_{e}}^d = Y_{n,e,a,c_{n},t_{n}}^d
\end{equation}

\noindent for $d \in \{ 0, 1\}$, $a \geq a^*$, where $t_{e}, t_{n}$ are the years of observation of cohorts $c_{e}, c_{n}$, and $t_e = t_n + c_e - c_n$, and $t_n$ is the year of observation of age $a$ for cohort $n$ ($t_n = a + c_n$). $\square$
\end{assumption}
\doublespacing

This assumption rules out any differences in cohort and time effects across the indicated values. The future year effect for the experimental sample is assumed to be the same as for the auxiliary sample counterpart measured at year $t_n$. 

We require that the support of the auxiliary sample contains the support of the experimental sample. This assumption is required to find counterpart values in the control and experimental samples. This assumption is straightforward to test and is satisfied in our samples. See Appendix~\ref{app:containsupport}.
...
\begin{equation}
sup( \bm{Y}_{e,a}, \bm{X}^d_{e,a}, \bm{B} ) \subseteq sup( \bm{Y}_{n,a}, \bm{X}^d_{n,a}, \bm{B} ), \quad d \in \{0,1\}. \quad \square
\end{equation}
\end{assumption}
\doublespacing

A strong sufficient condition for identifying the life-cycle profiles of individuals in the experimental sample using the individuals in the auxiliary samples is Condition~\ref{cond:cond1}.
...
\end{condition}
\doublespacing
It turns out that we are only interested in means for cost-benefit analysis. Thus, we can get by with Condition~\ref{cond:cond2}.
...
We are primarily interested in treatment effects, so we can get by with:
...
Condition~\ref{cond:cond3} does not rule out cohort or time effects but it assumes that they are equalized in experimental and auxiliary samples. Conditions~\ref{cond:cond1}--\ref{cond:cond3} could be further relaxed with direct knowledge of the cohort or/and time effects or low-dimension parameterizations of these effects (see, e.g. \citealp{Heckman_Robb_1985_ageperiod}). These conditions do not require that we take a position on the exogeneity of $\bm{X}^d_k, \: k \in \{e,n\}$. However, testing and interpretation are easier under exogeneity assumptions, as we explain next. Define $\varepsilon^d_{k,j,a}$ as the error term in Equation~\eqref{eq:outcome} in sample $k \in \{e,n\}$ for outcome $j$ at age $a$. To facilitate testing, we assume:
...
\noindent The fact that $D_e = R_e$ allows us to use experimental data to compute the distribution of $Y_{e,j,a}^d$ (i.e., $Y_{k,j,a}^d$ when fixing treatment status ($d$)).\footnote{Randomization allows us to do this.}

We assume that the variables $\bm{X}_{k,a}^d$ fully summarize treatment. Any effect that treatment has on outcomes operates through the inputs $\bm{X}_{k,a}^d$ and not through shifts in the production function relating inputs to outputs (see \citealp{Heckman_Pinto_etal_2013_PerryFactor}). Assumption~\ref{ass:summary} formalizes this.

\onehalfspacing
\begin{assumption} \label{ass:summary} \textbf{Structural Invariance}\\
For all $\bm{x}, \bm{b} \in supp(\bm{X}^d_{k,a}, \bm{B}_k)$,
\begin{align}
\phi_{k,j,a}^0 \left( \bm{x} \right) &= \phi_{k,j,a}^1 (\bm{x}, \bm{b}) \\   \nonumber
                                                                     &=: \phi_{j,a} (\bm{x}, \bm{b}),
\end{align}
\noindent $k \in \{e,n\}$, and for $a \in \{ 1, \ldots, A \}$ for any outcome $j \in \mathcal{J}_{a}$. $\square$
\end{assumption}
\doublespacing

This assumption combined with Assumption~\ref{ass:exog} and Equation~\eqref{eq:countersystem} enables us to write:
...
We can test relationship~\eqref{eq:withbetimplication} for $a \leq a^*$, when we observe $Y_{k,j,a}$ in both the experimental and auxiliary samples. The test is very similar to the test of Assumption~\ref{ass:summary}. We test the null hypothesis that there is no effect of sample membership on the conditional expectation computing $\mathbb{E} \left[ Y_{k,j,t} | \bm{X}^d_{k,t} = \bm{x}, \bm{B}_a = \bm{b} \right], \; d \in \{0,1\}$, pooling the experimental (treatment and control groups) and the auxiliary samples. When we do so, we do not reject the null hypothesis of no effect of conditioning on $\bm{X}_{k,a}^d$ and $\bm{B}_k, \: k \in \{e,n\}$.\footnote{This holds when pooling males and females and when testing by gender (see Appendix~\ref{app:invariance}).}

We can estimate non-parametric and semi-parametric relationships more precisely by focusing only on certain values $\left( \bm{X}^d_{k,a} \right)$ in the auxiliary samples that align with the experimental samples. We discuss this in Appendix~\ref{appendix:match}. Matching in this fashion reduces the need for parametric assumptions for the prediction models.
...
Previous analyses of ABC pay special attention to special education, arguing that savings due to a reduction in this category are substantial \citep{Barnett_Masse_2002_benefitcost,Barnett_Masse_2007_EER}.\footnote{Their analyses do not include CARE.} \textbf{[JJH: Jorge, update.]} This category is much less important in our calculation. Pooling males and females the net gain due to a reduction in special education is $\$3,633.3$ (2014 USD) (s.e. $703.7)$. For males the gain is $\$13,417.9$ (2014 USD) (s.e. $1,061.0$) and for females there is actually a loss of $6,276.5$ (2014 USD) (s.e. $\$1,018.5$).\footnote{This quantity is not discounted to ensure comparability with other analyses.} \textbf{[JJH: Please get updates from Jorge.] [JLG: These numbers are now up to date.]}
...
When analyzing different counterfactuals, there is a substantial difference between males and females. The difference is driven by one of the counterfactuals: treatment vs. alternative preschools. The estimates for both males and females generate a similar estimated ABC/CARE treatment effect compared to staying at home. Males benefit much more than treatment relative to alternative preschools than what they benefit of treatment relative to staying at home. This result is consistent with three sets of findings noted elsewhere: (i) stark gender differences resulting from attending low quality childcare \citep{Kottelenberg-Lehrer_2014_Gender-Effects,Baker_Gruber_Milligan_2015_Noncog_Defects}; (ii) females being less sensitive to uncertain environments \citep{Autor-etal_2015_Family-Disadvantage}. The program substantially reduced the costs of special education for boys. It raises the cost of education for girls because more of them attend school.
