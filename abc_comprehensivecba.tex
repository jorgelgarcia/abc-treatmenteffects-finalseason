...
\externaldocument{abc_comprehensivecba_appendix_2016-09-08b_jlg}
\pagenumbering{roman}
...
Let $\bm{Y}_t$ denote a vector of outcome of interest at age $t \in \{ 0,\dots,5 \}$ (individual subscripts are omitted for simplicity). The life cycle consists of $T+1$ periods $\left[0,\dots,T\right]$. The treatment phase is the first $t^*$ periods $\left[0,\dots,t^*\right]$. We have data up to age $t^{**}$. We lack data on the remainder of life $\left[t^*,\dots,T\right]$. $R \in \{0,1\}$ where $R=1$ indicates that a person was randomized into the program given $D=1$. $D=1$ indicates that parents agree to participate in the program.

Individuals are eligible to participate in the program if baseline background variables $\mathcal{B}\in\mathcal{B}_0$. In our context, $\mathcal{B}_0$ is the set of scores on the high risk index (HRI) previously discussed. As it turns out, empirically all of the eligible persons ($\mathcal{B}\in\mathcal{B}_0$) choose to participate in the program so
\begin{align}
Pr(D=1 | \mathcal{B} \in \mathcal{B}_0) = 1.\\
R=1 \Rightarrow D=1.
\end{align}

Thus, the treatment effects generated by the experiment are average treatment effects for the population for which $\mathcal{B}\in\mathcal{B}_0$ and not just treatment effects for the treated (TT). In addition, there is full compliance with the randomization. This, by itself, shows that \emph{ex ante} parents perceived that ABC/CARE was superior to other childcare alternatives. Thus, the treatment effects reported in this paper are average treatment effects for
$\mathcal{B}\in\mathcal{B}_0$. Let $\bm{Y}^1_t$ be the outcome vector for the treatment groups. There are two counterfactual outcome states for control groups:

\begin{eqnarray}
Y_{t,H}^0 &:& \textbf{ Subject stays at home in time period $t$} \nonumber \\
Y_{t,C}^0 &:& \textbf{ Subject attends alternative preschool in time period $t$}.  \nonumber
\end{eqnarray}
...
\begin{align}
&\bm{Y}_{t,H}^0, t= &\bm{Y}_H^0  \\
&\qquad \qquad \qquad \qquad \text{for all } t\in\{0,\dots,5\} \nonumber\\
&\bm{Y}_{t,C}^0, t =& \bm{Y}_C^0.
\end{align}

Let $V$ denote the fraction of time a child spends in an alternative preschool. The post-treatment counterfactual outcome when a child is in control status $\bm{Y}^0$ is
\begin{equation}
\bm{Y}^0 : = \left( 1 - V \right) \bm{Y}_H^0 + \left( V \right) \bm{Y}_C^0.\footnotemark
\end{equation}
\footnotetext{Technically, this is a per period average outcome.}
Our analysis abstracts from the dynamics of $V$ and any associated dynamics in outcomes. Ideally, we would construct the counterfactual $\bm{Y}_C^0 \left( V \right)$ specific to each realization of $V \in [0,1]$. However, this is empirically infeasible in light of the small number of observations available to us. Instead, we break $V$ into two categories, $V=0$ and $V>0$.\footnote{We treat $V$ as binary.}

One parameter of interest addresses the question: what is the effect of the program as implemented? i.e, what is the effect of the program without setting the parental decision to enroll the subject in an alternative preschool? This parameter is
\begin{equation}\label{eq:effect}
\bm{\Delta} := \mathbb{E} \left[ \bm{Y}_t -  \bm{Y}_t | D =1 \right] = \mathbb{E} \left[\bm{Y}^1_t - \bm{Y}^0_t | \mathcal{B} \in \mathcal{B}_0 \right].
\end{equation}
It addresses the effectiveness of the program relative to the quality of all alternatives available when the program was implemented for the sample of eligible persons. Random assignment identifies this parameter.
...
\bm{\Delta} \left(V = 0 \right) : =   \mathbb{E} \left[ \bm{Y}^1 \left( V \right) - \bm{Y}^0 \left( V \right) | V = 0, D = 1 \right] := \mathbb{E} \left[\bm{Y}^1 (V) - \bm{Y}^0 (V) | V = 0, \mathcal{B} \in \mathcal{B}_0 \right].
\end{equation}
...
We report simple combining functions that count the proportion of treatment effects by category that show beneficial outcomes. Consider a block of outcomes $N_l$ indexed by set $Q_l$. Let $j \in Q_l$ be a particular outcome within block $l$. Associated with it is a mean treatment effect

\begin{equation}
\Delta_j : = E(Y^1_j - Y^0_j | \mathcal{B} \in \mathcal{B}_0).
\end{equation}

We assume that outcomes can be evaluated and ordered so that $\Delta_j >0$ is a beneficial outcome.\footnote{All but 5\% of the outcomes we study can be ranked in this fashion. See Appendix~\ref{appendix:results} for our classification.} We summarize the effects of the program on outcomes within the block by the number of positive counts

\begin{equation}
C_l = \displaystyle\sum^{N_l}_{j=1} 1 (\hat{\Delta}_j >0).
\end{equation}
That is, we count the number of beneficial outcomes within each block. The proportion of beneficial outcomes in block $l$ is $C_l / N_l$.
...
The results for females show that the program increases employment, high school graduation, and college graduation. It reduces arrest (see Table~\ref{table:tescombined}). These results strengthen when we analyze females who did not receive alternative preschool. The results for education, child income, parental income, and crime are robust to controlling for substitution bias.
...
The central focus of this paper is on summarizing the multiple benefits of ABC/CARE using benefit/cost and rate of return analysis. This requires predicting the out of sample benefits of the program over the remainder life cycle. This is a fundamental recurrent problem in social science.\footnote{See, e.g., \cite{Heckman_Lochner_ea_2006_HEE}.} This section explains our strategy for using multiple data sources to interpolate and extrapolate the life-cycle costs and benefits of education, labor income, crime, and health.\footnote{The methodology for doing this exercise for public-transfer income is analogous to that of labor income so we suppress the description for brevity.} A variety of important practical issues are discussed in Appendix~\ref{appendix:methodology}.

\subsection{Predicting Future Outcomes}

Before discussing our approach to forecasting individual life cycle components, it is helpful to use a general notation. Let $Y^d_{j,t}$ be outcome $j$ at time $t$ for a treated person $(d\in\{0,1\})$. We introduce the notion of determinants of outcomes---vector $\bm{X}^d_t$---that may themselves be the outcomes of treatment, and may depend on lagged outcomes also affected by treatment. These variables are generated by the following equation
\begin{equation}\label{eq:trenchfoot}
Y^d_{j,t} = \phi^d_{j,t} (X^d_t, \mathcal{B}, \varepsilon^d_{j,t}), \quad d \in \{0,1\}
\end{equation}
where $j \in \mathcal{J}_t$ (the index set of possible outcomes) and $\varepsilon^d_{j,t}$ is a scalar unobservable determinant of $Y^d_{j,t}$. $Y^d_{j,t-1}$ can be included among the $X^d_{j,t}$. The equation of motion for determinant $k \in \mathcal{K}_t$ (the set of determinants) is given by
\begin{equation}\label{eq:frostbite}
X^d_{k,t} = \gamma^d_{k,t} (X^d_{t-1}, \mathcal{B}, \eta^d_{k,t}), \quad d \in \{0,1\}
\end{equation}
where $\eta^d_{k,t}$ is an unobserved determinant of $X^d_{k,t}$. At this point, the dependence among the $\varepsilon^d_{j,t}$ and the $\eta^d_{k,t}$ is assumed to be general.

The $X^d_{k,t}$ are allowed to be caused by treatment. Examples in our data are education, work experience, and crime.

We assume \emph{structural invariance} of \eqref{eq:trenchfoot} and \eqref{eq:frostbite}.\footnote{See, e.g., \citet{Frisch_1938_autonomy}, \citet{Haavelmo_1943_Econometrica,Haavelmo_1944_Econometrica}, \citet{Hurwicz_1962_structural}, and \citet{Heckman_Pinto_2015_EconometTheory}.} (Assumption~\eqref{ass:butts}). This means that however the arguments of \eqref{eq:trenchfoot} and \eqref{eq:frostbite} are determined, for the same numerical values of $(X^d_t, \mathcal{B}, \varepsilon^d_t)$ and $(X^d_t, \mathcal{B}, \eta^d_{j,t})$, we get the same values of $Y^d_{j,t}$.\footnote{Later work in statistics captures this concept by ``SUTVA.'' See \citet{Holland_1986_JASA} and \citet{Heckman_2008_ISR}.} This is a fundamental property of an invariant economic relationship used by economists since the time of \citet{Frisch_1938_autonomy}.

\begin{assumption}\label{ass:butts}
\begin{align}
&\text{Setting } X^d_t = (x_t, B=b, \varepsilon^d_t = \bar{\varepsilon}^d_t) \notag \\
&Y^d_{j,t} = \phi^d_{j,t} (x_t, b,  \bar{\varepsilon}^d_{j,t}) \notag \\
&\text{and setting } X^d_{t-1} = (x_{t-1}, B = b, \eta^d_t = \bar{\eta}_t) \notag \\
&X^d_{k,t} = \gamma^d_{k,t} (x_{t-1}, b, \bar{\eta}_t) \notag
\end{align}
\end{assumption}

Equations~\eqref{eq:trenchfoot} and \eqref{eq:frostbite} can be fit over the sample period $(0,\ldots,t^{**})$. We do not observe data on the experimental group after $t^{**}$. To predict outside the sample period, we access auxiliary data on older cohorts of individuals whose life cycle equations of motion are governed by Equations~\eqref{eq:trenchfoot} and \eqref{eq:frostbite}. This presumes absence of cohort effects. (Assumption A-2). We discuss this assumption below when we discuss our empirical estimates. Such an assumption is maintained in many studies of human capital investments (see \citealp{Heckman_Lochner_ea_2006_HEE}).

\begin{assumption}\label{ass:crotchrot}
\textbf{Absence of Cohort Effects}
\end{assumption}

Our auxiliary data do not have direct information on program participation. However, given the limited size of the sample experiment, it is reasonable to assume that no one is treated ($D=0$). This means that \eqref{eq:trenchfoot} and \eqref{eq:frostbite} for $d=0$ can be identified over the available supports of the available samples in the post-sample period. This implies that we can identify the post-experimental control group outcomes for the experiment if the support of the auxiliary data includes the support of the experimental data. Formally, if we let $S \in \{0,1\}$ indicate membership in the experimental sample ($S=1$ if in the experimental sample; $S=0$ otherwise), and letting $X_s = (X_0,\dots,X_T)_s$ and $Y_s = (Y_0,\dots,Y_T)_s$ where $s$ denotes sample membership, we assume

\begin{assumption}
\begin{equation*}
\text{sup}(X, Y, B)_{s=1} \subseteq \text{sup} (X, Y, B)_{s=0}.
\end{equation*}
\end{assumption}

In particular, we assume that $\mathcal{B}_0$ is contained in the support of the auxiliary sample.

Forecasting treatment group outcomes is a more challenging task. We do so by invoking the assumption that outcome equations \eqref{eq:trenchfoot} and \eqref{eq:frostbite} are the same across treatment and control group once we control for the treatment-induced determinants of $X$ and $Y$.

Formally, we assume

\renewcommand\theassumption{A--\arabic{assumption}(a)}
\begin{assumption}\label{ass:eczema}
Given $X^d_t, \mathcal{B}, \varepsilon^d_{j,t}$,
\begin{equation*}
\phi^1_{j,t} (X^d_t, \mathcal{B}, \varepsilon^d_{j,t}) = \phi^0_{j,t} (X^d_t, \mathcal{B}, \varepsilon^d_{j,t}).
\end{equation*}
\end{assumption}
...
We make a corresponding assumption for the evolution of inputs:
\addtocounter{assumption}{-1}
\renewcommand\theassumption{A--\arabic{assumption}(b)}
\begin{assumption}\label{ass:psoriasis}
Given $X^d_{t-1}, \mathcal{B}, \eta^d_{l,t}$,
\begin{equation*}
\gamma^1_{j,t} (X^d_{t-1}, \mathcal{B}, \eta^d_{t-1}) = \gamma^0_{j,t} (X^d_{t-1}, \mathcal{B}, \eta^d_{t-1}).
\end{equation*}
\end{assumption}

\ref{ass:eczema} and \ref{ass:psoriasis} are versions of structural invariance assumptions.
...
Components of $X^{d}_{0t}$ many not be observed. We can replace expectations by projections in forming mean treatment effects. If $\tilde{X}^{d}_{t}$ denotes observed input variables, we can replace~\eqref{eq:trenchfoot} and~\eqref{eq:frostbite} with projections.

\begin{center}
Question: Are these unbiased?\\
General Issue:
\end{center}

\begin{align*}
Y &= G(X,\varepsilon) \\
E^{\ast}(T|\tilde{X}) &= E^{\ast}(G(X,\varepsilon)|\tilde{X})
\end{align*}

By construction, this is unbiased for $Y$.

Under \ref{ass:butts}-\ref{ass:fox} projects unbiased -- can deal with omitted inputs.

\textbf{[JJH: More to come.]}
...
\begin{sidewaysfigure}[!htbp]
\centering
\caption{Qality Adjusted Life Years}\label{fig:qalys}
\begin{subfigure}[h]{0.4\textwidth}
		\centering
		\caption{Forecasted for ABC/CARE Males} \label{fig:qabcare1}
		\includegraphics[width=\textwidth]{output/qalylcycle_s1.eps}
\end{subfigure}%
\begin{subfigure}[h]{0.4\textwidth}
	\centering
	\caption{PSID, Disadvantaged Males} \label{fig:qpsid1}
		\includegraphics[width=\textwidth]{output/psidqaly_s1.eps}
\end{subfigure}
\begin{subfigure}[h]{0.4\textwidth}
		\centering
		\caption{Forecasted for ABC/CARE Females} \label{fig:qabcare0}
		\includegraphics[width=\textwidth]{output/qalylcycle_s0.eps}
\end{subfigure}%
\begin{subfigure}[h]{0.4\textwidth}
	\centering
	\caption{PSID, Disadvantaged Females} \label{fig:qpsid0}
		\includegraphics[width=\textwidth]{output/psidqaly_s0.eps}
\end{subfigure}
\footnotesize \justify
Note: Panels (a) and (c) display the forecasted quality-adjusted life years (QALYs) for ABC/CARE males and females by treatment status, based on forecasts that combine data from the Panel Study of Income Dynamics (PSID), the Health Retirement Study, the Medical Expenditure Panel Survey (MEPS), and the Medicare Current Beneficiary Survey (MCBS). Panels (b) and (d) display the forecasted quality-adjusted life years for disadvantaged males and females in the Panel Study of Income Dynamics (PSID), where disadvantaged is defined as being African American and having less than 12 years of education. QALYs are the quality-adjusted life years gain due to better health conditions\\
\end{sidewaysfigure}
