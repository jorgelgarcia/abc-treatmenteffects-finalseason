...
A central goal of this paper is to summarize the multiple benefits of ABC/CARE using benefit/cost and rate of return analyses. This requires access to data outside the ABC/CARE experiment to predict the costs and benefits of the program over the life cycle after the measurement phase of the study ends. This fundamental problem has long been addressed using a synthetic cohort approach.\footnote{See, e.g., \citet{Mincer_1974_schooling}.} Outcomes of ``comparable'' older cohorts not exposed to the program in auxiliary samples are used to find treatment and control group members.\footnote{See, e.g., \cite{Heckman_Lochner_ea_2006_HEE}. \citet{Heckman_Moon_etal_2010_RateofReturn} use a variety of methods to predict future labor income in the Perry Preschool Project. They fit a variety of panel data labor income dynamics models and project future labor income with them. The various methods are in general agreement.} But older cohorts may have received different endowments and different environments. An assumption of stationarity in earnings profiles may be grossly at odds with the data. 

This section explains our strategy for constructing out-of-sample treatment effects for synthetic cohorts. Our approach starts from and extends the analysis of \citet{Heckman_Pinto_etal_2013_PerryFactor}, who show, in a setting similar to ours, that the effect of treatment on outcomes operates through its effects on inputs in a stable production function rather than through shifts in the production function.

To construct benefit-cost estimates we (i) \textit{interpolate} components not observed due to intermittent data collection; and (ii) \textit{extrapolate} (predict) components not yet observed because the latest follow-up is taken when the subjects are in their mid-30s. We use subsamples of multiple sources of auxiliary data that are older versions of representative populations in our sample. Table~\ref{table:sources} presents the outcomes for which we conduct these analyses and the data sources used. We initially focus on labor income to illustrate our approach, but a similar methodology is used to predict other outcomes.
...
This subsection presents an informal summary of our approach. The rest of the section gives a formal justification, tests of key assumptions, and applications to other outcomes besides earnings. We have data on control- and treatment-group members through age $a^{\ast}$. We can estimate treatment effects within the experimental sample. We lack information on participant outcomes afterward. Yet, post-$a^{\ast}$ treatment effects are required to construct counterfactual life-cycle profiles.

Making valid predictions of out-of-sample treatment effects does not require making valid predictions of separate out-of-sample treatment and control profiles. Only valid predictions of their difference is required. Nonetheless, in this paper we focus on making valid predictions of separate treatment and post-$a^*$ profiles. Doing so allows us to test the validity of our methodology by comparing (within the support of the experimental sample) outcomes for the experimental control and treatment groups with those from the synthetic control and treatment groups we generate. 
...
We now present our analytical framework and its underlying assumptions. Omitting individual subscripts, outcome $j$ at age $a$ in the set of outcomes $\mathcal{J}_{a}$ is:
\begin{equation}\label{eq:outcome}
Y_{k,j,a}^d = \phi_{k,j,a}^d \left( \bm{X}_{k,a}^d, \bm{B} \right) + \varepsilon_{k,j,a}^d, \qquad j \in \mathcal{J}_a,
\end{equation}
where $d$ indicates that a person is fixed to control ($d = 0$) or treatment ($d = 1$) status and $k$ indicates if the outcome is observed in the experimental sample ($k = e$) or in the auxiliary sample ($k = n$). \footnote{Fixing and conditioning are fundamentally different concepts. See \citet{Heckman_Pinto_2015_EconometTheory} for a discussion. Our analysis applies to the methodology in that paper.} So far, we let the relationships between the dependent and independent variables in \eqref{eq:outcome} to not necessarily coincide across these samples. We interpret  $\phi_{k,j,a}^d \left( \cdot \right)$ as a production function mapping the inputs $\bm{X}_{k,a}^d$ together with background variables $\bm{B}$ to the output $Y_{k,j,a}^d$. Among the $\bm{X}^d_{k,a}$ are variables caused by treatment, including lagged dependent variables. 

Let $\bm{Y}_k^d$ denote all outcomes at all ages for $k \in \{e, n \}$, when treatment status is fixed to $d$. Similarly, $\bm{X}_k^d$ is the vector of all causal predictors of $\bm{Y}_k^d$ at all ages. For simplicity, we absorb $\bm{B}$ into this vector. Both $\bm{Y}_k^d$ and $\bm{X}_k^d$ include the full set of outcomes over the life cycle, even though they are not sampled after age $a^*$. We denote the joint distribution of these vectors by $F_{\bm{Y}_k^d, \bm{X_k^d}}$.

The auxiliary data come from older cohorts for which we observe the full life cycles, i.e. we observe $a \in \{1, \ldots, A\}$. In the auxiliary data, however, we do not observe treatment status, $d$. Even if we did, we do not know if cohort (c) or time (t) effects make make the experiences of the auxiliary-sample individuals different from the experiences of the experimental-sample individuals. To formalize this, let $Y_{j,k,a,c,t}^d$ be outcome $j$ for sample $k$ at age $a$ for cohort $c$ at time $t$ when treatment is fixed to $d$. We assume that Assumption~\ref{ass:alignment} holds, and henceforth drop the ``$c$'' and ``$t$'' sub-indices.

\onehalfspacing
\begin{assumption} (Alignment of Cohort and Time Effects) \label{ass:alignment}
For experimental sample cohort $c_{e}$ and auxiliary sample cohort $c_{n}$: 
\begin{equation}
Y_{j,e,a,c_{e},t_{e}}^d = Y_{j,e,a,c_{n},t_{n}}^d
\end{equation}

\noindent for $d \in \{ 0, 1\}$, $a \geq a^*$, where $t_{e}, t_{n}$ are the years of observation of cohorts $c_{e}, c_{n}$. $\square$
\end{assumption}
\doublespacing

A sufficient condition to identify the the life-cycle profiles of the individuals in the experimental sample using the individuals in the auxiliary samples is Condition~\ref{cond:cond1}. It turns out that we are only interested in averages. Thus, we can get by with Condition~\ref{cond:cond2}. Further, we are only interested in average treatment effects, so we can actually get by with Condition~\ref{cond:cond3}. Either of these conditions could be further relaxed with direct knowledge of the cohort or/and time effects or low-dimension parameterizations (see, e.g. \citet{Heckman_Robb_1985_ageperiod}). 

\onehalfspacing
\begin{condition} \textbf{Equality in Distributions Across the Experimental and Auxiliary Samples \label{cond:cond1}}
\begin{equation}
F_{\bm{Y}_e^d, \bm{X_e^d}} \left( \cdot, \cdot \right) = F_{\bm{Y}_n^d, \bm{X_n^d}} \left( \cdot, \cdot \right) 
\end{equation}

\noindent for $d \in \{0, 1 \}$.
\end{condition}
\doublespacing

\onehalfspacing
\begin{condition} \textbf{Equality in Expectations Across the Experimental and Auxiliary Samples \label{cond:cond2}}
\begin{equation}
\mathbb{E} \left[ \bm{Y}_e^d |  \bm{X_e^d} = \bm{x} \right] = \mathbb{E} \left[ \bm{Y}_n^d |  \bm{X_n^d} = \bm{x} \right]
\end{equation}
...
\onehalfspacing
\begin{condition} \textbf{Equality in Expected Treatment Effects Across the Experimental and Auxiliary Samples \label{cond:cond3}}
\begin{equation}
\mathbb{E} \left[ \bm{Y}_e^1 -  \bm{Y}_e^0 |  \bm{B} = \bm{b} \right] = \mathbb{E} \left[ \bm{Y}_n^1 -  \bm{Y}_n^0 |  \bm{B} = \bm{b} \right]. 
\end{equation}
\end{condition}
\doublespacing

The weakest form of these conditions, Condition~\ref{cond:cond3}, does not require for us to take any position on exogeneity of either. The condition only requires for biases to have the same patterns in the experimental and auxiliary samples. Testing and interpretation are easier under Condition~\ref{cond:cond1} and under exogeneity assumptions, as we explain next. In sum, we formalize assumptions under which it is possible to identify $\mathbb{E} \left[ Y_{e,j,a}^d | \bm{X}_{a}^d \right]$ out of sample. We use an economically-motivated assumption on the production function in \eqref{eq:outcome} and a set of additional assumptions to construct valid out-of-sample predictions. Our framework has testable implications, which we examine below.

Exogeneity is one of the additional assumptions and we formalize it as follows: 

\onehalfspacing
\begin{assumption}\label{ass:exog} \textbf{Exogeneity in the Experimental Sample}\\
For all $a, a'' \in \{ 1, \ldots, T \}$ and for $d, d' \in \{0,1\}$,
\begin{equation}
\varepsilon^d_{k,j,t} \indep \bm{X}^{d^{\prime}}_{k,a^{''}} 
\end{equation}
for all outcomes $j \in \mathcal{J}_{t}$, where ``$\indep$'' denotes independence. $\square$
\end{assumption}
\doublespacing
...
\begin{spacing}{1}
\begin{align}
Y_{k,j,a}     =& \left( 1 - D \right) Y_{k,j,a}^0    + \left( D \right) Y_{k,j,a}^1, \\
&&j \in \mathcal{J}_a, a \in \{1,\dots,a^*\} \nonumber \\
\bm{X}_{k,a} =& \left( 1 - D \right) \bm{X}_{k,a}^0 + \left( D \right) \bm{X}_{k,a}^1. \nonumber \label{eq:countersystem}
\end{align}
\end{spacing}
\noindent The fact that $D = R$ allows us to use experimental data to compute the distribution of $Y_{e,j,a}^d$ (i.e., $Y_{k,j,a}^d$ when fixing treatment status ($d$)).\footnote{Randomization allows us to do this.}

We assume that the variables $\bm{X}_{k,a}^d$ fully summarize treatment. Any effect that treatment has on an outcome operates through the inputs $\bm{X}_{k,a}^d$ and not through shifts in the production function relating inputs to outputs. Further, we assume that the variables in $\bm{X}_{k,a}^d$ any differences in the production of $Y_{k,j,a}^d$ across individuals in the experimental and auxiliary samples, and not shifts in the production functions across the samples. Assumption~\ref{ass:summary} formalizes this.

\onehalfspacing
\begin{assumption} \label{ass:summary} \textbf{Structural Invariance}\\
For all $(\bm{x}) \in supp(\bm{X}^d_{k,a})$,
\begin{align}
\phi_{k,j,a}^0 \left( \bm{x} \right) &= \phi_{k,j,a}^1 \left( \bm{x} \right) \\   \nonumber
                                                                     &=: \phi_{j,a} \left( \bm{x}  \right)
\end{align}
\noindent and for $a \in \{ 1, \ldots, a^\ast \}$ for any outcome $j \in \mathcal{J}_{a}$. $\square$
\end{assumption}
\doublespacing
...
\begin{align}
E \left[ Y_{k,j,t}^d | \bm{X}_{a}^d  = \bm{x}, D = d \right] &=  E  \left[ Y_{k,j,t} | \bm{X}_{a}^d  = \bm{x} , D = d \right] \nonumber\\
&=E  \left[ Y_{k,j,t} | \bm{X}^d_{a}  = \bm{x} \right], a \in \{1,\dots,A\} \label{eq:invariancetest}
\end{align}
...
Equation~\eqref{eq:invariancetest} is a statement relating outcomes for $Y_{k,j,t}^d$ to treatment effects for $\bm{X}_{k,t}^d$, together with the background variables $\bm{B}$. It is possible to test \ref{ass:summary} within the experimental sample at ($a \leq a^*$). The test consists of asking if $\bm{X}_{e,a}^d, \bm{B}$ predict the within experimental sample treatment effects for $Y_{e,j,t}$. Under the null that \ref{ass:summary} is correct, a separate indicator variable for treatment status is irrelevant when computing $E \left[ Y_{e,j,a} | \bm{X}_{a}^d  = \bm{x}, \bm{B} =  \bm{b}, D = d \right]$. In Appendix~\ref{app:invariance}, we test the null hypotheses implied by this assumption for the outcomes that we predict in our cost-benefit analysis. We do not reject the null hypothesis that the coefficient associated with random assignment to treatment is different from zero conditioning on $\bm{X}_{e,a}^d, \bm{B}$.\footnote{This holds both when pooling males and females and when testing separately by gender (see Appendix~\ref{app:invariance}).}

While we do not require exogeneity, it is useful to test the additional assumptions. Let $K$ indicate whether an observation belongs to the experimental sample ($K = e$) or to the auxiliary sample ($K = n$). By equation \eqref{eq:invariancetest}, we can write: 

\begin{equation}\label{eq:withbetimplication}
\mathbb{E} \left[ Y_{j,a} | \bm{X}^d_{a} = \bm{x}, K=e \right] = \mathbb{E} \left[ Y_{k,j,a} | \bm{X}^d_{a} = \bm{x} \right], \quad d \in \{0,1\}, \quad k \in \{e,n\}.
\end{equation}

We can test Equation~\eqref{eq:withbetimplication} for $a \leq a^*$, when we observe $Y_{e,j,a}$ in both the experimental and auxiliary samples. The test is very similar to the test of Assumption~\ref{ass:summary}. We test the null hypothesis that there is no effect of $K$ on the conditional expectation computing $E \left[ Y_{k,j,t} | \bm{X}^d_{k,t} = \bm{x}, K = k \right], \; d \in \{0,1\}$, pooling the experimental (treatment and control groups) and the auxiliary samples. When we do so, we do not reject the null hypothesis of no effect of $S$ conditioning on $\bm{X}_{k,a}^d$.\footnote{This holds when pooling males and females and when testing by gender (see Appendix~\ref{app:invariance}).}

We can estimate non-parametric and semi-parametric relationships by focusing only on certain values of $\left( \bm{X}^d_{k,a} \right)$ in the auxiliary samples. We discuss this in Appendix~\ref{appendix:match}. Matching in this fashion reduces the need for parametric assumptions for the prediction models.

Finally, we invoke Assumption~\ref{ass:contain} to guarantee that the dynamic relationship in the auxiliary sample is valid for any value of $\left( \bm{X}_{k,a}, \bm{B} \right)$ in the experimental sample. We reintroduce $\bm{B}$ for clarity. \footnote{Is is straightforward to test this assumption. We provide evidence favoring it in Appendix~\ref{app:containsupport}.}

\onehalfspacing
\begin{assumption} \label{ass:contain} \textbf{Support Conditions} \\
For $a \in \{ 1, \ldots, A \}$, the support of $\left( \bm{Y}_{e,a}, \bm{X}^d_{e,a}, \bm{B} \right)$ in the experimental sample is contained in the support of $\left( \bm{Y}_{n,t}, \bm{X}^d_{n,t}, \bm{B} \right)$ in the auxiliary sample:
\begin{equation}
sup( \bm{Y}_{e,a}, \bm{X}^d_{e,a}, \bm{B} ) \subseteq sup( \bm{Y}_{n,a}, \bm{X}^d_{n,a}, \bm{B} ), \quad d \in \{0,1\}. \square
\end{equation}
\end{assumption}
\doublespacing
...
Let Assumptions~\ref{ass:exog} and \ref{ass:summary} hold and let Assumption~\ref{ass:contain} hold for the experimental and the synthetic control and treatment groups constructed from the auxiliary sample constructed using our analysis. Then, for any value of $\left( \bm{X}_{a}, \bm{B} \right)$, $\mathbb{E} \left[ Y_{j,a} | \bm{X}^d_{a} = \bm{x}, \bm{B} = \bm{b}, K = n \right]$ is a consistent out-of-sample prediction of $Y_{k,j,t}^d$ for $d \in \{ 0, 1\}$, for all $j \in \mathcal{J}_{t}$ and for any $a \geq a^*, a \in \{a^*, \ldots, A \}$. $\Box$
\end{theorem}

\noindent \begin{proof}\\
We need to show that
\begin{equation}
\mathbb{E} \left[ Y_{j,a} | \bm{X}^d_{a} = \bm{x}, \bm{B} = \bm{b}, K = n \right] = \mathbb{E} \left[ Y_{e,j,a}^d | \bm{X}_{e}^d = \bm{x}, \bm{B} = \bm{b}  \right], \quad d \in \{0,1\}.
\end{equation}
Equality follows from the arguments above for which equation~\eqref{eq:invariancetest} holds.
\end{proof}
\doublespacing

\subsubsection{Testing for Endogeneity}\label{section:accendog}

We investigate the endogeneity issue in the experimental and auxiliary samples in Appendix~\ref{app:endogeneity}. We follow \citet{Heckman_Pinto_etal_2013_PerryFactor} and assume that $\varepsilon_{k,j,t}^d$ obeys a factor structure, $k \in \{e,n\}$. In Appendix~\ref{app:endogeneity}, we provide evidence supporting exogeneity in both samples.
