...
We now present our analytical framework and its underlying assumptions for the Stage II forecasting analysis. Our analysis is based on a causal model for treatment ($d=1$) and control ($d=0$) outcomes for measure $j$ at age $a$ in sample $k \in \{e,n\}$ where $e$ denotes membership in the experimental sample and $n$ denotes membership in the control sample:
...
\subsubsection{Summarizing Our Approach and What We Do in Practice}

Using the data sources listed in Table~\ref{table:sources}, we execute our analysis in two stages. In Stage I, we construct comparison samples of group members using different criteria: (a) barebones eligibility criteria ($\bm{B}_n \in \mathcal{B}$); (b) using $\bm{X}^d_k$ (variables affected by treatment); (c) using both (a) and (b). In constructing comparison groups using $\bm{X}^d_k$, we are limited to variables common to both experimental and comparison group samples. In Stage II, we use the samples so constructed to create post-$a^\ast$ forecasting models. The non-parametric estimates described in Section~\ref{usingmatching} combine the two stages. In Appendix~\ref{appendix:match}, we report sensitivity analyses based on different choices of $\bm{B}$ and $\bm{X}^d$. We find that, in our samples, our estimates are robust to alternative choices of matching criteria in constructing comparison samples in Stage I.
