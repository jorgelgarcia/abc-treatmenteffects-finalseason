...
Following \cite{Mincer_1974_schooling} around $t=t^*$ (early 30s), we can estimate a good measure of perpetuity flow of lifetime earnings---i.e., $\frac{Y_{t^*}}{r}=NVP$ of earnings (this is Mincer's earnings at overtaken age). The estimated treatment effect for net present value in. Basic points we can get within sample experimental estimates of $\mathbb{E}[\frac{(\bm{Y}^{1}_{t^{\ast}} - \bm{Y}^{0}_{t^{\ast}})}{r}\large], t < t^*$ but unless we assume that $\bm{X}^1_t-\bm{X}^0_t$ spans fully all omitted inputs, we cannot get causal effect of inputs even within experiments.



\subsubsection{Model with Measurements}

\begin{align}
M^d_t &= \lambda^d \theta^d_t + U^d_t \quad \theta^d_t \indep U^d_t \forall t \\
\varepsilon^d_t &= \beta^d \theta^d_t + w^d_t \quad \theta^d_t \indep w^d_t \forall t  \notag
\end{align}

With our auxiliary measurement system and three or more measures, given normalizations, we can identify $\lambda^d$, $\sum_{\theta^d_t}$, $\sum_{U^d_t} d \in \{0,1\} t \in \{0,\dots,T\}$ and use the Bartlett method to predict factors $\hat{\theta}^d_t$. We control for factors in fitting Equation~\eqref{eq:trenchfoot} and can correct for measurement error.

Basic points we can get within sample experimental estimates of \\ $\mathbb{E}(\bm{Y}^1_t - \bm{Y}^0_t | \bm{B} \in \bm{B}_0), t < t^*$ but unless we assume that $\bm{X}^1_t-\bm{X}^0_t$ spans fully all omitted inputs, we cannot get causal effect of inputs even within experiments.
