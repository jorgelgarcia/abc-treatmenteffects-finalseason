...
*QALYs refers to the quality-adjusted life years. Any gain corresponds to better health conditions until predicted death, with $\$150,000$ (2014 USD) as base value for a year of life. \textbf{[Typist: No loner a slant in category titles]}\\}
\end{sidewaysfigure}
...
We construct synthetic treatment and control cohorts.\footnote{See, e.g., \citet{Mincer_1974_schooling} and the survey in \citet{Heckman_Lochner_ea_2006_HEE}.} Outcomes of ``comparable'' older cohorts in auxiliary samples who are not exposed to the program are used to construct virtual treatment and control groups. A major problem is finding counterparts to the treatment group in the auxiliary samples. Who among those samples would have received treatment if the program had been available to them? In addition, older cohorts may have received different endowments and experienced different environments. An assumption of stationarity in life cycle profiles across cohorts may be grossly at odds with the data.
...
We interpolate components not observed due to intermittent data collection and construct counterfactual components not yet observed. Table~\ref{table:sources} presents the outcomes for which we conduct these analyses and the data sources used. We initially focus on labor income to illustrate our approach, but a similar methodology is used to predict the other outcomes.
...
We first present an informal summary of our approach. The next section gives a formal justification and tests of key assumptions. The remaining sections give applications to other outcomes besides labor income. We have data on control- and treatment-group members through age $a^{\ast}$. We can estimate treatment effects within the experimental sample. We lack information on participant outcomes afterward. Post-$a^{\ast}$ treatment effects are required to construct counterfactual life-cycle profiles.
...
We now present our analytical framework and its underlying assumptions. 
 
Our analysis is based on a structural causal model for treatment ($d=1$) and control ($d=0$) outcomes for measure $j$ at age $a$ in sample $k \in \{e,n\}$ where $e$ denotes membership in the experimental sample and $n$ denotes membership in the control sample:
\begin{equation}\label{eq:outcome}
Y^d_{k,j,a} = \phi^d_{k,j,a} (\bm{X}^d_{k,a}, \bm{B}_k) + \varepsilon^d_{k,j,a}.
\end{equation}
$\phi^d_{k,j,a}$ is an invariant structural production relationship mapping inputs $\bm{X}^d_{k,a}, \bm{B}_k$ into output $Y^d_{k,j,a}$ holding error term $\varepsilon^d_{k,j,a}$ fixed.\footnote{Fixing and conditioning are fundamentally different concepts. See \citet{Heckman_Pinto_2015_EconometTheory} for a discussion. Our analysis applies to the methodology in that paper.} For specificity we normalize $\varepsilon^d_{k,i,j}$ to have mean zero. Among the $\bm{X}^d_{k,a}$ are variables caused by treatment, including lagged dependent variables. The relationships between the dependent and independent variables in \eqref{eq:outcome} do not necessarily coincide across the samples. 

Let $\bm{Y}_k^d$ denote the vector of all outcomes at all ages for $k \in \{e, n \}$, when treatment status is fixed to $d$. Similarly, $\bm{X}_k^d$ is the vector of all causal predictors of $\bm{Y}_k^d$ at all ages. Both $\bm{Y}_k^d$ and $\bm{X}_k^d$ include the full set of outcomes over the life cycle, even though they are not sampled after age $a^*$. The background variables may have different distributions in the two samples. We denote the joint distribution of these vectors conditional on $\bm{B}_k = \bm{b}$ by $F_{\bm{Y}_k^d, \bm{X}_k^d} | \bm{B}_k = \bm{b}$.

The auxiliary data $(n)$ come from older cohorts for which we observe more complete segments of their life cycles, i.e., we observe $a \in \{1, \ldots, A\}$. We do not observe what treatment status $d$ would have been in the auxiliary data. Even if we did, we do not know if cohort ($c$) or time ($t$) effects make the experiences of the auxiliary-sample individuals different from the experiences of the individuals in the experimental sample.

To formalize this problem, and our solution to it, let $Y_{j,k,a,c,t}^d$ be outcome $j$ for sample $k$ at age $a$ for birth cohort $c$ at time $t$ when treatment is fixed to $d$. We make the following assumption, and henceforth drop the ``$c$'' and ``$t$'' sub-indices. It amounts to assuming away the problem by saying cohort and time effects operate identically across samples in the following sense: 
...
\end{equation}
for $d \in \{ 0, 1\}$, $a \geq a^*$, where $t_{e}, t_{n}$ are the years cohorts $c_{e}, c_{n}$ are observed, where $t_e = t_n + c_e - c_n$, and $t_n$ is the year the age $a$ outcome is observed for cohort $n$ ($t_n = a + c_n$). $\square$
\end{assumption}
\doublespacing

This assumption rules out any differences in cohort and time effects across the indicated values. The future year effect for the experimental sample is assumed to be the same as for the auxiliary sample counterpart measured at year $t_n$. We can weaken Assumption~\ref{ass:alignment} if we have prior knowledge about year and/or cohort effects or if we can parameterize estimable functions of $c$ and $t$.\footnote{See \cite{Heckman_Robb_1985_JE}.}

We further require that the support of the auxiliary sample contains the support of the experimental sample. This assumption allows us to find counterpart values in the control and experimental samples. 
...
\end{assumption}
\doublespacing
This assumption is straightforward to test and is satisfied in our samples. See Appendix~\ref{app:containsupport}.
...
We are primarily interested in treatment effects, so we can get by with an even weaker assumption:
...
Condition~\ref{cond:cond3} does not rule out cohort or time effects. It assumes that they are equalized in experimental and auxiliary samples. These conditions do not require that we take a position on the exogeneity of $\bm{X}^d_k, \: k \in \{e,n\}$. However, using economic theory to generate and interpret treatment effects and to test the validity of our synthetic control groups is easier under exogeneity assumptions, as we explain next. To facilitate testing, we assume:
...
\end{equation}
for all $\bm{b}$ in the support of $\bm{B}_k, \: k \in \{e,n\}$, for all outcomes $j \in \mathcal{J}_{a}$, where ``$M \indep N|Q$'' denotes independence of $m$ and $N$ given $Q$. $\square$
\end{assumption}
\doublespacing

Adapting previous notation to account for sample membership, in the experimental sample, parents of eligible children ($\bm{B}_e \in \mathcal{B}_{0}$), always agree to participate in the program ($W_e = 1$) and accept treatment ($R_e = D_e$).\footnote{Thus, it is always true that $\bm{B}_e \in \mathcal{B}_{0}$ implies $W = 1$. Afterward, there are cases of attrition that could be interpreted as $R_e \neq D_e$, but this was not the case at the beginning of treatment. We distinguish attrition from non-compliance.} We assume that this condition holds in the virtual auxiliary sample. Given this condition, we can use $D_e$ and $R_e$ interchangeably and apply a standard \citet{Quandt_1972_JASA} switching regression model to write experimental outputs and inputs as
\begin{spacing}{1}
...
\noindent The fact that $D_e = R_e$ allows us to use experimental data to compute the distribution of $Y_{e,j,a}^d$ (i.e., $Y_{k,j,a}^d$ when fixing treatment status ($d$)).
...
\phi_{k,j,a}^0 \left( \bm{x}, \bm{b} \right) &= \phi_{k,j,a}^1 (\bm{x}, \bm{b}) \\   \nonumber
                                                                     &=: \phi_{j,a} (\bm{x}, \bm{b}),
\end{align}
\noindent $\phi^d_{k,j,a}(\bm{x})$ is the function generating the causal effect of setting $\bm{X}^d_{k,a}=\bm{x}$ holding $\varepsilon^d_{k,j}$ fixed for $a \in \{1,\dots,A\}$ for any outcome $j \in \mathcal{J}_{a}$. This assumption says two things: (i) the structural functions evaluated for the same arguments of the function are identical for treatment and control groups. It also says (ii) that the structural relationships are identical in the experimental and auxiliary samples. $\square$
\end{assumption}
\doublespacing

Assumption~\ref{ass:summary} combined with Assumption~\ref{ass:exog}, Equations~\eqref{eq:countersystem}, and the assumption of a zero mean for the errors $(E(\bm{\varepsilon}^d_{k,j,a}))=0$ for all $a \in \{1,\dots,A\}, d \in \{0,1\}$ and $k \in \{e,n\}$ enable us to write:
...
\end{equation}
Relationship~\eqref{eq:withbetimplication} is testable for $a \leq a^*$, when $Y_{k,j,a}$ is observed in both the experimental and auxiliary samples. We do not reject the null hypothesis of no differences in the conditional mean functions in the experimental and auxiliary samples conditioning on $\bm{X}_{k,a}^d$ and $\bm{B}_k, \: k \in \{e,n\}$.\footnote{This holds when pooling males and females and when testing by gender (see Appendix~\ref{app:invariance}).}

We note that under exogeneity assumption~\ref{ass:exog} and invariance condition~\ref{ass:summary} we can use matching to construct counterparts to treatment and control groups in the auxiliary sample.\footnote{\citet{Heckman_Ichimura_etal_1998_Econometrica} use this procedure.} We discuss this approach in Appendix~\ref{appendix:match}. Matching is one way to non-parametrically estimate the conditional mean functions. \textbf{[JJH: Jorge - do we still have the straight matching estimates? They are a test of the exogeneity assumption.] [JLG: please see Appendix C, where we do have these results and mention them as a test. We also analyze results under different forms of matching.]}

%%30b

Collecting results, under exogeneity we obtain the following theorem:

\onehalfspacing
\setcounter{theorem}{0}
\begin{theorem}\label{theorem:main} \textbf{Valid Out-of-Sample Forecasts} \\
Under Assumptions~\ref{ass:alignment}-\ref{ass:summary}, Condition~\ref{cond:cond2} holds for any value of $\left( \bm{X}^d_{k,a}, \bm{B}_k \right)$. This is an immediate consequence of the cited assumptions. $\Box$
\end{theorem}
...
Parental income is an important component of the benefit/cost ratio. We take a conservative approach in our baseline estimates and do not account for potential shifts in profiles in parental income due to education and work experience subsidized by childcare (see the discussion in Section~\ref{section:pincome}). This is based on observed parental income from when the ABC/CARE participants where ages 0 to 21.

Alternative approaches considering the gain for the parents through age 65 generate an increase in the gain due to parental income (parental income columns). As we noted in Section~\ref{section:cbamethodology}, our estimates ignore any cohort effects. Individuals in ABC/CARE could experience positive cohort effects that might (i) make them more productive and therefore experience wage growth \citep{Lagakos_Moll_etal_2016_LifeCycle_NBER}; (ii) experience a negative shock such as an economic crisis and therefore experience a wage decline \citep{Jarosch_2016_JobSecurity_Econometrica}. Our estimates are robust when we vary annual growth and decay rates between $-.5\%$ to $.5\%$.
...
In Appendix~\ref{appendix:bcaestimates} we use our analysis to examine the empirical foundations of recent cost-benefit studies of early childhood programs that use short term estimates of experimental test score gains coupled with auxiliary estimates of the impact of test scores on earnings (see, e.g., work by Kline and Walters). We show that this approach applied to the ABC data greatly understates the true benefit-cost ratio because (a) earnings are only forecast through a young age (27) and (b) benefits extend beyond earnings. The difference is sizable. Applying the Kline and Walters method to the ABC/CARE experiment, we would find a cost-benefit ratio of 1.4 compared to our estimate of 5.7. 
