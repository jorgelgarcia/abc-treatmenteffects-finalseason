...
This paper documents the large array of life-cycle benefits of an influential early childhood program targeted to disadvantaged children. The program has substantial impacts on the lives of participants. Monetizing benefits and costs across multiple domains, we estimate a rate of return of 12\% per annum and a benefit/cost ratio of 5.7 with substantial differences across genders, favoring males.
...
Section~\ref{section:conclusion} summarizes the paper. There are substantial benefits that differ by gender. Girls benefit across more domains. However, the monetized value of the treatment effects is substantially greater for boys. This discrepancy is largely driven by labor income and health benefits, as well as reduced crime for males. Males benefit much less than females from being placed in lower quality alternative early childcare programs, compared to remaining with their mothers at home.
...
Our analysis is based on the first phase and pools the treatment group in CARE with the ABC treatment group. The second-phase treatment of ABC/CARE had very weak, if any, treatment effects (for evidence, see \citealp{Campbell_Conti_etal_2014_EarlyChildhoodInvestments} and \citealp{ABCCARE_Dataset}). \citet{Campbell_Conti_etal_2014_EarlyChildhoodInvestments} establish the validity of pooling the data on second phase treatments and controls with the first phase controls in ABC.

We do not use the data on the CARE treatment group that only received home visits. \citet{Campbell_Conti_etal_2014_EarlyChildhoodInvestments} and \citet{ABCCARE_Dataset} show that there is no statistically significant effect of the CARE family education component alone.
...
This subsection presents an informal summary of our approach. The rest of the section gives a formal justification as well as tests of key assumptions. Readers not interested in technical details may wish to skip to Section~\ref{section:cbaresults}, where we discuss our empirical results.

We have data on control- and treatment-group individuals through age $t^{\ast}$. We lack information on participant outcomes afterward. Yet, post-$t^{\ast}$ outcomes are required to construct counterfactual life-cycle profiles. We predict these outcomes using a three-step procedure.

First, in step 1, we use the experimental sample to conduct a dynamic mediation analysis relating the vector of outcomes at time $t$ ($\bm{Y}^{d}_{i,t}$) and predictor variables (and interactions) that are affected by treatment ($\bm{X}^{d}_{i,t}$), as well as background variables ($\bm{B}$). It turns out that we can predict within-sample treatment effects using this procedure. In step 2, we construct counterparts to treatment- and control-group members in the non-experimental samples. In step 3, we use the estimated dynamic relationships on the samples so constructed to predict the post-$t^{\ast}$ outcomes.

Figure~\ref{fig:labor-income-profiles} displays the life-cycle labor income profiles predicted for the treatment and control groups. It also compares the realized labor income to the model-predicted labor income at $t^*$. This pattern of life-cycle labor income is typical for low-skilled workers \citep{Blundell-etal_2015_J-Pub-E,Gladden_Taber_2000_WageProgression,Sanders-Taber_2012_AR,Lagakos_Moll_etal_2016_LifeCycle_NBER}.\footnote{The prediction of labor (transfer) income is based on indicator variables of being male and being black, mother's education, average PIAT scores from ages 5 to 7, years of education at age 30, body-mass index at 34, labor (transfer) income at ages 21 and 30, and one-period lagged labor (transfer) income. We use labor income at age 21 to initialize this process. The age-30 observation helps us validate our procedure. A justification for the use of these variables, evidence on their predictive power, and a sensitivity analysis to different prediction variables are in Appendix~\ref{appendix:methodology}.}
...
As previously noted, in the experimental sample, all of the parents of children with characteristics $\bm{B} \in \mathcal{B}_0$ agree to participate in the program.  Because the non-experimental sample has no treatment group members, one test of the validity of our prediction procedure is to compare the labor incomes of individuals in the non-experimental sample for whom $\bm{B} \in \mathcal{B}_0$ to the labor incomes of individuals in the synthetic control group constructed in our three-step procedure (i.e., the groups in the non-experimental samples used to extrapolate control group outcomes). We compare labor income at $t^*$ of both groups to the observed labor income at $t^*$ in the experimental control group. Figure~\ref{figure:controltests} makes this comparison. We plot the average labor incomes of individuals in our non-experimental sample for whom $\bm{B} \in \mathcal{B}_0$ with those of the constructed synthetic control group from ages 20 to 45. We also display the labor income of the experimental control group at $t^*$ (age 30).\footnote{The graphs stop at age 45 because we do not observe all of the components of the risk index determinants of eligibility after age 45 in the non-experimental samples. We use only a subset of this index to make life-cycle projections. These variables are effective predictors over the age range for which the full set of $\bm{B}$ is available.} The agreement is reassuringly close.
...
\subsection{Constructing Future Counterfactuals}\label{section:just}

We now present our analytical framework and its underlying assumptions. Omitting individual subscripts, outcome $j$ at time $t$ in the set of outcomes $\mathcal{J}_{t}$ is:
\begin{equation}\label{eq:outcome}
Y_{j,t}^d = \phi_{j,t}^d \left( \bm{X}_{t}^d, \bm{B} \right) + \varepsilon_{j,t}^d, \qquad j \in \mathcal{J}_t,
\end{equation}
where $d$ indicates that a person is fixed to control ($d = 0$) or treatment ($d = 1$) status. We interpret  $\phi_{j,t}^d \left( \cdot \right)$ as a production function mapping the inputs $\bm{X}_{t}^d$ together with background variables $\bm{B}$ to the output $Y_{j,t}^d$. Among the $\bm{X}^d_t$ are variables caused by treatment, including lagged dependent variables. We initially assume that $\varepsilon_{j,t}^d$ is exogenous. We discuss and test possible violations of this assumption below.
...
\begin{align}
Y_{j,t}     =& \left( 1 - D \right) Y_{j,t}^0    + \left( D \right) Y_{j,t}^1, \qquad j \in \mathcal{J}_t, \nonumber \\
\bm{X}_{t} =& \left( 1 - D \right) \bm{X}_{t}^0 + \left( D \right) \bm{X}_{t}^1. \label{eq:countersystem}
\end{align}
The fact that $D = R$ allows us to use experimental data to compute the distribution of $Y_{j,t}^d$ (i.e., $Y_{j,t}^d$ when fixing to treatment status ($d$)).\footnote{Randomization allows us to do this. \textit{Fixing and conditioning are fundamentally different concepts}. See \citet{Heckman_Pinto_2015_EconometTheory} for a discussion.}

We provide a general framework in which we can identify $\mathbb{E} \left[ Y_{j,t}^d | \bm{X}_{t}^d = \bm{x}_t^d, \bm{B} = \bm{b}  \right]$ in $t > t^*$ using non-experimental samples. We use two economically-motivated assumptions on the production function in \eqref{eq:outcome} and a set of auxiliary assumptions to construct out-of-sample predictions. This allows us to predict long-term, counterfactual treatment effects. Our framework has testable implications, which we implement below.

First, we assume that the variables $\bm{X}_{t}$ fully summarize treatment. That is, we assume that any effect that treatment has on an outcome operates through the inputs $\bm{X}_{t}$ and not through shifts in the production function relating inputs to outputs. Assumption~\ref{ass:summary} formalizes this.

\begin{assumption} \label{ass:summary} \textbf{Structural Invariance within Sample}\\
...
\end{align}
\noindent for $t \in \{ 1, \ldots, t^\ast \}$ and for any outcome $j \in \mathcal{J}_{t}$.
\end{assumption}
...
&=\mathbb{E} \left[ Y_{j,t} | \bm{X}^d_{t}  = \bm{x}^d_{t}, \bm{B} =  \bm{b} \right], \label{eq:invariancetest}
\end{align}
...
Equation~\eqref{eq:invariancetest} is a statement on how experimental treatment effects on $Y_{j,t}^d$ are affected by treatment effects on $\bm{X}_{t}^d$, together with the background variables $\bm{B}$. It is not a formula for predicting $Y_{j,t}^d$ out-of-sample. It is possible to test \ref{ass:summary} within the experimental sample at $t = t^*$. The test consists of asking if $\bm{X}_{t}^d, \bm{B}$ predicts the within experimental sample treatment effects for $Y_{j,t}$. Under the null that \ref{ass:summary} is correct, an indicator variable for treatment status is irrelevant when computing $\mathbb{E} \left[ Y_{j,t} | \bm{X}_{t}  = \bm{x}_{t}, \bm{B} =  \bm{b}, D = d \right]$. We test the null hypotheses implied by this assumption for the outcomes that we subsequently predict. We fail to reject the null hypothesis that the coefficient associated with random assignment to treatment is different from zero conditioning on $\bm{X}_{t}^d, \bm{B}$.\footnote{This holds both when pooling males and females and when testing separately by gender (see Appendix~\ref{app:invariance}).}

We next present conditions under which we can use relationships fit on non-experimental samples to provide valid forecasts of $Y_{j,t}^d$ beyond $t^\ast$. Write outcome $j$ at time $t$ in the set of outcomes $\mathcal{J}_{t}$ in the non-experimental sample as:
\begin{equation}\label{eq:outcomenonexp}
Y^d_{j,t} = \widetilde{\phi}_{j,t} \left( \bm{X}^d_{t}, \bm{B} \right) + \varepsilon_{j,t}, \quad d \in \{0,1\}, \quad j \in \mathcal{J}_t, \quad t>t^\ast
\end{equation}
and make an assumption analogous to Assumption~\ref{ass:exog} for the non-experimental sample:

\begin{assumption}\label{ass:exogne} \textbf{Exogeneity in the Non-experimental Sample}\\
For all $t, t'' \in \{ 1, \ldots, T \}$,
\begin{equation}
\varepsilon_{j,t} \indep \bm{X}^d_{t^{''}}, \quad d \in \{0,1\}, \quad j \in \mathcal{J}_t, \quad t>t^\ast.
\end{equation}
\end{assumption}

Under Assumptions~\ref{ass:exog} and \ref{ass:exogne}, a prediction of $Y^d_{j,t}$ for the experimental sample based on \eqref{eq:outcomenonexp} is valid under a structural invariance assumption. That is, \eqref{eq:outcomenonexp} is valid under the assumption that the dynamic economic relationships in \eqref{eq:outcome} and \eqref{eq:outcomenonexp} are not affected by cohort effects or any other potential shifters of the economic relationships between $\left( \bm{X}_{t}, \bm{B} \right)$ and $Y_{t,j}$ not captured by $\bm{X}^d_t, \bm{B}$. Formally,

\onehalfspacing
\begin{assumption}\label{ass:invariance} \textbf{Structural Invariance} \\
\noindent For any  $\left( \bm{X}^d_{t}, \bm{B} \right)$,
\begin{equation}
\phi_{j,t} \left( \bm{X}^d_{t}, \bm{B} \right) = \widetilde{\phi}_{j,t} \left( \bm{X}^d_{t}, \bm{B} \right), \quad d \in \{0,1\}
\end{equation}
for $t \in \{ t^\ast +1, \ldots, T \}$ and for any outcome $j \in \mathcal{J}_{t}$.\footnote{Assumptions~\ref{ass:summary} and \ref{ass:invariance} appeal to a fundamental property of invariant economic relationships used by economists since the time of \citet{Frisch_1938_autonomy}. Frisch called this notion ``autonomy.'' Much later work in statistics captures structural invariance as one aspect of ``SUTVA.'' See \citet{Holland_1986_JASA} and \citet{Heckman_2008_ISR}.}
\end{assumption}
\doublespacing
...
Theorem~\ref{theorem:main} formalizes our conditions for valid out-of-sample predictions.

\onehalfspacing
\begin{theorem}\label{theorem:main} \textbf{Valid Out-of-Sample Predictions} \\
Let Assumptions~\ref{ass:exog},~\ref{ass:summary}, \ref{ass:exogne} hold and let Assumption~\ref{ass:invariance} hold for the experimental and the synthetic control and treatment groups constructed from the non-experimental sample constructed using our analysis. Then, for any value of $\left( \bm{X}_{t}, \bm{B} \right)$, $\mathbb{E} \left[ Y_{j,t} | \bm{X}^d_{t} = \bm{x}^d_{t}, \bm{B} = \bm{b}, S = 0 \right]$ is a consistent out-of-sample prediction of $Y_{j,t}^d$ for $d \in \{ 0, 1\}$, for all $j \in \mathcal{J}_{t}$ and for any $t \geq t^* \in \{t^*, \ldots, T \}$. $\Box$
\end{theorem}
...
To summarize, Theorem~\ref{theorem:main} states that when the dynamic relationship between $Y_{j,t}$ and $\left( \bm{X}_{t}, \bm{B} \right)$ satisfies invariance properties in the experimental samples and in the comparison group samples, and exogeneity and common support conditions are satisfied, we obtain consistent out-of-sample predictions.\footnote{Adding our method to account for attrition to the theorem is straightforward. See Appendix~\ref{appendix:attrition}.}

\subsubsection{Accounting for Endogeneity}\label{section:accendog}

We effectively match individuals in the control and treatment groups to individuals in the non-experimental samples based on \textit{observed characteristics}. In general, the dependence between $\bm{X}_{t}$ and $\bm{\varepsilon}_{t}$ is not the same across those two samples. Assumptions~\ref{ass:exog} and ~\ref{ass:exogne} could be violated in the experimental sample, the non-experimental sample, or both samples. These violations could occur, for example, when there are unmeasured inputs generated by the experiment.

Unless the biases in the predictions for both the treatment and control groups are exactly the same, the predicted treatment effect resulting from our prediction procedure is not consistent. If prediction equations are separable in unobservables, a sufficient condition to obtain unbiased predictions is to assume that $\mathbb{E} \left[ \varepsilon_{j,t}^0 | \bm{X}_{t}^d, \bm{B} \right] = \mathbb{E} \left[ \varepsilon_{j,t}^1 | \bm{X}_{t}^d, \bm{B} \right]$.

To investigate the endogeneity issue in the experimental and comparison samples in Appendix~\ref{app:endogeneity}, we follow \citet{Heckman_Pinto_etal_2013_PerryFactor} and assume that $\varepsilon_{j,t}^d$ obeys a factor structure. We provide evidence supporting Assumptions~\ref{ass:summary}, \ref{ass:invariance}, and \ref{ass:contain} in Appendix~\ref{appendix:methodology}. Exogeneity Assumptions~\ref{ass:exog} and \ref{ass:exogne} could easily be violated, especially when we include lagged dependent variables among the predictors. There, we motivate these concerns and provide tests for these two assumptions.

\subsection{Predicting Parental Income} \label{section:pincome}

ABC/CARE offers childcare to the parents of treated children for more than nine hours a day for five years, 50 weeks a year. Recall that only $27\%$ of mothers of children reported living with a partner at baseline. This status barely changed during the experiment (See Appendix~\ref{appendix:background}). Arguably, this is the component that generates the treatment effects in maternal education, labor force participation, and parental income reported in Table~\ref{table:tescombined} and Appendix~\ref{appendix:results}.
...
\subsection{Crime\footnote{The input of Andr\'{e}s Hojman was critical for this component of our analysis.}}\label{sec:crime}
...
\textbf{[JJH: I stopped here for today]}
...
{\normalsize \textbf{[Typist/Jorge: The order ``pooled, males, females'' has now been made official throughout the document.]}}
\end{tablenotes}
\end{threeparttable}
\end{adjustbox}
\end{table}
