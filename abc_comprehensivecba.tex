...
It is also fruitful to ask: what is the effectiveness of the program with respect to a counterfactual world in which the child stays at home full time? The associated causal parameter is:
...
\begin{tablenotes}
\tiny
\item \textbf{Treatment effects estimated}\\
\leftright{\textbf{(1)} $\mathbb{E} \big[ Y^1 - Y^0 | D = 1, \bm{B} \in \mathcal{B}_0\big]$}{\textbf{(4)} $\mathbb{E} \big[ Y^1(v) - Y^0 | X, V=0, D = 1 \big] $} \par
\leftright{\textbf{(2)} $\mathbb{E} \big[ Y^1 - Y^0 |  X, D = 1 \big]$}{\textbf{(5)} $\mathbb{E} \big[\big[ Y^1 \big] - \mathbb{E} \big[ Y^0 \big]| X, V=1, D =1 \big]$} \par
\leftright{\textbf{(3)} $\mathbb{E} \big[\big[ Y^1 \big] - \mathbb{E} \big[ Y^0 \big]| X, V =0, D =1 \big]$}{\textbf{(6)} $\mathbb{E} \big[ Y^1 - Y^0(v) | X, V=1 , D = 1\big]$}
\end{tablenotes}
\end{threeparttable}
\end{table}
\restoregeometry
...
Figure~\ref{fig:control-sub} shows the quality of the interpolation and extrapolation of labor income by comparing the forecasts in the ABC/CARE data with the life-cycle profile of disadvantaged individuals in the PSID.\footnote{Our pattern of life-cycle labor income is typical for low-skilled workers. See, e.g., \cite{Blundell-etal_2015_J-Pub-E}.} \textbf{[JJH: How are these formed? What are the predictors?]}
...
\begin{sidewaysfigure}[!htbp]
\centering
\caption{Quality Adjusted Life Years}\label{fig:qalys}
\begin{subfigure}[h]{0.4\textwidth}
		\centering
