...
\noindent \textbf{[MT: I would not put this in the introduction. Presenting treatment effects in the introduction before discussing methodology of how the effects were constructed is not ok. I would move the graphs to the results and simply mention in the introduction that when testing counts of socially beneficial effects we find positive outcomes.]} To illustrate the suitability of our data for assessing the effectiveness of early childhood education, consider the following exercise. Construct 141 measures of human development across the life-cycle---e.g., cognition at age 5, high school graduation and labor income at age 30, hypertension at age 34. Then, count the number of measures for which the program had a ``socially beneficial'' effect. For both ABC and CARE, we do this by focusing on the first phase of treatment to compare children who received center-based childcare to children who did not---noting that assignment was random.\footnote{In ABC, this implies comparing the children who were randomly assigned to the treatment group to the children who were randomly assigned to the control group, in the first phase. In CARE, this implies comparing the children who were randomly assigned to receive center-based childcare and home visits to the children who were randomly assigned either to the group receiving home visits only or to the control group, in the first phase as well.} Figure~\ref{fig:ppositive} displays the results from this exercise: ABC and CARE positively impact a noticeable majority of this variety of outcomes \textbf{[MT: what does noticeable majority mean.]}. In fact, throughout the paper, we formalize the count in this exercise as a statistic to summarize the effectiveness of ABC and CARE, while accounting for methodological challenges inherent to the evaluation of social programs.
...
\noindent The first phase of randomization was conducted at the family level, so pairs of siblings and twins were jointly randomized to either treatment or control groups.\footnote{Sibling pairs occurred when the two siblings were close enough in age such that both of them were eligible for the program.} Although we know that pairing \textbf{[MT: matching (?)]} was based on HRI, maternal characteristics, number of siblings, and gender, we do not know the original pairs. The study collected an initial sample of 120 families. Twenty-two children did not complete the first-phase of treatment as initially assigned by the randomization (see Table~\ref{table:abccompromises}). We characterize each case and explain how to correct for them in Section~\ref{section:methodology}.\\

\textbf{[MT: I recommend just including the table and deleting the below discussion, which doesn't provide additional information. Provide a paragraph on the sensitivity analysis nonetheless. Also if the discussion is deleted provide a column in the table with how we use data from the case.]}
...
\textbf{[MT: Removing the extensive re-discussion of the table in ABC will make this section not as noticeably off balanced --- lol. Maybe provide a discussion on the assumptions you are making in treating the attrition cases in CARE as they are treated in ABC.]}
...
\textbf{[MT: Table 4 feedback: Add an additional column before ABC with the check marks. Include just the raw information under CARE (not the checkmarks).]}
...
\textbf{[MT: this table is still really small. You could use longtable to break up into 2 pages.]}
...
\noindent In our main results, we combine data from ABC and CARE to test the hypothesis in \eqref{eq:ho}. To do so, we compare children who were randomly assigned to treatment in ABC and to the highest level of care in CARE with children who were not assigned center-based childcare in either program.\footnote{In Appendix~\ref{appendix:results} we show that (i) the family education treatment had no substantial effect compared to the control group, (ii) comparing the groups receiving center-based childcare to the control groups of ABC and CARE independently provide results very similar to those we discuss here.} We pool the sample in testing our hypotheses of interest in order to evaluate center-based childcare as a policy, rather than as it was implemented in a specific program. Appendix~\ref{appendix:results} displays further results not presented here. \\

\textbf{[MT: You should include observations in the table.]}
\textbf{[MT: Confusing: it seems at first that you are only presenting results for females. Give an outline of the results section at the beginning to give readers a roadmap.]} \\

\noindent We present results first for females in Tables~\ref{tab:ate_female_main1} and \ref{tab:ate_female_main2}. \textbf{[MT: What is motivating you to subset male vs. female? Maybe offer why policymakers would be interested in splitting the sample. This could raise post-hoc analysis/multiple testing concerns that aren't addressed.]} Each outcomes generates eight estimates. To illustrate how we interpret them, consider high school graduation at age 30. Column~(1) displays the mean difference in graduation between children assigned and not assigned to receive center-based childcare. This difference amounts to 20 percentage points. In column~(2) we present this difference controlling for a set of variables and accounting for program attrition.\footnote{See Appendix~\ref{appendix:controls} for our procedure for selecting controls.} In this case, the estimate remains practically unchanged. This is generally the case for all the estimates we consider, so we will restrict our discussion to estimates in which we include controls and account for program attrition.\\
...
\noindent The results in Table~\ref{tab:counts_female} are compelling. Our count statistic shows that more than 70\% of all treatment effect estimates we present are positive \textbf{[MT: In what sample?]}. We contrast three hypotheses using this count. We test the null-hypothesis of at most 25\%, 50\%, and 75\% of outcomes having a positive treatment effect, without regards to statistical significance. We easily reject the first two hypotheses. Second, at least 20\% of these effects are significant at a 10\% level. \textbf{[MT: Any inference on this? Or just presenting a count?]} Note that at a 10\% significance level, 10\% of the effects are expected to be significant by chance.
