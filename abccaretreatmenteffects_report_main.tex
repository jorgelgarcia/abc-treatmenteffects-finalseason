...
\noindent To see this, let $Y_r$ denote a counter-factual outcomes and $A$ denote compliance. Then, $Y_{r} \left(\text{fix } A = a \right)$ represents the counter-factual outcome $Y$ when $A$ is fixed to compliance ($A = 0$) or non-compliance ($A = 1$). It is not necessarily the case that 

\begin{equation}
Y_{r} \left(\text{fix } A = 0 \right) \sim Y_{r} \left(\text{fix } A = 1 \right). 
\end{equation}

\begin{assumption} \normalfont \label{assumption:balance} (Conditional Independence in Non-compliance) The counter-factual outcome $Y_{r} \left(\text{fix } A = a \right)$ follows the same distribution regardless of the value on which $A$ is fixed, after condition on $X$ (observed characteristics): 
...
\noindent For illustration, suppose we want to estimate the $\text{ITT} \left( \text{fix } P = 0 \right)$ using a linear and separable parameterization. Then, estimate the following model 
...
\noindent An alternative to evaluate the first-phase of ABC and CARE is to combine the data from both programs and test the effect of the common elements of their treatment. The treatment group of ABC and the center-based childcare and family education treatment group of CARE were subject to the same center-based childcare treatment component. For the ABC children, this was actually the only component. For the CARE children, they had family education in addition.\\

\noindent A simple alternative to combine the data is the following. Suppose that we fail to reject $H_{0}^D$ and we reject $H_{0}^F$. That is, (i) we cannot reject the null hypotheses of no treatment effect of the family education treatment in CARE; and (ii) we can reject the null hypothesis of treatment effect of center-based childcare and family education treatment in CARE. Together, this evidence favors assuming that the center-based childcare was the main component generating treatment effects in CARE, if any. If this holds, we can consider the children fixed to control in ABC and CARE and the children fixed to treatment in ABC and to center-based childcare and family education treatment in CARE to test the hypothesis in \eqref{eq:ho}. That is, to test the effect of center-based childcare. Moreover, we can consider fixing the children in the treatment groups to either no alternative preschool or to alternative preschool. We can proceed as in Section~\ref{section:methodsquestions} to construct the estimates and the corrections for non-compliance, which require the assumptions we state there.\\

\noindent We can also parameterize the effect of of the family education component. To illustrate this, consider a linear and separable parameterization to estimate $\text{ITT} \left( \text{fix} P = 0\right)$, the effect of center-based childcare, combining the data of ABC and CARE. As in Section~\ref{section:methodsquestions}, $R$ denotes being assigned to a treatment group with center-based childcare---i.e., to treatment in ABC or to center-based childcare and family education in CARE. Similarly, $H$ indicates being assigned to family education treatment, i.e. to family education treatment in CARE. Then we write, 

\begin{equation}
Y = \Delta \left( \text{fix } P = 0\right) R +  \Gamma \left( \text{fix } P = 0\right) H + \upsilon. \label{eq:fixfam}
\end{equation}

\noindent In this case, $\text{ITT} \left( \text{fix } P = 0\right)$ is the effect of center-based childcare when fixing the preschool to alternative (to no alternative at all) and family education treatment---randomization to family treatment education allows us to fix family education without needing assumptions similar to Assumption~\ref{assumption:matching}.\\

\noindent We report results for the two methods we propose to combine the information in ABC and CARE, as we explain in this section.

\subsection{Second-phase Treatment}

\noindent ABC and CARE had the same second phase of treatment. In ABC, children were randomized to this second phase, which enables to test series of hypotheses related to its efficacy. To clarify this, we consider an additional dimension in the sub-index of the counter-factual outcome notation. Let $S$ indicate random assignment to second-phase treatment and Let $Y_{r,s}$ denote the outcome when first-phase treatment is fixed to first-phase treatment $r$ and second-phase treatment $s$.\\
...
Fixing to control       & $H_{0}^G: Y_{0,0} = Y_{0,1}$ & $H_{0}^H: Y_{0,0} = Y_{1,0}$ \\
Fixing to treatment  & $H_{0}^I: Y_{1,0} = Y_{1,1}$ & $H_{0}^J: Y_{0,1} = Y_{1,1}$ \\ \\ \hline
No fixing                 & $H_{0}^A: Y_{d,0} = Y_{d,1}$ & $H_{0}^K: Y_{0,s} = Y_{1,s}$ \\  \hline \hline
\end{tabular}
\begin{tablenotes}
\footnotesize
...
\noindent Table~\ref{table:hypotheses} enumerates the different hypotheses we are able to test. To illustrate how we construct the tests, consider the following hypothesis: 

\begin{equation}
H_{0}^G: Y_{0,0} \sim Y_{0,1}.  \label{eq:h0fixfirst}
\end{equation}

\noindent That is, test the effect of second-phase treatment, fixing first-phase treatment to control status. Arguing as in Section~\ref{section:methodsquestions}, we can construct an ITT to test this hypothesis: 

\begin{eqnarray}
\text{ITT} = \mathbb{E} \left[ Y | R = 0, S = 1 \right] - \mathbb{E} \left[ Y | R = 0, S = 0 \right]. 
\end{eqnarray}

\noindent Perfect compliance to treatment allows us to identify the expectation of the outcome when fixing first-phase treatment to control status and the second-phase treatment to treatment status, $\mathbb{E} \left[ Y_{0,1} \right]$. by with the empirical counter-part of $\mathbb{E} \left[ Y | R = 0, S = 1 \right]$. Similarly, we are able to identify $\mathbb{E} \left[ Y_{0,0} \right]$ through $\mathbb{E} \left[ Y | R = 0, S = 0 \right]$. Under imperfect compliance we can use the weighting method in Section~\ref{section:methodsquestions} to identify and estimate these expectations. The rest of tests in which the first-phase treatment is fixed proceed similarly.\\

\noindent Alternatively, it is also of interest to test if the second-phase treatment had an effect, independently of the first-phase assignment. That is, test 

\begin{equation}
H_{0}^K: Y_{d,1} \sim Y_{d,0}, \label{eq:h02}
\end{equation}

\noindent where $d \in \{0,1\}$. The hypothesis in \eqref{eq:h02} is the second-phase counter-part to the the hypotheses in \eqref{eq:ho} and relates to a similar question: how effective is the second-phase of ABC, relative to no second-phase treatment? It does not consider dynamics between the first and second phases of treatment, while hypothesis fixing the first-phase treatment do. 
...
\noindent The methodology that we discuss so far (i) clarifies the hypothesis we test; (ii) offers adjustments to non-compliance; (iii) clarifies the policy questions that program evaluation parameters are able to answer; (iv) offers alternative parameters that relate to policy-relevant questions in the presence of treatment substitution; (v) states how to tests hypotheses in the context of multiple treatment branches and treatment phases.\\

\noindent Both ABC and CARE have a unique feature we exploit next: multiple measures helping assess how the programs foster human development in several dimensions over the life cycle. While our discussion so far states how to test the effectiveness of the programs at boosting a single outcome, human development is multidimensional, so we want to tests the effect of the program in several dimensions.\\

\noindent In our companion paper \citep{Elango_et_al_2015_ABC_unpublished}, we offer one possibility for measuring the effectiveness of a program through to two statistics, the benefit-to-cost ratio and the internal rate of return. To do that, we account for the life-cycle gains of the program in several dimensions, e.g. employment, income, government transfers, health, crime. A simpler, yet informative alternative is to count the number of outcomes that the program has a socially positive effect on and present an inference method on it.\\

\noindent A counting procedure is inevitably subjective. For each outcome, we need to take a stand to classify it as socially positive or as socially negative. To gain objectivity, we present the count on 547, all of which we think are either socially positive or socially negative non-controversially. We present an extensive list of the outcomes and their classification as socially positive or socially negative in Appendix~\ref{appendix:data}. We offer some relevant examples in Section~\ref{section:data}.\\

\noindent The different counts we propose are forms of combining functions \citep{Pesarin_Salmaso_2010_PermutationTests}.\footnote{This reference offers a comprehensive treatment of combining functions.} These are functions that mean to combine information on several outcomes when testing patterns in a scientific or social phenomenon, in our case an early childhood education program. To fix ideas, suppose we want to test the hypothesis 

\begin{equation}
H_{0}^A: Y_{0} \sim Y_{1} \label{eq:hoagain}
\end{equation}

\noindent for multiple outcomes $Y$ and that for each outcome, individually, we already have an inference procedure readily available, i.e. we know the hypothesis, the statistics we use to test, and the $p$-value associated to the test. For example, we want to test \eqref{eq:hoagain} using the ITT and we use a bootstrapped, non-parametric $p$-value.\\

\noindent We consider three counts. First, a sum of the socially positive treatment effects. We can calculate this sum over the complete set of outcomes or certain categories of outcomes (e.g. employment, health, crime) and do it regardless of their significance when individually contrasted to test \eqref{eq:hoagain}. Second, we use sum the outcomes that are significantly socially positive at the $10\%$ level, when hypothesis \eqref{eq:hoagain} is individually tested. Third, we use the same sum but we consider a $5\%$ significance level.\\

\noindent Consider constructing a combining function, in our case a count, for a group or block of outcomes $\mathcal{O}^g$, where $g \in \mathcal{G}$ and $\mathcal{G}$ is the index set for the groups of outcomes. Alternatively, we can construct the count for $\mathcal{O} : =  \bigcup \limits _{g \in \mathcal{G}} \mathcal{O}^g$. Each group includes the number of outcomes $\# \mathcal{O}^g$, and an associated sequence of treatment effects $\{ \widehat{\Delta}_{j}^{g} \}_{j = 1}^{\# \mathcal{O}^g}$. The test statistic for group $g$ is the count. For example, if we count the number of socially positive outcomes, the test statistics is for group $g$ is 
