...
# Trimming
inc = psid.filter(regex='^inc_trans_pub[0-9][0-9]')
...
pwide.columns = ['{}{}'.format('inc_trans_pub', a) for a in pwide.columns]
...
# Trimming
inc = nlsy.filter(regex='^inc_trans_pub[0-9][0-9]')
...
nwide.columns = ['{}{}'.format('inc_trans_pub', a) for a in nwide.columns]
...
datasets = [
    ('psid-transfer', psid), 
    ('nlsy-transfer', nlsy),
    ('cnlsy-transfer', cnlsy),
    ('extrap-transfer', extrap), 
    ('interp-transfer', interp),
    ('abc-mini', abcd)
    ]
