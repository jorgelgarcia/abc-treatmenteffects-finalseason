...
\noindent \textbf{[JJH: Please separate (1) invariance from (2) tests of treatment effects] [JLG: What Equation~\eqref{eq:linearexp} tests is if, once intermediate inputs are accounted for, $R$ (randomization into treatment) is still a significant determinant of the outcome of interest. If not, then the intermediate inputs in fact mediate treatment. An analogous test is presented for invariance across samples.][JJH: What analogous test?][JLG: The test asking whether, once inputs are accounted for, $K$ (experimental/non-experimental sample membership) is till a significant determinant of the outcome of interest. If not, the the second aspect of invariance is satisfied.]}\\
...
Panel (a) of Table~\ref{table:invariance} displays estimates of the coefficients characterizing Equation~\eqref{eq:linearexp} for labor income at age 30 by gender. For both females and males the tests are precise.\footnote{Note that after accounting for background variables and the intermediate inputs, average labor income is \$2,213 (2014 USD) higher in the treatment group for females. This value is relatively small in the context of annual labor income at age 30 and given that the average in the control group is \$23,443 (2014 USD). The same holds for the males, where the treatment-control difference is \$ 232 net of inputs is and the average for the control group is  \$29,340 (2014 USD).} We do not reject the null hypothesis that the technology is invariant across treatment regimes. Panel (b) of Table~\ref{table:invariance} reports estimates for the rest of coefficients in Equation~\eqref{eq:linearexp}. Years of education is an especially important input in our forecast. It is a statistically and economically significant determinant of labor income and it is experimentally boosted by ABC/CARE (see Table~\ref{table:summint}). \textbf{[JJH: What are the results by gender?] [JLG: I dropped employment because you are right, we need to be concise. The results are now by gender.] [JJH: Residuals?] [JLG: That comes in the next table, Table~\ref{table:invarianceerrors}.]}
...
\normalsize
\item \textbf{[JJH: What if we test by gender, do we fail?] [JLG: Everything is by gender now.]}
\end{tablenotes}
\end{threeparttable}
\end{table}

To test Condition~\eqref{eq:suff2}, note that the coefficients associated with $K$ (an indicator of membership in the experimental ($K = 1$) or non-experimental sample ($K = 0$)), denoted by $\Lambda$, is zero, assuming the following baseline linear technology: 
...
Panel (c) in Table~\ref{table:invariance} displays estimates of the coefficients characterizing Equation~\eqref{eq:linearnoexp} for labor income at age 30 for males and females. \textbf{[JJH: Suddenly you dissagregate by gender - Do this for Panel (a) + (b), please.] [JLG: It is there now.]} The estimates of $\Lambda$ are small in magnitude and not statistically significant (the averages of labor income in the experimental and non-experimental sample for females and males are $\$24,584$ and $\$40,007$, and  $\$24,098$ and $\$32,717$ (2014 USD), respectively). We do not reject that the technologies are invariant across samples.\\

\noindent \textbf{[JJH: Suddenly, employment dropped. Jorge, Do you realize that skipping around in this disjoined way turns OFF readers aim for consistency.] [JLG: Ok. All results now are based on labor income at age 30 by gender.]}\\
...
\noindent \textbf{[JJH: Please make format of Tables compatible.] [JLG: This table is now compatible with Table 2 and reports by gender.]}\\

We use the analogous steps to test Condition~\eqref{eq:crucial2}. First, we test invariance in the distributions of $\bm{\varepsilon}_{k,a}^d$ across treatment regimes within the experimental sample for labor income and employment at age 30. Then, invoking invariance across treatment regimes, we test invariance across the experimental and non-experimental residual distributions. To obtain the residuals, we our forecasting model in Equation~\ref{eq:outcome} assuming a linear technology. \textbf{[JJH: Why not test residuals with $R$ + $K$?] [JLG: (a) because our forecasting model in Equation~\eqref{eq:outcome} does not have $R$ + $K$ and we need the empirical counterpart to our model; (b) we just failed to reject the null that $R$ and $K$ belong in our model.]} Before testing, we adjust the residuals for model estimation error as explained in step 6 of Appendix~\ref{appendix:bootstrapspreds}.

With the empirical counterparts of $\bm{\varepsilon}_{k,a}^d$ in hand, we implement two tests to compare the distributions in Table~\ref{table:invarianceerrors}: mean comparisons through $t$ test and Kolgomorov-Smirnov test. Within the experiment and at age 30, we do not to reject a common distribution of $\bm{\varepsilon}_{k,a}^d$ for both females and males---Panel (a). As before, under this null hypothesis we pool the experimental treatment and control groups and the synthetic treatment and control groups to test invariance across samples by gender. In all but one of our tests we do not reject invariance across samples---Panel (b).
