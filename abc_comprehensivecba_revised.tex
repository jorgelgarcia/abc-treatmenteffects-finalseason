...
\noindent \textbf{[JJH: Typists print the new appendix please.]} \\
\noindent \textbf{[The new appendix is printed.]} \\

\noindent \textbf{[JJH: Paper collapses in Table 2.] \\ 
\noindent [JLG: I redrafted Section 3. You requested separating ``treatment effects'' from ``invariance'' in your comments to the memo. That is being done now. The tests are clarified and details on each number presented in the Tables are discussed. I am also including answers to your questions in the memo into this directly.]}\\ 

\noindent \textbf{[JJH: Jennifer - this is still too long - correct?] \\ 
\noindent [Jennifer: The published paper should be no more than 35 pages, inclusive of text, print appendixes, references, tables, and figures. That works out to about 42 pages in the current format (12pt font, 1'' margins, double-spaced). The official due date at JPE is July 22.]\\ 
\noindent [JJH: Who said July 22 is deadline?]\\ 
\noindent [Jennifer: July 22 was the automatic deadline that the JPE system generated (the default 180 days for revisions). The stated policy is that revisions should be returned ``less than a year'' after the decision was sent out (your decision was sent out on January 23, 2018). Please let me know if you would like an extension.]}\\ 
\noindent \textbf{[JJH: Yes we want an extension to January 23, 2019.]} \\

\noindent \textbf{[JLG: Professor, I iterated with Jennifer and she advised that the paper might still be too long by a few pages. Two options are: Displaying Table 3 in the Appendix (which, as Jennifer advised, Uhlig recommends putting in the Appendix) This saves two pages in print and sending Section 6 to the Appendix. For this paper, we might want to stay with treatment vs. next best? We briefly discussed this a month ago or so. The more specific counterfactuals, for the purpose of this paper, may be Appendix material. This saves 5 or 6 pages.] \\
\noindent [JJH: I agree. Give me two days. Today, I am beat.]\\
\noindent [JLG: I implemented that -sent Table 3 to Appendix  and sent Section 6 to appendix. If you disagree we can bring it back.]}
...
\noindent for $d \in\{0,1\},  k\in\{e,n\}, \  a\in\{1,\dots,\bar{A}\}$, where $\bm{\varepsilon}_{k,a}^d = D\bm{\varepsilon}_{k,a}^1 + (1-D)\bm{\varepsilon}_{k,a}^0$. These conditions are testable in the overlap of the experimental and the non-experimental samples.
...
Under Assumption~\ref{ass:exog}, we build on \citet{Heckman_Pinto_etal_2013_PerryFactor} and assume that the variables $\bm{X}_{k,a}^d$ fully summarize treatment in the sense that any effect that treatment has on outcomes operates through the inputs, $\bm{X}_{k,a}^d$, and not through shifts in the production function relating inputs to outputs. Assumption~\ref{ass:summary} formalizes this condition.
...
Assumption~\ref{ass:summary} has two distinct aspects that could be resolved further into two separate assumptions: can (i) structural relationships evaluated at the same arguments have identical values for treatment and control groups in the experimental sample, and can (ii) analogous conditions hold in both the experimental and non-experimental samples. Condition (ii) enables us to simulate treatment and control outcomes in non-experimental samples.
\end{remark}
...
\footnotetext{If the only goal is to forecast treatment effects, the minimal requirement is that experimental treatment effects should equal differences in the conditional means of the non-experimental samples evaluated at $\bm{X}_{n,a} = \bm{x}^1$ and  $\bm{X}_{n,a} = \bm{x}^0$:
\renewcommand{\theequation}{6c}
...
Note Equations~\eqref{eq:suff1} and~\eqref{eq:suff2} are sufficient conditions for Equation~\eqref{eq:nec} to hold. \textbf{[JJH: Maybe we should invoke this and be done.][JLG: We did so in the previous version, and asked the reader to see Appendix C for the tests. But one referee, and I think that I agree with him/her says that the evidence in favor of invariance (and a lot of the stuff that we use to have in the Appendix) is too crucial to be tabled.]}}

\noindent \textbf{[JJH: Please separate (1) invariance from (2) tests of treatment effects] [JLG: What Equation~\eqref{eq:linearexp} tests is if, once intermediate inputs are accounted for, $R$ (randomization into treatment) is still a significant determinant of the outcome of interest. If not, then the intermediate inputs in fact mediate treatment. An analogous test is presented for invariance across samples.][JJH: What analogous test?][JLG: Yes. The tests comes below.]}\\

We test these conditions at age 30, where we observe labor income in the experimental sample for both the treatment and the control groups. If Condition~\eqref{eq:suff1} holds, the coefficient associated with $R$ (randomization to treatment  in ABC/CARE, which, for our sample, is equivalent to $D$), denoted by $\Delta$, should be zero when assuming linearity. We estimate the following baseline technology: 
\setcounter{equation}{6}
\begin{subequations}
\begin{equation}
Y_{e,30}^d = \Delta \cdot R +  \bm{B}_{e,30} \cdot \bm{\gamma}_{e,30}^d +\bm{X}_{e,30}^d \cdot \bm{\beta}_{e,30}^d + \varepsilon_{e,a}^d, \label{eq:linearexp} 
\end{equation}

\noindent where $Y_{e,30}^d$ and $\varepsilon_{e,a}^d$ are scalars and $\bm{\gamma}_{e,30}^d$ and $\bm{\beta}_{e,30}^d$ are vectors of coefficients. Failing to reject the null hypothesis $H_{0}: \Delta = 0$ is equivalent to failing to reject invariance across treatment regimes.\\


Panel (a) of Table~\ref{table:invariance} displays estimates of the coefficients characterizing Equation~\eqref{eq:linearexp} for labor income at age 30 pooling males and females to increase precision. To enhance the evidence we also display analogous estimates for employment at age 30, an outcome related to labor income. We fail to reject the null hypothesis that $H_{0}: \Delta = 0$. \textbf{[JJH: I thought that we rejected non-invariance!] [JLG: No. We fail to reject invariance. That has always been the test we implement because the way in which our implications are formulated in 5a and 5b. We operate under the null of those equalities.]} In both cases, the tests are precise, noting that after accounting for background variables and the intermediate inputs, average labor income is $\$786.262$ (2014 USD) higher in the treatment group. This value is small in the context of annual labor income at age 30 and given that the average in the control group is $\$26,083$ (2014 USD). We conclude that the technology is invariant across treatment regimes. The rest of the components in Panel (a) indicate that the outcomes are non-trivial functions of the outputs of interest (which is verified in step 2). Years of education is an especially important input in our forecast. It is a statistically and economically significant determinant of labor income and it is experimentally boosted by ABC/CARE (see Table~\ref{table:summint}).

\begin{table}[!htpb]
\begin{threeparttable}
\caption{Invariance in Technologies $ \left( \bm{\phi}_{k,a}^d \left( \bm{x}, \bm{b} \right) \right)$ at Age 30} \label{table:invariance}
\centering
\footnotesize
\begin{tabularx}{16.5cm}{XcX}
& \begin{tabular}{lcccc} \toprule
 \multicolumn{5}{l}{\textbf{Panel (a). Invariance Across Treatment Regimes}} \\
& \multicolumn{2}{c}{Employment} &   \multicolumn{2}{c}{Labor Income} \\ 
    			      & coefficient & $p$-value & coefficient & $p$-value \\ \\
 $R$ (treatment indicator) &0.000 & 0.999 & 821.087 & 0.926 \\ \\
...
\multicolumn{5}{l}{Sample: Experimental Treatment and Control Groups at Age 30} \\ \midrule
 \multicolumn{5}{l}{\textbf{Panel (b). Invariance Across Experimental and Non-Experimental Samples}} \\ 
 & \multicolumn{2}{c}{Female Labor Income} &   \multicolumn{2}{c}{Male Labor Income} \\
       			      & coefficient & $p$-value & coefficient & $p$-value \\ \\
...
\multicolumn{5}{l}{Sample: Experimental Treatment and Control Groups and Non-Experimental Synthetic }\\
\multicolumn{5}{l}{Control and Treatment Groups at Age 30} \\ \bottomrule
\end{tabular} & 
\end{tabularx}
\begin{tablenotes}
\footnotesize
\item Note for Panel (a): Estimates of the coefficients in Equation~\eqref{eq:linearexp} for employment and labor income at age 30 and outcomes pooling males and females. $R$ denotes the treatment indicator ($R = 0$ for control-group individuals and $R = 1$ for treatment-group individuals). $\bm{B}_{e,30}$ is comprised of age-30 baseline variables not affected by treatment (mother's education at birth) and $\bm{X}_{e,30}^d$ is age-30 intermediate inputs.\\ 
\item Note for Panel (b): Estimates of the coefficients in Equation~\eqref{eq:linearnoexp} for labor income at age 30 by gender pooling the experimental treatment and control groups and the synthetic cohort at age 30. $K$ membership to the experimental or non-experimental sample ($K = 0$ synthetic cohort in the non-experimental sample and $K = 1$ experimental sample). $\bm{B}_{k,30}$ is comprised of age-30 baseline variables not affected by treatment (mother's education at birth) and $\bm{X}_{k,30}^d$ is age-30 intermediate inputs.\\ 
\end{tablenotes}
\end{threeparttable}
\end{table}

Under the null hypothesis of invariance across treatment regimes, we test Condition~\eqref{eq:suff2}. At age 30, we observe labor income in both the experimental sample and in our synthetic cohort created in the non-experimental sample in step 1. Invariance across treatment regimes allows us to test this implication pooling the treatment and control groups in the experimental sample \textbf{[JJH: You mean controls of experimental sample.] [JLG: No. Both the treatment and the control groups in the experimental sample because we already failed to reject invariance.]} with the synthetic controls and treatment groups. If Condition~\eqref{eq:suff2} holds, the coefficients associated with $K$ (an indicator of membership in the experimental ($K = 1$) or non-experimental sample ($K = 0$)), denoted by $\Lambda$, is zero when estimating the following baseline linear technology: 

\begin{equation}
\text{Y}_{k,30} = \Lambda \cdot K +  \bm{B}_{k,30} \cdot \bm{\gamma}_{k,30} +\bm{X}_{k,30} \cdot \bm{\beta}_{k,30} + \varepsilon_{k,a}. \label{eq:linearnoexp} 
\end{equation}
\end{subequations}

Panel (b) in Table~\ref{table:invariance} displays estimates of the coefficients characterizing Equation~\eqref{eq:linearnoexp} for labor income at age 30 for males and females. Estimates by gender in this case are precise because pooling experimental and non-experimental samples generates a larger sample. The estimates of $\Lambda$ are small in magnitude and not statistically significant (the averages of labor income in the experimental and non-experimental sample for females and males are $\$24,584$ and $\$40,007$, and  $\$24,098$ and $\$32,717$ (2014 USD), respectively). We conclude that the technologies are invariant across samples.\\

\noindent \textbf{[JLG: I still think that 5a with exogeneity (A2) implies our testable key implications 6a and 6b and that 5b and its tests could go in the Appendix. But I may be wrong. In any event, here are the tests.]}\\

\begin{table}[!htpb]
\begin{threeparttable}
\caption{Invariance in Distributions of the  Error Terms $\left( F_{k,a}^d \right)$, Age 30} \label{table:invarianceerrors}
\centering
\footnotesize
\begin{tabularx}{16.5cm}{XcX}
& \begin{tabular}{lcccc} \toprule
...
Equality in distributions & \multicolumn{2}{c}{K-S $p$-value} &  \multicolumn{2}{c}{K-S $p$-value}  \\
                                      & \multicolumn{2}{c}{0.290} &  \multicolumn{2}{c}{0.372}  \\ 
\multicolumn{5}{l}{Sample: Experimental Treatment and Control Groups at Age 30} \\ \midrule
...
We use the analogous steps to test Condition~\eqref{eq:crucial2}. First, we test invariance in the distributions of $\bm{\varepsilon}_{k,a}^d$ across treatment regimes within the experimental sample for labor income and employment at age 30. Under the null hypotheses of invariance across treatment regimes, we then test invariance across the experimental and non-experimental samples. To obtain the empirical counterparts to $\bm{\varepsilon}_{k,a}^d$ in Equation~\eqref{eq:outcome} and perform these tests, we residualize the outcomes of interest from the baseline variables and the intermediate inputs. That is, we obtain residuals from equations analogous to Equations~\eqref{eq:linearexp} and \eqref{eq:linearnoexp}, in which $R$ and $K$ are not part of the estimation procedure---recall that we find evidence favoring these exclusions in Table~\ref{table:invariance}. \textbf{[JJH: Did you adjust for residual estimation error] [JLG: Yes. As in all of our estimations. See next for location of procedure.]} Before testing, we adjust the residuals for model estimation error as explained in step 6 of Appendix~\ref{appendix:bootstrapspreds}.

With the empirical counterparts of $\bm{\varepsilon}_{k,a}^d$ in hand, we implement three tests to compare the distributions in Table~\ref{table:invarianceerrors}. The first two are standard: mean comparisons through $t$ and Kolgomorov-Smirnov tests. Within the experiment and at age 30, we fail to reject a common distribution of $\bm{\varepsilon}_{k,a}^d$ for both employment and labor income---Panel (a). As before, under this null hypothesis we pool the experimental treatment and control groups and the synthetic treatment and control groups to test invariance across samples by gender. In all but one of our tests we fail to reject invariance across samples---Panel (b).
