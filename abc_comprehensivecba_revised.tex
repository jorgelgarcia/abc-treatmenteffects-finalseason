...
\noindent \textbf{[Updated length: 39 pages (references and some comments and dialogue included.) This excludes title and addresses pages.]}
...
\noindent for $d \in\{0,1\},  k\in\{e,n\}, \  a\in\{1,\dots,\bar{A}\}$, where $\bm{\varepsilon}_{k,a} = D\bm{\varepsilon}_{k,a}^1 + (1-D)\bm{\varepsilon}_{k,a}^0$ and $\bm{X}_{k,a} = D\bm{X}^1_{k,a}+(1-D)\bm{X}^0_{k,a}$. These conditions are testable in the overlap of the experimental and the non-experimental samples. Our forecasting procedure is as follows:

\begin{enumerate}
\item \textbf{Construct a Baseline Synthetic Cohort.} Choose a non-experimental sample with $\bm{B}_n \in \bm{B}_0$.
\item \textbf{Determine the Relevant Inputs.} Find a set of input variables $\bm{X}^d_{k,a}$ (i) that are available in both experimental and non-experimental data; (ii) with support in the non-experimental data containing the support of their counterparts in the experimental data; (iii) that are non-trivial determinants of outcomes; and (iv) that are affected by treatment.
\item \textbf{Estimate Equation~\eqref{eq:outcome} and evaluate the testable implications of Conditions~\eqref{eq:crucial} and~\eqref{eq:crucial2}.} Using the sample constructed in step 1 and the inputs chosen in step 2, estimate the technology. A crucial step for making forecasts of general validity is establishing exogeneity of the inputs of the technology or accounting for non-exogeneity using appropriate econometric methods.
\item \textbf{Test the Validity of the Technology Estimated in Step 3 with the Experimental Sample.} Using the technology estimated in step 3 and validated in step 4, predict treatment effects within the experimental sample and contrast them with the realized treatment effects.
\item \textbf{Forecast the Experimental Data Out of Sample.} Using the technology estimated in step 3 and validated in step 4, forecast life-cycle treatment effects. Adjust for cohort effects as necessary.
...
\subsection{Forecasting of Labor Income}

We illustrate our methodology for forecasting labor income. For a more extensive discussion, see Appendix~\ref{appendix:incomplete}.
