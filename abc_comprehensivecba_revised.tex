...
\noindent Our forecasting procedure builds on the methodology of \cite{Heckman_etal_2013_Matching}. Experimental outcomes are modeled as the outputs of treatment-invariant technologies. Treatment affects inputs only and not the relationship between inputs and outputs. We measure how treatment affects inputs within sample and forecast future input paths resulting from experimental manipulations. We use non-experimental data to estimate the technologies associated with treatment and use the experimental data to test the hypothesis of treatment invariance of technology and the ability of the estimated technology to reproduce experimental results using the input changes induced by the experiment. Given policy invariance, these input changes define the intervention and link it to changes in child environments occurring outside the experiment.
...
\doublespacing
Testing Assumption~\ref{ass:summary} using the experimental and non-experimental samples without imposing parametric assumptions requires common support conditions over the age period $a \leq a^*$. This is captured by Assumption~\ref{ass:contain}.

\onehalfspacing
\begin{assumption} \textbf{In-Sample Support Conditions} \label{ass:contain}
For $a \leq a^*$,
\begin{equation}
supp( \bm{Y}_{e,a}^d, \bm{X}^d_{e,a}, \bm{B}_e, \bm{\varepsilon}_{e,a} ) \subseteq supp( \bm{Y}_{n,a}^d, \bm{X}^d_{n,a}, \bm{B}_n, \bm{\varepsilon}_{n,a} ). \label{eq:what}
\end{equation}
To forecast out of sample input paths in order to forecast out of sample treatment effects, we require: (i) that the support of the non-experimental data contain the support of the future values that experimentals could experience and (ii) that we can correctly forecast the mean future values. Condition (i) is a version of Assumption~\ref{ass:contain} for $a>a^*$, which for the sake of brevity we do not state formally.\end{assumption}

A sufficient condition for (ii) besides structural invariance is

\begin{assumption}\label{ass:more}
For each admissible sample path associated with individual $\omega$ observed at $a \leq a^*$, we can accurately forecast $a>a^*$, where $\bm{X}^d_{e,a} (\omega), \bm{B}_e (\omega)$ are the values for individual $\omega$ and $\mathbb{E} \left[ \bm{\varepsilon}^d_{e,a}\right]$ the future mean values of shocks. 
\end{assumption}

\begin{remark}
This condition includes adjustments for cohort effects. 
\end{remark}
...
Note, however, that if the only goal is to construct unbiased forecasts of mean treatment effects, the minimal requirement is that experimental treatment effects should equal differences in the conditional means of the non-experimental samples evaluated at $\bm{X}_{n,a} = \bm{x}^1$ and  $\bm{X}_{n,a} = \bm{x}^0$ respectively:
...
\end{eqnarray}
\end{subequations}
Rather than just assume \eqref{eq:nec}, we test sufficient conditions for it to hold: i.e., we test Equations~\eqref{eq:suff1} and~\eqref{eq:suff2}. We test Condition~\eqref{eq:suff1} across treatment regimes and Condition~\eqref{eq:suff2} for $d = 0$ at age 30, where we observe labor income in the experimental sample for both the treatment and the control groups. Assuming linearity, if Condition~\eqref{eq:suff1} holds, the coefficient associated with $D$, denoted by \texttau, should be zero in
...
We use auxiliary information and structural economic models to guide monetization of treatment effects and to extrapolate the measured benefits and costs to the full life cycles of participants. We account for model estimation and forecast error and conduct extensive sensitivity analyses of our estimates to alternative assumptions and methodologies. Under a variety of plausible assumptions, we estimate that the tax-adjusted internal rate of return of the program ranges from 8\% to 18.3\%. These estimates demonstrate the social profitability of ABC/CARE. We show that estimates from a robust nonparametric matching strategy are close to those from our structural approach. 
