...
Under Assumption~\ref{ass:exog}, we build on \citet{Heckman_Pinto_etal_2013_PerryFactor} an assume that the variables $\bm{X}_{k,a}^d$ fully summarize treatment in the sense that any effect that treatment has on outcomes operates through the inputs, $\bm{X}_{k,a}^d$, and not through shifts in the production function relating inputs to outputs. Assumption~\ref{ass:summary} formalizes this condition.
...
\noindent \textbf{[JJH: Verbal descriptions of statistical procedure are not helpful.] [JLG: Here are the implications and the equations that we estimate to test the implications.]}\\
...
\noindent \textbf{[JJH: Please separate (1) invariance from (2) tests of treatment effects] [JLG: What Equation~\eqref{eq:linearexp} tests is if, once intermediate inputs are accounted for, $R$ (randomization into treatment) is still a significant determinant of the outcome of interest. If not, then the intermediate inputs in fact mediate treatment. An analogous test is presented for invariance across samples.]}\\

At age 30, we observe labor income in the experimental sample for both the treatment and the control groups. If Condition~\eqref{eq:suff1} holds, the coefficient associated with $R$ (randomization to treatment assignment in ABC/CARE, which, as discussed, is equivalent to $D$), denoted by $\Delta$, with $R$ (the treatment indicator) is zero when estimating the following baseline linear technology: 
\setcounter{equation}{6}
\begin{subequations}
\begin{equation}
\text{Y}_{e,30}^d = \Delta \cdot R +  \bm{B}_{e,30} \cdot \bm{\gamma}_{e,30}^d +\bm{X}_{e,30}^d \cdot \bm{\beta}_{e,30}^d + \varepsilon_{e,a}^d, \label{eq:linearexp} 
\end{equation}

\noindent where $Y_{e,30}^d$ and $\varepsilon_{e,a}^d$ are scalar entries in the vectors $\bm{Y}_{e,a}$ and $\bm{\varepsilon}^d_{k,a}$ and $\bm{\gamma}_{e,30}^d$ and $\bm{\beta}_{e,30}^d$ are vectors of coefficients. Failing to reject the null hypothesis $H_{0}: \Delta = 0$ is equivalent to failing to reject invariance across treatment regimes.\\

\noindent \textbf{[JJH: Table 2 has too little documentation.] [JLG: The two next paragraphs describe what each Panel in Table 2 describes. I have also put descriptions in each of the rows of the Table.]}\\

Panel (a) of Table~\ref{table:invariance} displays estimates of the coefficients characterizing Equation~\eqref{eq:linearexp} for labor income at age 30 pooling males and females to increase precision. To enhance the evidence we also display analogous estimates for employment at age 30, an outcome related to labor income. We fail to reject the null hypothesis that $H_{0}: \Delta = 0$. \textbf{[JJH: I thought that we rejected non-invariance!] [JLG: No. We fail to reject invariance. That has always been the test we implement because the way in which our implications are formulated in 5a and 5b. We operate under the null of those equalities.]} In both cases, the tests are precise, noting that after accounting for background variables and the intermediate inputs, average labor income is $\$786.262$ (2014 USD) higher in the treatment group. This value is small in the context of annual labor income at age 30 and given that the average in the control group is $\$26,083$ (2014 USD). We conclude that the technology is invariant across treatment regimes. The rest of the components in Panel (a) indicate that the outcomes are non-trivial functions of the outputs of interest (which is verified in step 2). Years of education is a specially important input in our forecast. It is a statistically and economically significant determinant of labor income and it is experimentally boosted by ABC/CARE (see Table~\ref{table:summint}).
...
Equality in distributions & \multicolumn{2}{c}{K-S $p$-value} &  \multicolumn{2}{c}{K-S $p$-value}  \\
                                      ...
                                      & \multicolumn{2}{c}{Rosenbaum $p$-value} &  \multicolumn{2}{c}{Rosenbaum $p$-value}  \\
