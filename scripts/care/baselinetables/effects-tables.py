...
for RV in [0,1]:
    ...
    #for a in ['7']:
        ...
        if a == '0':
            rslt = pkl.load(open(os.path.join(paths.rslt, 'bootstrap_balance_{}.pkl').format(RV_s), 'rb'))
        if a in ['5', '6']:
            rslt = pkl.load(open(os.path.join(paths.rslt, 'bootstrap_balance_coh{}_{}.pkl'.format(a, RV_s)), 'rb'))
        if a == '7':
            rslt = pkl.load(open(os.path.join(paths.rslt, 'bootstrap_balance_crime_{}.pkl'.format(RV_s)), 'rb'))        
        if a == '8':
            rslt = pkl.load(open(os.path.join(paths.rslt, 'bootstrap_balance_health_{}.pkl'.format(RV_s)), 'rb'))        
        if a == '10':
            rslt = pkl.load(open(os.path.join(paths.rslt, 'bootstrap_balance_attrition_{}.pkl'.format(RV_s)), 'rb'))        
        if a == '11':
            rslt = pkl.load(open(os.path.join(paths.rslt, 'bootstrap_balance_postattrition_{}.pkl'.format(RV_s)), 'rb'))        
        ...
        if a in ['5','6']:
            pooled_t = data.query('R==1&cohort=={}'.format(a))[outcomes.index].count().to_frame()
            male_t = data.query('male==1&R==1&cohort=={}'.format(a))[outcomes.index].count().to_frame()
            female_t = data.query('male==0&R==1&cohort=={}'.format(a))[outcomes.index].count().to_frame()
            counts_t = pd.concat([pooled_t, male_t, female_t], axis=1)
            counts_t.columns = pd.MultiIndex.from_product([['pooled', 'male', 'female'], [None], ['obs']], names=['sex', 'est', 'stat'])
            
            pooled_c = data.query('R==0&cohort=={}'.format(a))[outcomes.index].count().to_frame()
            male_c = data.query('male==1&R==0&cohort=={}'.format(a))[outcomes.index].count().to_frame()
            female_c = data.query('male==0&R==0&cohort=={}'.format(a))[outcomes.index].count().to_frame()
            counts_c = pd.concat([pooled_c, male_c, female_c], axis=1)        
            counts_c.columns = pd.MultiIndex.from_product([['pooled', 'male', 'female'], [None], ['att']], names=['sex', 'est', 'stat'])           
        ...
        if a in ['10']:
            ...
            pooled_t = data.loc[attriters][outcomes.index].count().to_frame()
            male_t = data.loc[attriters_male][outcomes.index].count().to_frame()
            female_t = data.loc[attriters_female][outcomes.index].count().to_frame()
            counts_t = pd.concat([pooled_t, male_t, female_t], axis=1)
            counts_t.columns = pd.MultiIndex.from_product([['pooled', 'male', 'female'], [None], ['obs']], names=['sex', 'est', 'stat'])
            
            pooled_c = data.drop(attriters, axis=0)[outcomes.index].count().to_frame()
            male_c = data.drop(attriters, axis=0).query('male==1')[outcomes.index].count().to_frame()
            female_c = data.drop(attriters, axis=0).query('male==0')[outcomes.index].count().to_frame()
            counts_c = pd.concat([pooled_c, male_c, female_c], axis=1)        
            counts_c.columns = pd.MultiIndex.from_product([['pooled', 'male', 'female'], [None], ['att']], names=['sex', 'est', 'stat'])      
    
        if a in ['11']:
        
            pooled_t = data.drop(attriters, axis=0).query('R==1')[outcomes.index].count().to_frame()
            male_t = data.drop(attriters, axis=0).query('male==1&R==1')[outcomes.index].count().to_frame()
            female_t = data.drop(attriters, axis=0).query('male==0&R==1')[outcomes.index].count().to_frame()
            counts_t = pd.concat([pooled_t, male_t, female_t], axis=1)
            counts_t.columns = pd.MultiIndex.from_product([['pooled', 'male', 'female'], [None], ['obs']], names=['sex', 'est', 'stat'])
            
            pooled_c = data.drop(attriters, axis=0).query('R==0')[outcomes.index].count().to_frame()
            male_c = data.drop(attriters, axis=0).query('male==1&R==0')[outcomes.index].count().to_frame()
            female_c = data.drop(attriters, axis=0).query('male==0&R==0')[outcomes.index].count().to_frame()
            counts_c = pd.concat([pooled_c, male_c, female_c], axis=1)        
            counts_c.columns = pd.MultiIndex.from_product([['pooled', 'male', 'female'], [None], ['att']], names=['sex', 'est', 'stat'])      
        ...
        for sex in ['pooled']:  
            for i, cat in enumerate(outcomes.category.drop_duplicates().tolist()):
                ...
                if a == '0':
                    table.write(os.path.join(paths.apptables, 'baseline_balance_care_{}'.format(RV_s)))
                if a in ['5','6']:
                    table.write(os.path.join(paths.apptables, 'baseline_coh{}_care_{}'.format(a, RV_s)))
                if a == '7':
                    table.write(os.path.join(paths.apptables, 'crime_balance_care_{}'.format(RV_s)))
                if a == '8':
                    table.write(os.path.join(paths.apptables, 'health_balance_care_{}'.format(RV_s)))                
                if a == '10':
                    table.write(os.path.join(paths.apptables, 'attrition_balance_care_{}'.format(RV_s)))                
                if a == '11':
                    table.write(os.path.join(paths.apptables, 'postattrition_balance_care_{}'.format(RV_s)))                                
