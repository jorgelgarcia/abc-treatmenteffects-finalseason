...
b = 3  # number of work each worker does
...
for gender in genderloop
	...
	Matchfinal["$(gender)"] = vcat(matchfinal_pre1, matchboot[11]["$(gender)"], matchboot[12]["$(gender)"], matchboot[13]["$(gender)"], matchboot[14]["$(gender)"], matchboot[15]["$(gender)"], matchboot[16]["$(gender)"], matchboot[17]["$(gender)"], matchboot[18]["$(gender)"], matchboot[19]["$(gender)"], matchboot[20]["$(gender)"])
	...
	for P_switch in (0, 1)
		...
		writetable("$(results)/matching/tax_$(gender)_P$(P_switch).csv", ResultOutput["matching_$(gender)_P$(P_switch)"])
		c = c + 1
  end
end
