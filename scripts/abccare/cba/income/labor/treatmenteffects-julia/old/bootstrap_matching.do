...
foreach sex in male female pooled {
	...
	foreach var of varlist c*_`sex' {
		local name = subinstr("`var'", "_`sex'", "", .)
		rename `var' `name'
	}
	...
	* determine file names by P_switch
	foreach P_switch in 0 1 {	
		...
		file open matching_pooled_P`P_switch' using "${results}/matching/labor_`sex'`P_suffix'.csv", write replace
	}
