...
for sex in ['male', 'female', 'pooled']:
	params_interp[sex] = pd.concat([rslt[k][0][sex] for k in range(aux_draw)], axis=0, keys=range(aux_draw), names=['adraw'])
 	...
 	errors[sex] = pd.concat([rslt[k][2][sex] for k in range(aux_draw)], axis=0, keys=range(aux_draw), names=['adraw'])
   	...
   	params_interp[sex].to_pickle(os.path.join(paths.rslts, 'transfer_interp_params_{}.pkl'.format(sex)))
   	params_extrap[sex].to_pickle(os.path.join(paths.rslts, 'transfer_extrap_params_{}.pkl'.format(sex)))
   	errors[sex].to_pickle(os.path.join(paths.rslts, 'transfer_errors_{}.pkl'.format(sex)))
   	'''

   	# output projections .csv
   	projections[sex].to_csv(os.path.join(paths.rslts, 'transfer_proj_{}.csv'.format(sex)))
