...
ITTboot = pmap(ITTrun, [b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b])
ITTfinal = Dict()
...
for gender in genderloop
	...
	ITTfinal["$(gender)"] = vcat(ITTfinal_pre2, , ITTboot[21]["$(gender)"], ITTboot[22]["$(gender)"], ITTboot[23]["$(gender)"], ITTboot[24]["$(gender)"], ITTboot[25]["$(gender)"], ITTboot[26]["$(gender)"], ITTboot[27]["$(gender)"], ITTboot[28]["$(gender)"], ITTboot[29]["$(gender)"], ITTboot[30]["$(gender)"])
	...
	for P_switch in (0, 1, 10)
		...
		writetable("$(results)/itt/transfer_$(gender)_P$(P_switch).csv", ResultOutput["itt_$(gender)_P$(P_switch)"])
		c = c + 1
  end
end
