...
B = 30 # number of workers being used
b = 34  # number of work each worker does

matchboot = pmap(matchingrun, [b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b, b])
Matchfinal = Dict()
...
for gender in genderloop
	...
	Matchfinal["$(gender)"] = vcat(matchfinal_pre2, matchboot[21]["$(gender)"], matchboot[22]["$(gender)"], matchboot[23]["$(gender)"], matchboot[24]["$(gender)"], matchboot[25]["$(gender)"], matchboot[26]["$(gender)"], matchboot[27]["$(gender)"], matchboot[28]["$(gender)"], matchboot[29]["$(gender)"], matchboot[30]["$(gender)"])
	...
	for P_switch in (0, 1)
		...
		writetable("$(results)/matching/transfer_$(gender)_P$(P_switch).csv", ResultOutput["matching_$(gender)_P$(P_switch)"])
		c = c + 1
  end
end
