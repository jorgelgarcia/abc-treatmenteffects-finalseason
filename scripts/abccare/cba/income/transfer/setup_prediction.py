...
def predict_abc(interp, extrap, interp_index, extrap_index, abc, verbose=True):
	...
	for sex in ['pooled', 'male', 'female']: 
		...
		params_extrap[sex] = pd.DataFrame([[np.nan for j in range(len(cols.extrap.predictors) + 2)] for k in range(31,80)], index = range(31,80))
	...
	for sex in ['pooled', 'male', 'female']:  
		...
		projection_extrap[sex] = abcd_extrap.dot(params_extrap[sex].drop('rmse', axis=1).T) + error_mat[sex].drop('R', axis = 1).loc[:,slice(31,79)]
