...
/*
Project :       ABC CVA
Description:    Test of invariance of prediction function, labor income, ABC and CARE samples
...
matrix rownames allests = m_ed0y piatmath years_30y si21y_inc_labor si34y_bmi cogfactor noncogfactor cons
foreach b of numlist 1(1)$bootstraps {
	preserve
	bsample

	// construct ipw
	reg si30y_inc_labor m_ed0y piatmath years_30y si21y_inc_labor si34y_bmi
	...
	reg si30y_inc_labor m_ed0y cogfactor noncogfactor [aw=ipw] if R == 1, robust
	matrix t1f = e(b)
	matrix t1fcomplete`b' = [t1f[1,1],J(1,4,.),t1f[1,2..4]]'
	matrix rownames t1fcomplete`b' = m_ed0y piatmath years_30y si21y_inc_labor si34y_bmi cogfactor noncogfactor cons
	matrix colnames t1fcomplete`b' = t1fcomplete`b'
	mat_capp allests : allests t1fcomplete`b'

	
	reg si30y_inc_labor m_ed0y piatmath years_30y si21y_inc_labor cogfactor noncogfactor [aw=ipw] if R == 1, robust
	matrix t2f = e(b)
	matrix t2fcomplete`b' = [t2f[1,1..4],J(1,1,.),t2f[1,2..4]]'
	matrix rownames t2fcomplete`b' = m_ed0y piatmath years_30y si21y_inc_labor si34y_bmi cogfactor noncogfactor cons
	matrix colnames t2fcomplete`b' = t2fcomplete`b'
	mat_capp allests : allests t2fcomplete`b'
	

	reg si30y_inc_labor m_ed0y piatmath years_30y si21y_inc_labor si34y_bmi cogfactor noncogfactor [aw=ipw] if R == 1, robust
	matrix t3f = e(b)
	matrix t3fcomplete`b' = t3f[1,1..8]'
	matrix rownames t3fcomplete`b' = m_ed0y piatmath years_30y si21y_inc_labor si34y_bmi cogfactor noncogfactor cons
	matrix colnames t3fcomplete`b' = t3fcomplete`b'
	mat_capp allests : allests t3fcomplete`b'

	
	// treatment regressions with no factor
	reg si30y_inc_labor m_ed0y [aw=ipw] if R == 1, robust
	matrix t1 = e(b)
	matrix t1complete`b' = [t1[1,1],J(1,6,.),t1[1,2]]'
	matrix rownames t1complete`b' = m_ed0y piatmath years_30y si21y_inc_labor si34y_bmi cogfactor noncogfactor cons
	matrix colnames t1complete`b' = t1complete`b'
	mat_capp allests : allests t1complete`b'
	
	reg si30y_inc_labor m_ed0y piatmath years_30y si21y_inc_labor [aw=ipw] if R == 1, robust
	matrix t2 = e(b)
	matrix t2complete`b' = [t2[1,1..4],J(1,3,.),t2[1,2]]'
	matrix rownames t2complete`b' = m_ed0y piatmath years_30y si21y_inc_labor si34y_bmi cogfactor noncogfactor cons
	matrix colnames t2complete`b' = t2complete`b'
	mat_capp allests : allests t2complete`b'

	reg si30y_inc_labor m_ed0y piatmath years_30y si21y_inc_labor si34y_bmi [aw=ipw] if R == 1, robust
	matrix t3 = e(b)
	matrix t3complete`b' = [t3[1,1..5],J(1,2,.),t3[1,6]]'
	matrix rownames t3complete`b' = m_ed0y piatmath years_30y si21y_inc_labor si34y_bmi cogfactor noncogfactor cons
	matrix colnames t3complete`b' = t3complete`b'
	mat_capp allests : allests t3complete`b'
	
	// control regressions with factor
	reg si30y_inc_labor m_ed0y cogfactor noncogfactor [aw=ipw] if R == 0, robust
	matrix c1f = e(b)
	matrix c1fcomplete`b' = [c1f[1,1],J(1,4,.),c1f[1,2..4]]'
	matrix rownames c1fcomplete`b' = m_ed0y piatmath years_30y si21y_inc_labor si34y_bmi cogfactor noncogfactor cons
	matrix colnames c1fcomplete`b' = c1fcomplete`b'
	mat_capp allests : allests c1fcomplete`b'
	
	reg si30y_inc_labor m_ed0y piatmath years_30y si21y_inc_labor cogfactor noncogfactor [aw=ipw] if R == 0, robust
	matrix c2f = e(b)
	matrix c2fcomplete`b' = [c2f[1,1..4],J(1,1,.),c2f[1,2..4]]'
	matrix rownames c2fcomplete`b' = m_ed0y piatmath years_30y si21y_inc_labor si34y_bmi cogfactor noncogfactor cons
	matrix colnames c2fcomplete`b' = c2fcomplete`b'
	mat_capp allests : allests c2fcomplete`b'

	reg si30y_inc_labor m_ed0y piatmath years_30y si21y_inc_labor si34y_bmi cogfactor noncogfactor [aw=ipw] if R == 0, robust
	matrix c3f = e(b)
	matrix c3fcomplete`b' = c3f[1,1..8]'
	matrix rownames c3fcomplete`b' = m_ed0y piatmath years_30y si21y_inc_labor si34y_bmi cogfactor noncogfactor cons
	matrix colnames c3fcomplete`b' = c3fcomplete`b'
	mat_capp allests : allests c3fcomplete`b'
	
	// control regressions with no factor
	reg si30y_inc_labor m_ed0y [aw=ipw] if R == 0, robust
	matrix c1 = e(b)
	matrix c1complete`b' = [c1[1,1],J(1,6,.),c1[1,2]]'
	matrix rownames c1complete`b' = m_ed0y piatmath years_30y si21y_inc_labor si34y_bmi cogfactor noncogfactor cons
	matrix colnames c1complete`b' = c1complete`b'
	mat_capp allests : allests c1complete`b'
	
	reg si30y_inc_labor m_ed0y piatmath years_30y si21y_inc_labor [aw=ipw] if R == 0, robust
	matrix c2 = e(b)
	matrix c2complete`b' = [c2[1,1..4],J(1,3,.),c2[1,2]]'
	matrix rownames c2complete`b' = m_ed0y piatmath years_30y si21y_inc_labor si34y_bmi cogfactor noncogfactor cons
	matrix colnames c2complete`b' = c2complete`b'
	mat_capp allests : allests c2complete`b'

	reg si30y_inc_labor m_ed0y piatmath years_30y si21y_inc_labor si34y_bmi [aw=ipw] if R == 0, robust
	matrix c3 = e(b)
	matrix c3complete`b' = [c3[1,1..5],J(1,2,.),c3[1,6]]'
	matrix rownames c3complete`b' = m_ed0y piatmath years_30y si21y_inc_labor si34y_bmi cogfactor noncogfactor cons
...
foreach num of numlist 1(1)3 {
	outtable using abccare_funcform`num', mat(allcoefsc`num') replace nobox center f(%9.3f)
}
