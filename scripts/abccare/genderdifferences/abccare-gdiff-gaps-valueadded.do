	...
	foreach c in `outcome_categories' {
		...
		foreach v in ``c'' {
			...
			forvalues s = 0/1 {
				...
				qui sum `v' if male == `s' & R == 0 & dc_mo_pre == 0 // & dc_mo_pre != .
				local b`v'`s'`b'_R0 = r(mean)
				
			}
			
			if `b`v'1`b'_R0' - `b`v'0`b'_R0' != 0 {
				if (`b`v'1`b'_R1' - `b`v'0`b'_R1') - (`b`v'1`b'_R0' - `b`v'0`b'_R0')/(`b`v'1`b'_R0' - `b`v'0`b'_R0') > 0 {
...
,
	...
	legend(order(1 2 3) rows(3) label(1 "{bf:Proportion Value-added > 0}") 
	label(2 "+/- s.e.") label(3 "p-value {&le} 0.10") size(small))
;
# delimit cr

cd $output
graph export "gendergaps-valueadded-proportion-home.eps", replace
