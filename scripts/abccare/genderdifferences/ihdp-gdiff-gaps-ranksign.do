...
This file:	abccare-gdiff-gaps-ranksign FOR IHDP
*/
...
// filepaths
global data	   	= "$klmshare/Data_Central/IHDP2/Cleaned_data/base/"
...
forvalues b = 0/$bootstraps {
	...
	foreach c in `categories' {
		...
		foreach v in ``c'' {
			forvalues s = 0/1 {
				qui sum `v' if admin_sex == `s' & admin_treat == 0  
				local b`v'`s'`b'_R0 = r(mean)
				qui sum `v' if admin_sex == `s' & admin_treat == 1
...
file open tabfile using "${output}/ihdp-proportion-summary-full.tex", replace write
...
file write tabfile "% This file generated by: abccare-cba/scripts/abccare/genderdifferences/ihdp-gdiff-gaps-ranksign.do" _n
file close tabfile
