...
keep id R male only_child0y has_child_sib0y p_inc*
...
forval sex = 0/2 {
	preserve
		...
		forval sib = 0/1 {
			...
			reg pinc_npv R if has_child_sib0y == `sib'
		...
		local legend	legend(cols(3) order(1 "No Siblings Younger than 5" 2 "> 0 Siblings Younger than 5" 3 "p-value {&le} .10") size(small))
		local yaxis		ytitle("Treatment Effect on Parental Income (1000s of 2014 USD)", size(small))  ylabel(0[20]160, angle(h) glcolor(gs14))
		...
		graph export ${output}/abccare_pinc_npv_sibs5_`NAME`sex''.eps, replace
