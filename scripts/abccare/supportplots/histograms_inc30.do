...
global auxdata psid nlsy cnlsy
local dataobs
...
graph bar pdfabc pdfpsid pdfnlsy pdfcnlsy, 
	...
	legend(label(1 ABC/CARE (N = `abc_n')) label(2 PSID (N = 14,135)) label(3 NLSY79 (N = 10,119)) label(4 CNLSY (N = 1,887)) size(small) rows(2))
...
cd "$filedir"
graph export "$output/support_inc30.eps", replace
