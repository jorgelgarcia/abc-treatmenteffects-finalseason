...
// ready data
global datatreatp   = "$klmmexico/abccare/treatmenteffects/has_child_sib0y_restricted_20160927"
...
foreach sex in pooled {
	...
	twoway (bar itt varind if sexnum == 1, color(gs8) barwidth(.9))
	       ...
	       (scatter itt varind if ittp <= .11, msymbol(circle) mlwidth(medthick) mlcolor(black) mfcolor(black) msize(large))
		, 
		legend(cols(3) order(1 "All Siblings Younger than 5" 2 "All Siblings 5 or Older" 3 "p-value {&le} .10") size(small))
	...
	#delimit cr
	graph export abccare_pincomesumsibage_s`sex'.eps, replace
