...
for t in [1,2]:    # t is for stepdown or no stepdown
    ...
    for sex in ['male', 'female']:
        ...
        # write out tables
        if t == 1:
            table.write(os.path.join(paths.maintables, 'rslt_{}_main_ts'.format(sex)))
        if t == 2:
            table.write(os.path.join(paths.maintables, 'rslt_{}_main_sd_ts'.format(sex)))
...
for t in [1,2]:
    ...
    for sex in ['pooled', 'male', 'female']:
        for i, cat in enumerate(outcomes.category.drop_duplicates().tolist()):
            ...
            # write out tables
            if t == 1:
                table.write(os.path.join(paths.apptables, 'rslt_{}_cat{}_ts'.format(sex, i)))
            if t == 2:
                table.write(os.path.join(paths.apptables, 'rslt_{}_cat{}_sd_ts'.format(sex, i)))            
...
for sex in ['pooled', 'male', 'female']:
    ...
    # write out tables
    table.write(os.path.join(paths.apptables, 'rslt_{}_counts_ts'.format(sex)))
...
for sex in ['pooled', 'male', 'female']:
    ...
    for cen_sig in [(0.5, 100), (0.1, 10)]:
        ...
        # write out tables
        table.write(os.path.join(paths.apptables, 'rslt_{}_counts_n{}a{}_ts'.format(sex, n, a)))
