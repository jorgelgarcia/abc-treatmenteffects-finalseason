...
conDict["controls_abc"] = [:male, :hrabc_index, :apgar1, :apgar5, :m_teen0y, :grandma_county]
...
keepvar = [:id, :family, :R, :RV, :P, :cohort_group1, :cohort_group2, :cohort_group3, :cohort_group4, :cohort_group5, :cohort_group6, :only_child0y, :has_child_sib0y]
...
# Drop CARE home visit individuals
abccare = abccare[!((abccare[:RV] .== 1) & (abccare[:R] .== 0)), :]
...
# ------------------------------------------- #
if child_sibs == 1
  abccare = abccare[abccare[:has_child_sib0y] .== 1, :]
end

if child_sibs == 0
  abccare = abccare[abccare[:has_child_sib0y] .== 0, :]
end
