...
function mestimate(sampledata, outcomes, outcome_list, draw, ddraw, bootsample, bygender)
    ...
    for gender in mgender
     ...
     # ------------------------------ #
      for p in (0, 1)
        ...
        outMat["matching_$(gender)_P$(p)"] = DataFrame(rowname = [], draw = [], ddraw = [], controln = [],
                              epan_ipw = [], epan_N = [])
          ...
          for y in outcome_list
            ...
            try
             #  lm(fml, usedata)
              ...
              push!(outMat["matching_$(gender)_P$(p)"], [y, draw, ddraw, controls_n, NA, NA])
              continue
            end
            control_list = [:R]
