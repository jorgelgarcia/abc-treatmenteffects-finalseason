...
function ITTestimator(sampledata, outcomes, outcome_list, draw, ddraw, bootsample, bygender)
        ...
        controls = conDict["controls$(controls_number)"]
          ...
          for y in outcome_list
              ...
              # If the regression fails
                ...
                push!(outMat["ITT_$(gender)_P$(p)"], [y, draw, ddraw, controls_n, NA, NA, NA, NA, NA, NA, NA, NA, NA])
                continue
              end
              ...
              # If the regression fails
                  ...
                  push!(outMat["ITT_$(gender)_P$(p)"], [y, draw, ddraw, controls_n, ITT_none_coeff, ITT_none_pval, ITT_none_N, NA, NA, NA, NA, NA, NA])
                  continue
              end
              ...
              if in(parse("ipw_$(y)"), names(usedata))
                ...
                # If the regression fails
                    ...
                    push!(outMat["ITT_$(gender)_P$(p)"], [y, draw, ddraw, controls_n, ITT_none_coeff, ITT_none_pval, ITT_none_N, ITT_control_coeff, ITT_control_pval, ITT_control_N, NA, NA, NA])
                    continue
                end
              ...
              push!(outMat["ITT_$(gender)_P$(p)"], [y, draw, ddraw, controls_n,
  ...
  if bygender == 1
    Output = hcat(outMat["ITT_male_P0"], outMat["ITT_male_P1"], outMat["ITT_male_P10"], outMat["ITT_female_P0"], outMat["ITT_female_P1"], outMat["ITT_female_P10"])
  elseif bygender == 0
