...
// autocorrelation 
xtgls inc_labor male m_ed0y piatmath years_30y si21y_inc_labor linc_labor, corr(psar1) force igls rhotype(dw) 
estimates store psar1cnlsy
cd $output
outreg2 [psar1cnlsy] using auto_cnlsy_fe, replace alpha(.01, .05, .10) sym (***, **, *) dec(3) par(se) r2 tex(frag)
...
// autocorrelation 
xtgls inc_labor male years_30y si30y_inc_labor linc_labor, corr(psar1) force igls rhotype(dw) 
estimates store psar1psidnlsy

cd $output
outreg2 [psar1psidnlsy] using auto_psidcnlsy_fe, replace alpha(.01, .05, .10) sym (***, **, *) dec(3) par(se) r2 tex(frag)
