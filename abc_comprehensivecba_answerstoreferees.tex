...
\noindent Response. The forecasting is fleshed out and illustrated in the introduction. The econometric portion has now been compressed and reorganized, so that the basic testable implications could be mathematically expressed. The rest has been left out.\\
...
\noindent Response. The paper is relatively long so we decided to intersperse the literature review in the introduction. The first paragraph of the paper actually states what the concrete contribution of our paper is. While you are right that there has been much work on the identifying an estimating the gains from early education, much less is the work on long term outcomes and on life-cycle benefits. Our paper is the first to provide a methodology to forecast life-cycle benefits to estimate rates of return. In fact, by doing so it provides a template to evaluate social programs and not only early childhood education programs. The appropriate comparisons and relevant literature are cited in the introduction. A whole section is devoted to compare out method with ``back-of-the-envelope'' calculations that current papers in the literature employ. We think that this gives a clear idea of what the contribution of our paper is.\\

\noindent \textbf{Editor. I am not really sure that the econometric theory portion (most of section 3, thus) with all its abstract assumptions is all that helpful: parts of it are sloppy (see e.g. main comment 4), parts of it are unclear (see e.g. main comment 4 and several minor comments), and parts of it seem to hide rather than illuminate the crucial parts of the analysis (see in particular my main comment 5 below). This should all be much more coherent. Perhaps, for the JPE, one can afford to off-shelve some of the detailed, but ultimately tangential econometric theory formulation to the technical appendix: it sometimes feels like something that got formulated, after the intuition of the general principles were understood (and those would be good to formulate) rather than the other way around. I am not sure that the formalism will be of much help to any reader, at a first pass. I shall admit, I may have been missing the crucial bit, which you might strongly feel should be part of the main text. But perhaps even then there is a solution of drastically reducing it all to that, while moving the full-fledged version to the technical appendix.}\\

\noindent Response.  We are now only keeping the basic. Having labor income as a ``running example'' allows us to get rid of one of the subindices and sets that we were carrying around---see Equation (1) in page 10. Then, we reduced the number of assumptions to what is absolutely basic to derive the testable implications. The testable implications are very relevant in explaining the intuition of the forecasting as well as how would one take our method to practice in other samples. The whole procedure is now explained in only a few equations.\\
...
\noindent Response. That is correct, as we express in pg.\ 17, one could get by with the difference. We rely on the stronger conditions of predicting in levels both the treatment and the control because these provides us with additional testable implications---Equations (5) and (6) in pg.\ 17---in which our method performs very well.\\
...
\noindent Response. It shows up because there are two \text{different} types of predictors: $\bm{B}_k$ background variables that are not affected by treatment; and (ii) $\bm{X}_{k,a}^d$ which could be affected by treatment (what you call outcomes up to age a). The key assumption for obtaining our forecasts is actually that $\bm{X}_{k,a}^d$ summarizes the treatment effect that the program could have on $Y_{k,a}^d$. Any treatment-control difference in the outcomes of interest arises from a treatment-control difference in $\bm{X}_{k,a}$ (as dictated by the function $\phi$). These points are clearly disguised in the list of steps in page 11.\\
...
\noindent Response. We now thoroughly comment on this in Section 3.1, where we detail the construction of the synthetic cohorts. As you point out, it makes more sense to construct one synthetic control group and one synthetic treatment group for doing the analysis across all of the outcomes. That is what we do.\\
...
\noindent Response. We understand the source of the confusion and this comment is an opportunity to clarify. In Equation (1) we define what the relationship between the outcomes of interest $Y_{k,a}^d$ and two \text{different} types of predictors: $\bm{B}_k$ background variables that are not affected by treatment; and (ii) $\bm{X}_{k,a}^d$ which could be affected by treatment (what you call outcomes up to age a). The key assumption for obtaining our forecasts is actually that $\bm{X}_{k,a}^d$ summarizes the treatment effect that the program could have on $Y_{k,a}^d$. Recall: we do not want to match individual-to-individual. Instead, we want to create comparable samples to treatment and control in the auxiliary samples. These points are clearly disguised in the list of steps in page 11.\\

\noindent \textbf{Editor. Cut figure 2: TMI. This is technical appendix material. Key results ``in print'' only, please.}\\

\noindent Response. We find this figure to be helpful in communicating the robustness of our results, but we understand that the first draft contained way too much information. We put moved this figure to the results section and expanded rely much more on it to disguise our results.\\

\noindent \textbf{Editor. P.7, ``when we omit crime ... still ... substantial returns''. State how much.}\\

\noindent Response. This sentence has actually been edited out.\\
...
\noindent Response. Section 2 does describe the main ingredients and Section 4.4 expands on the cost of each of the annual components of the costs. We do not think it is convenient to elaborate further on this in the main text. But the following table (Table B.1 in the Appendix) expands on the costs. As it is usual in these programs, teaching is the most expensive input. Note from Appendix B that we went back to the primary sources (even invoices) to be able to compute these costs.\\
...
\noindent Response. This part of the main text contained a lot of jargon and we decided to leave it out of the text to avoid confusion. For all practical matters, what it is important is that the program was randomized. The randomization was done by the program themselves. The paired randomization process as you described it is exactly how they carried out the randomization. Siblings (either when they were twins or when they were not) were paired and randomized as a unit and put together either in treatment or control status. The steps and a diagram describing the randomization protocol in details are in Appendix A, pages 6 to 10.\\
...
\noindent Response. The definitions of $W, D, R$ are now provided in two consecutive paragraphs in pg. 10. We do think that it is important to define them deparately. $W$ is offer to participate of randomization, $R$ is randomizations, and $D$ is compliance. It is \textit{not} a feature of all programs that $W = 1$ and $D = R$. When interpreting the external validity of our estimates, this is a powerful fact. Once invited, everyone agrees to participate and complies. Thus, our inferences are validad for everyone in the population who was given an offer to begin with. We can locate those people by asking who satisfies $\bm{B} \in \mathcal{B}_0$ (i.e.,\ who is eligible).\\
...
\noindent Response. This has been rephrased (page 10, first two paragraphs). Both $R$ takes $1$ if the child is randomizes to treatment and $0$ if she is randomized to control. $D$ is $1$ if the child complies to the treatment status (i.e.,\ the parents comply to what the randomization protocol indicates). $R$ and $D$ are \textit{ex-ante} different: The former is a not a decision and the latter is.\\

\noindent \textbf{Editor. P. 11: The first sentence of the paragraph ``Individuals ...'' should need one or several extra words at the end to make it proper English.}\\

\noindent Response. We have clarified that $\mathcal{B}_0$ is the et of scores on the risk index that determines program eligibility (pg. 10). Eligibility and the index are discussed in Section 2 as part of the description of the program. The index is spelled out on footnote 15., and then summarizes in detail in Appendix A2.\\
...
\noindent Response. We agree and actually came to the conclusion that the $j$ indexing was unnecessary. $j$ indexed one of many outcomes, and we are now using one outcome to summarize our method. The generalization to many outcomes is obvious and thus we dropped this indexing.\\
...
\noindent Response. We have done so. Table 1 was not effective, and, as you suggest, we base everything on our labor income example. Instead of providing a tedious tables we spelled out a recipe to replicate our procedure in pg. 11. An analogous recipe for replicating our inference procedure is in Appendix C3.\\

\noindent \textbf{Editor. P. 14: what is a* ? If that is a number, why not state it? Something else?}\\ 

\noindent Response. As our labor income example makes clear, $a^*$ is a point where the researcher observes both a realized value in the experimental sample and a forecast (output of the methodology). In our case, $a^* = 30$. But it could take many value across applications, and that's why we leave it in abstract. We want to provide the steps to replicate our methodology in other settings.\\
...
\noindent Response. Correct. This section has changed completely. \\

\noindent \textbf{Editor. P.15:you have not told the reader much about B0. See also my comment 8 above.}\\

\noindent Response. The same response applies to that of your previous comment.\\
...
\noindent Response. Yes it is., but we have clarified it in pg. 5. The standard reference relevant to this literature is \citet{Heckman_Pinto_etal_2013_PerryFactor}. We have provided that.\\
...
\noindent Response. We more concisely state why would this be important in pg. 20, and do present results using this alternative in Table 3 (last three columns). Instead of following the steps in pg. 11, an alternative is simply to study the average profiles of treatment-groups-alikes and control-group-alikes (and take the respective average difference between these two groups). That is what we call non-parametric matching. It is a useful check because it compresses our procedure to a simple exercise, but it relies on the same key assumptions (and that's why spelling out our method is relevant). We believe that this is a method that some readers will be familiar with, and will find appealing the robustness of our results to using it. \\
...
\noindent Response. The explanation that we gave before failed to communicate why the fact that the constructed profiles match is important. To explain this, we refer to the recipe that we provide in pg. 11. One thing is: obtain samples that resemble our treatment and control groups at baseline, or what we call constructing synthetic cohorts. The other thing is: using these samples to identify and estimate structural relationships and form a forecast. The reason why Figure 3 is a compelling revision of our construction of our methodology because in the auxiliary sample there is no treatment at all (everyone who satisfies $\bm{B} \in \mathcal{B}_0$ is observationally equivalent to a control-group child). Thus, our forecast for the average profile for the control group should be the same as the average profile for people who satisfy $\bm{B} \in \mathcal{B}_0$ in the auxiliary sample. We elaborate on this in Section 3.1.\\
...
\noindent Response. We now only keep subscripts for what is relevant to our methodology: treatment status, experimental and non-experimental samples, and age. The rest is gone, and was in fact convoluted.\\
...
\noindent Response. Let us clarify this because it is at the heart of our methodology. What we mean is that \textit{both} belonging to the experimental and non-experimental samples and treatment or control status could be summarized by $\bm{x}$ and $\bm{b}$. That is, the realization of an outcomes of interest could be backed out from the realizations of $\bm{x}$ and $\bm{b}$. That is why it is crucial for us to document that treatment is in fact operating on $\bm{x}$ and that $\phi \left( \cdot, \bm{b} \right)$ is a non-trivial function of $\bm{x}$, which we document in Section 3.2. If $\bm{x}$ and $\bm{b}$ summarize membership to either the experimental or the non-experimental sample and treatment status, then there is no $\phi_{k,j}^d$ for $d=0,1$ and $k = e,n$ but one and only one \textit{invariant} $\phi$.\\
...
\noindent Response. We wonder whether we would be able to keep that model there to make the table self-contained. Each of the specifications are particular cases of that model and the table would be hard to read if the reader is referred to text to go over the cases when reading the results. If you disagree, we will move this to the Appendix.\\

\noindent \textbf{Editor. Is, say, page 29 a verbal description of what you described more formally above? Something else? How do the two relate? And what do you do, exactly? For example, the paragraph ``Our methodology...'' on page 29 needs a more precise fleshing out in the technical appendix, and you should point to that in the main text.}\\

\noindent Response. As we clarify in text now, forecasting the outcomes other than labor income rely on similar assumptions and some additional steps. We provide brief details in Section 4 for each of these outcomes and then refer to the reader to each of the relevant appendices (there is one appendix for each additional outcome or labor income).\\
...
\noindent Response. Based on this comment, we have retitled our paper replacing the word prototypical with influential. We maintain that the program is influential, but as you very well say it may not be prototypical. In footnote 6, as well as Table A.33, we provide a list of programs around the world that have replicated ABC/CARE. We explicitly acknowledge in the introduction that ``were offered as a child development form of childcare,'' (pg.\ 1) and that ABC/CARE `` is a prototype of what early childhood education should be (e.g., high staff to child ratios, active monitoring of children, and relaxation of time constraints to single mothers starting form birth) and not necessarily what early childhood is nationwide.'' (pg.\ 2).\\

\noindent \textbf{Referee 1. Second, this paper unduly relies on citation of the authors' own unpublished analyses and sometimes the unpublished work of others. This is not necessary in most cases. The authors fail to cite in the appropriate locations the peer-reviewed publication(s) that make(s) the same point about many foundational issues, such as the study's original design and prior seminal findings. For example, the fact that the children in the control condition were not prevented from attending center-based childcare in the community appears in major prior articles and the benefits of this intermediate, non-randomized treatment ``substitute" has been effectively documented (notably, \citet{Burchinal_etal_1989_CD_Daycare-Pre-K-Dev}). Why would the authors cite an unpublished source of their own (specifically, \citet{Garcia_Heckman_Ziff_2017_Gender-Diff_UNPUBLISHED}, unpublished) for this fact? Actually, the Garcia et al (2017) is cited so often that one wonders how much it overlaps with this paper and what is needed from that not readily available paper to understand this current paper? This strongly detracts from the current paper.}\\

\noindent Response. This point is very well taken and we made an effort to: (i) cite \citet{Garcia_Heckman_Ziff_2017_Gender-Diff_UNPUBLISHED} only when necessary; and (ii) rely on sources that are primary to the facts that we intend to document or refer to. Examples of these appear mostly when we discuss the content of the program in Section 2: 

\begin{enumerate}
\item We refer to \citet{Burchinal_etal_1989_CD_Daycare-Pre-K-Dev} and other contemporaneous sources when discussing take-up of alternative childcare arrangements by the control group. 
...
\noindent About the frequency in which \citet{Garcia_Heckman_Ziff_2017_Gender-Diff_UNPUBLISHED}, we kept it to the minimal and explained the difference between that paper and this paper explicitly. We do want to know that \citet{Garcia_Heckman_Ziff_2017_Gender-Diff_UNPUBLISHED} is our companion paper. These two papers are the first to use administrative data on crime. Previous studies have relied on self-reported crime outcomes and have claimed (somewhat not surprisingly) no effects. We elaborate on this in Appendix E, which is devoted to criminal outcomes.\\

\noindent Perhaps this is too much detail for our paper so we place a summary this in a footnote 24 of pg.\ 14., given that our aim is to provide a forecasting methodology and not treatment effects on the observed outcomes but note that: \citet{Anderson_2008_JASA}---a often-cited study in this literature---provides estimates of factors for the Carolina Abecedarian Project (ABC) that combine many outcomes into a condensed summary measure, the data he uses are not current with important health, survey, and administrative data collected from the ABC sample after age 21, resulting in different results than what we report. The most glaring example of this is crime outcomes, whose treatment effects are not significant until after age 21 in ABC. We also note that previous literature does not separate the effects by gender and, as a consequence, do not find precise treatment effects on education and labor income. As we report in Table 1, the effects are substantial when considering effects by gender. Previous studies presenting treatment effects of ABC and CARE include \citet{Ramey_etal_1985_Project-CARE_TiECSE,Clarke_Campbell_1998_ABC_Comparison_ECRQ,Campbell_Pungello_etal_2001_DP,Campbell_Ramey_etal_2002_ADS,Campbell_Wasik_etal_2008_ECRQ,Campbell_Conti_etal_2014_EarlyChildhoodInvestments}. Only \citet{Campbell_Conti_etal_2014_EarlyChildhoodInvestments} divide the treatment effects by gender, however, to focus on the health effects, with men having many more positive effects especially in cardiovascular and metabolic conditions. This is consistent with the results we report below.\\
...
\noindent Response. We have added the information in the previous answer as a means to clarify for the reader what is the source of the differences between our results the results in previous papers.\\
...
\noindent Response. The difference in outcomes should be clear from the answer before. We speculate that the other assumptions that we make have to do with the fact that we assume that (i) once treatment is offered, all of the families agree to participate of the program; (ii) once randomized into treatment, there is full compliance.\\ 

\noindent We believe that the work that we have done to document the randomization protocol and the sample of participants is novel in its thorough detail. Previous sources have disqualified the validity of the randomization protocol without knowledge of the causes why different individuals attrit from the sample as the data collection process evolved \citep[e.g.][]{Whitehurst_2014_Senate_Testimony}. While this is too much detail for our main text, Appendix A (and in particular Figures A3 and A4 and Tables A3 and A4) detail exactly why these assumptions hold. Instead of disregarding the sample as invalid, we went back to the original program designers and files and found out that attrition is actually not concerning. For ABC, which validity has been questioned, we argue that our assumptions are sound because most cases of non-compliance were exogenous to the program implementers or to household choices and we elaborate on how we deal with them in Appendix A and summarize next:

\begin{enumerate}

\item Four individuals in the treatment group did not complete the treatment. They left 3, 10, 6, and 9 months after the treatment began. We treat the as though they have had treatment (because they did, at least for a few months). In later life follow ups, we fully observe outcomes for this children and include them in our computations. This would only bias our results downwards, as this children did not complete treatment. In terms of information loss for estimating treatment effects, that is not an issue for these four children.

\item Four individuals died: two of them in the treatment group and two of them in the control group. Given the causes of death (heart disease for two of them, SIDS for one of them, and a pedestrian accident for one of them), we interpret this as missing at random. This would be information lost at random.
...
\noindent Response. Our summary and conclusions section, Section 8, is now devoted to briefly summarize and qualify its findings to assess and acknowledge these important caveats.\\
...
\noindent Response. We cannot provide a direct solution to the fact that the paper is very long because there is a lot to document given the nature of the paper. However, we intended to make the paper much more concrete, in part using a lot of the feedback that you provided. In sum: (i) we briefly introduced the issue and illustrated with labor income, which is workhorse example in what remains of the paper; (ii) provided a background and data section; (iii) and then headed directly to the forecast of the paper. In pg.\ 11, we provide a recipe of the exact steps that we follow, hoping that the reader can understand exactly what is being done. In that case, the Appendix could become simply a means for consulting details, instead of explicit content that the reader needs to understand (which we agree that is not practical or realistic). We hope that readers with different interests are able to consult different sections of the appendix.\\
...
\noindent Responde. After reformatting the paper, we are actually documenting in the paper all of the ingredientes to form the labor income forecast: (i) a motivation of the issue and a summary of the main ingredients and results in the introduction; (ii) a brief summary of the program and data in Section 2, the theoretical components and, importantly, a list disguising each step in Section 3, and so on. The paper should be self-contained in terms of forecasting labor income. For the rest of the outcomes, the relevant appendices are cited.\\
...
\noindent Response. That is what we do now. We summarize our approach in the introduction and illustrate it using labor income. The reader, after finishing the introduction, should now have a clear idea of what the methodology is. The rest of the paper documents the crucial details and the rest is in the Appendix.\\

\noindent \textbf{Referee 3. Appendix C.3 is important, and much of it is necessary to understand what you actually, and the pros and cons of alternative approaches.}\\

\noindent Response. We brought crucial ingredients of Appendix C.3 into the text. In particular, after listing each of the components in our forecasting procedure in pg.\ 11, we proceed to document and test the relevant steps associated with each step. For example, Section 3.1 provides relevant details on the construction of the synthetic cohorts, Section 3.2 documents the treatment effects on the inermediate inputs that we use to forecast, and Section 3.4. provides the tests of invariance. These elements were relegated to Appendix C.3, while being crucial.\\
...
\noindent Response. We have relegated this, and particularly Table 1 of the previous version, to Appendix C.\\
...
\noindent Response. Yes. It is possible to proceed under weaker assumptions. It is actually the case that the only object to forecast is the average difference between the treatment and control. This weaker condition is provided in pg. 17, footnote 27. We do note, however, that the stronger the assumption the sharper the testable implication the researcher is able to obtain. Mean independence allows us to derive the implications in Equations (5) and (6) and provide the tests in Figures 1 and 3. This would not be possible if Equation (4) were the only implication.\\
...
\noindent Response. Thanks for asking for this clarification, which was missing in the previous draft. To make clear this, we devote a brief section, Section 6, to this issue. This is more auxiliary to our study because our main interest is to compare treatment to control, so we discuss it as complementary and not as a main result. We acknowledge that dealing with substitution bias relies on further assumptions, which may convolute too much our exposition if presented as a main result.\\
...
\noindent Response. As detailed in pg.\ 10, there are predictors not affected by treatment, $\bm{B}_k$, and predictors affected by treatment, $\bm{X}_{k,a}^d$. The former allow us to gain precision when identifying and estimating $\phi$ in the auxiliary samples. We choose them to be the set of variables that are available across the two samples and are available at baseline in the experimental sample. The latter are the most important ones, as our whole methodology relies on them as ``mediators'' of treatment. That is precisely what is embedded in Assumption A-2. As detailed in pg.\ 11, a variable is a predictor variables: (i) are availlable in the experimental and the non-experimental data; (ii) the support in the non- experimental data contains the support in the experimental data; (iii) mediate or cause labor income; and (iv) are affected by treatment. Section 3.2 documents that these holds for the intermediate inputs that we use.\\
...
\noindent Response. This is an excellent point that we cannot attack practically.\\ 
...
\noindent Response. We believe that the new exposition of the methodology has clarified this. By assuming that treatment is mediated by $\bm{X}_{k,a}^d$ the treatment-control differences in any outcomes of interest will result from the treatment-control difference in $\bm{X}_{k,a}^d$ (as dictated by the structural function $\phi$). One could think of doing this non-parametrically and that is exactly what we discuss in Section 3.6.\\
...
\noindent Response. We appreciate this point, but, again, we are limited by practicalities in this regard. To bring up a serious discussion in our paper about essential heterogeneity, we would need to allow for heterogeneous responses to treatment. In our small-sample experiment, this proves impossible. We approximate a complex reality by providing estimate of the average treatment effect of the program, and statistics related to it, for the population for which $\bm{B} \in \mathcal{B}_0$, assuming a homogenous response.\\
