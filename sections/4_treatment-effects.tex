...
\textbf{[JJH: Why these 7?][See above, we thought these were illustrative.]}
\centering
...
The estimates contrasting the effects for females and males in Columns (3) and (5) are not based on matching; the estimates in (4) and (6) are. For the matching estimates, we rely on observed, baseline characteristics. In Appendix~\ref{appendix:bvariables}, we explain our choice of these variables and we make a thorough analysis to conclude that there is little sensitivity to the choice of these variables.\footnote{We also present this sensitivity analysis changing the variables used to condition while estimating treatment effects and changing the variables used to construct the weights to account for attrition.}

Finally, we test whether estimated treatment effects are equal across genders. Table~\ref{table:genderdifference-tes} reports the two-sided $p$-values testing if the estimates in (1) of Tables~\ref{table:tescombinedmales} and~\ref{table:tescombinedfemales} are equal. Males have larger treatment effects on parental income at age 21, labor income, drug use, and on the measures of blood pressure. The treatment effects are significantly different for most outcomes. \textbf{[JJH: What happens for full 95?][We report these in Appendix E. Please note they have expanded to 126. We edited the text accordingly.]}

\begin{sidewaystable}[!htbp]
\textbf{[JJH: There are few gender differences! I thought we found more. The paper collapses with this table. Do a rank sign test.][We implemented this test, and it substantially helped. Please see the new results below.]}
\centering
\begin{threeparttable}
