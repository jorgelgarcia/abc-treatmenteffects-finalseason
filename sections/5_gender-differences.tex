...
To summarize these treatment effects across categories of outcomes, we provide a count of the number of positive treatment effects for the different counterfactual comparisons. These are based on 126 outcomes reported in Appendix B. Due to lack of power after dividing the control group into staying at home and attending alternative childcares, we rely solely on the combining functions rather than on the exact non-parametric tests listed in Tables 1 and 4.\footnote{We adjust for dependence across outcomes and for pretesting in constructing standard errors (see Appendix B). We account for non-random selection into alternative childcare using matching, as described in Section 3.}

Figures~\ref{fig:ppositive} display the estimated combining functions. Under the null hypothesis, the proportion of positive treatment effects if $.50$. Men benefit more from treatment when compared to attending alternatives, as opposed to staying at home. For women the benefits are similar in both comparisons according to the combining functions. In Appendix~\ref{}, we show that women generally benefit more from treatment if compared to staying at home, as opposed to attending alternatives. We lack the power to verify statistically this pattern using combining functions. 
